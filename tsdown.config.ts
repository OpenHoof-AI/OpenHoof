...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.OPENHOOF_BUILD_VERBOSE === "1") {
    return undefined;
  }
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8")) as {
          openhoof?: { extensions?: unknown; setupEntry?: unknown };
        };
        packageEntries = Array.isArray(packageJson.openhoof?.extensions)
          ? packageJson.openhoof.extensions.filter(
              (entry): entry is string => typeof entry === "string" && entry.trim().length > 0,
            )
          : [];
        const setupEntry =
          typeof packageJson.openhoof?.setupEntry === "string" &&
          packageJson.openhoof.setupEntry.trim().length > 0
            ? packageJson.openhoof.setupEntry
            : undefined;
        if (setupEntry) {
