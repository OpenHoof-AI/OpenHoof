...
const { clearPluginCommands, getPluginCommandSpecs, loadOpenHoofPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadOpenHoofPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@openhoof/build-smoke-plugin",
      type: "module",
      openhoof: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'openhoof/plugin-sdk';",
...
const registry = loadOpenHoofPlugins({
  ...
  env: {
    ...process.env,
    OPENHOOF_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
    OPENHOOF_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
  },
  config: {
    plugins: {
      enabled: true,
