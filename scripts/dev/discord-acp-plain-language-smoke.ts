...
type DriverMode = "token" | "webhook" | "openhoof";
...
function resolveStateDir(): string {
  const override = process.env.OPENHOOF_STATE_DIR?.trim() || process.env.HOOFBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.OPENHOOF_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".openhoof");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver openhoof] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) OpenHoof spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|openhoof> Driver transport mode (default: token)\n" +
    ...
    "  --openhoof-bin <path>        OpenHoof CLI binary for driver=openhoof (default: openhoof)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  OPENHOOF_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  OPENHOOF_DISCORD_SMOKE_DRIVER\n" +
    "  OPENHOOF_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  OPENHOOF_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  OPENHOOF_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  OPENHOOF_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  OPENHOOF_DISCORD_SMOKE_AGENT\n" +
    "  OPENHOOF_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  OPENHOOF_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  OPENHOOF_DISCORD_SMOKE_POLL_MS\n" +
    "  OPENHOOF_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  OPENHOOF_DISCORD_SMOKE_OPENHOOF_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.OPENHOOF_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.HOOFBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.OPENHOOF_DISCORD_SMOKE_DRIVER ||
    process.env.HOOFBOT_DISCORD_SMOKE_DRIVER ||
    "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "openhoof"
        ? "openhoof"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.OPENHOOF_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.HOOFBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.OPENHOOF_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.OPENHOOF_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.HOOFBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.OPENHOOF_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.OPENHOOF_DISCORD_SMOKE_AGENT ||
    process.env.HOOFBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.OPENHOOF_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.HOOFBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.OPENHOOF_DISCORD_SMOKE_INSTRUCTION ||
    process.env.HOOFBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.OPENHOOF_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.OPENHOOF_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.OPENHOOF_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const openhoofBin =
    resolveArg("--openhoof-bin") || process.env.OPENHOOF_DISCORD_SMOKE_OPENHOOF_BIN || "openhoof";
...
async function openhoofCliJson<T>(params: { openhoofBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.openhoofBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`openhoof ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithOpenhoof(params: {
  openhoofBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await openhoofCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    openhoofBin: params.openhoofBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "openhoof") {
    return await readMessagesWithOpenhoof({
      openhoofBin: params.args.openhoofBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `openhoof-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await openhoofCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("openhoof message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "openhoof"
            ? await readMessagesWithOpenhoof({
                openhoofBin: args.openhoofBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from OpenHoof.`,
        diagnostics: {
          bindingCandidates: [
            {
