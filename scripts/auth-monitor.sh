...
#
# Suggested cron: */30 * * * * /home/admin/openhoof/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenHoof notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., openhoof-alerts)
...
STATE_FILE="$HOME/.openhoof/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenHoof if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openhoof
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenHoof to $NOTIFY_PHONE..."
            openhoof send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenHoof is down. Run: ssh l36 '~/openhoof/scripts/mobile-reauth.sh'" "urgent"
    exit 1
