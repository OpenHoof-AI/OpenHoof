...
INSTALL_URL="${OPENHOOF_INSTALL_URL:-https://openhoof.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENHOOF_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENHOOF_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openhoof"
PACKAGE_NAME="${OPENHOOF_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENHOOF_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENHOOF_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENHOOF_INSTALL_LATEST_OUT:-}"
fi
