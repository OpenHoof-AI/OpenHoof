...
function stripWorkspaceOpenhoofDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.openhoof !== "workspace:*") {
    return false;
  }
  delete devDeps.openhoof;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceOpenhoofDevDependency(pkg);
