...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "openhoof.plugin.json");
    const distPluginDir = path.join(distExtensionsRoot, dirent.name);
    const distManifestPath = path.join(distPluginDir, "openhoof.plugin.json");
    ...
    if (packageJson.openhoof && "extensions" in packageJson.openhoof) {
      packageJson.openhoof = {
        ...packageJson.openhoof,
        extensions: rewritePackageExtensions(packageJson.openhoof.extensions),
        ...(typeof packageJson.openhoof.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.openhoof.setupEntry) }
          : {}),
      };
    }
