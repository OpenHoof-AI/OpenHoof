...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openhoof/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenHoof Re-Auth" -c "After getting key, run: ssh $SERVER '~/openhoof/scripts/mobile-reauth.sh'" --id openhoof-auth
        ;;
    *)
