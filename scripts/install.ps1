# OpenHoof Installer for Windows (PowerShell)
# Usage: iwr -useb https://openhoof.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://openhoof.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\openhoof",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🐴 OpenHoof Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one OpenHoof.$NC" -Level info
    Write-Host ""
}
...
function Install-OpenHoofGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing OpenHoof from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/openhoof/openhoof.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\openhoof\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\openhoof.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install OpenHoof from git to $GitDir" -Level info
        } else {
            Install-OpenHoofGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install OpenHoof via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-OpenHoofNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'openhoof onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🐴 OpenHoof installed successfully!" -Level success
}
