 ...
 * `openhoof update`. Use it to verify the completion installation flow
...
\`openhoof update\`, \`openhoof doctor\`, and \`openhoof onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenHoof Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`openhoof completion --install\` later to enable.`));
    return;
  }
