#!/bin/bash
# Setup OpenHoof Auth Management System
...
echo "=== OpenHoof Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openhoof-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenHoof message: Send warning via OpenHoof itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openhoof-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openhoof-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openhoof-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openhoof-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/HoofdAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/HoofdAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Hoofd*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (HoofdAuth) shows status and opens auth URL if needed."
echo "The full widget (HoofdAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status openhoof-auth-monitor.timer"
echo ""
echo "Quick commands:"
