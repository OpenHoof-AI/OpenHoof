...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "openhoof.json");
  ...
  const sessionId =
    process.env.OPENHOOF_ZAI_FALLBACK_SESSION_ID ??
    process.env.HOOFBOT_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const baseEnv: NodeJS.ProcessEnv = {
    ...
    HOOFBOT_CONFIG_PATH: configPath,
    HOOFBOT_STATE_DIR: stateDir,
    ZAI_API_KEY: zaiKey,
    Z_AI_API_KEY: "",
  };
  ...
  const run1 = await runCommand(
    "run1",
    ["openhoof", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["openhoof", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
