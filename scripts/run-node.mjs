...
const extensionRestartMetadataFiles = new Set(["openhoof.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.OPENHOOF_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENHOOF_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[openhoof] ${message}\n`);
};

const runOpenHoof = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["openhoof.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
