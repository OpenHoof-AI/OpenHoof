...
import type { ApiContributor, Entry, MapConfig, User } from "./update-hooftributors.types.js";

const REPO = "openhoof/openhoof";
const PER_LINE = 10;

const mapPath = resolve("scripts/hooftributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing hooftributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README hooftributors: ${entries.length} entries`);
