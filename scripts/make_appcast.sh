...
ZIP=${1:?"Usage: $0 OpenHoof-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/openhoof/openhoof/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^OpenHoof-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/openhoof/openhoof/releases/download/v${VERSION}/"}
