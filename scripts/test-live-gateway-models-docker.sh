...
docker run --rm -t \
  ...
  -e OPENHOOF_LIVE_GATEWAY_MODELS="${OPENHOOF_LIVE_GATEWAY_MODELS:-${HOOFBOT_LIVE_GATEWAY_MODELS:-modern}}" \
  -e OPENHOOF_LIVE_GATEWAY_PROVIDERS="${OPENHOOF_LIVE_GATEWAY_PROVIDERS:-${HOOFBOT_LIVE_GATEWAY_PROVIDERS:-}}" \
  -e OPENHOOF_LIVE_GATEWAY_MAX_MODELS="${OPENHOOF_LIVE_GATEWAY_MAX_MODELS:-${HOOFBOT_LIVE_GATEWAY_MAX_MODELS:-24}}" \
  -e OPENHOOF_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${OPENHOOF_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-${HOOFBOT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}}" \
  -v "$ROOT_DIR":/src:ro \
