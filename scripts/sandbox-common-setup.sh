...
BASE_IMAGE="${BASE_IMAGE:-openhoof-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-openhoof-sandbox-common:bookworm-slim}"
...
OPENHOOF_DOCKER_BUILD_USE_BUILDX="${OPENHOOF_DOCKER_BUILD_USE_BUILDX:-0}"
OPENHOOF_DOCKER_BUILD_CACHE_FROM="${OPENHOOF_DOCKER_BUILD_CACHE_FROM:-}"
OPENHOOF_DOCKER_BUILD_CACHE_TO="${OPENHOOF_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${OPENHOOF_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${OPENHOOF_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${OPENHOOF_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${OPENHOOF_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${OPENHOOF_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=openhoof.sandbox=1)
NOTE
