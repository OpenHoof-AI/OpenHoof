...
IMAGE_NAME="openhoof-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  OPENHOOF_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  OPENHOOF_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export OPENHOOF_ENTRY

home_dir=$(mktemp -d "/tmp/openhoof-plugins-e2e.XXXXXX")
...
JS
  cat > "$dir/openhoof.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/openhoof.plugin.json" "$id"
...
mkdir -p "$HOME/.openhoof/extensions/demo-plugin"

cat > "$HOME/.openhoof/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.openhoof/extensions/demo-plugin/openhoof.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$OPENHOOF_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/openhoof-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@openhoof/demo-plugin-tgz",
  "version": "0.0.1",
  "openhoof": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/openhoof.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$OPENHOOF_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$OPENHOOF_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/openhoof-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@openhoof/demo-plugin-dir",
  "version": "0.0.1",
  "openhoof": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/openhoof.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$OPENHOOF_ENTRY" plugins install "$dir_plugin"
node "$OPENHOOF_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/openhoof-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@openhoof/demo-plugin-file",
  "version": "0.0.1",
  "openhoof": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/openhoof.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$OPENHOOF_ENTRY" plugins install "file:$file_pack_dir/package"
node "$OPENHOOF_ENTRY" plugins list --json > /tmp/plugins4.json
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "openhoof/fixture-marketplace"
    }
  }
}
JSON

node "$OPENHOOF_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$OPENHOOF_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$OPENHOOF_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$OPENHOOF_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
const configPath = path.join(process.env.HOME, ".openhoof", "openhoof.json");
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$OPENHOOF_ENTRY" plugins update marketplace-shortcut --dry-run
node "$OPENHOOF_ENTRY" plugins update marketplace-shortcut
node "$OPENHOOF_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
