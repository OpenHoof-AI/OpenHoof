...
INSTALL_URL="https://openhoof.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_OPENHOOF_BIN="/opt/homebrew/bin/openhoof"
GUEST_OPENHOOF_ENTRY="/opt/homebrew/lib/node_modules/openhoof/openhoof.mjs"
...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/openhoof-parallels-smoke.XXXXXX)"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://openhoof.ai/install.sh
...
spawn {*}$cmd
send -- "printf '__OPENHOOF_READY__\\n'\r"
expect "__OPENHOOF_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/openhoof-prl.sh <<'__OPENHOOF_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__OPENHOOF_SCRIPT__\r"
send -- "/bin/bash /tmp/openhoof-prl.sh; rc=\$?; rm -f /tmp/openhoof-prl.sh; printf '__OPENHOOF_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__OPENHOOF_RC__:(-?[0-9]+)} {
...
export OPENHOOF_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/openhoof-install.sh
bash /tmp/openhoof-install.sh${version_arg_q}
$GUEST_OPENHOOF_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_OPENHOOF_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/openhoof-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/openhoof"; check_path "\$root/openhoof/extensions"; if [ -d "\$root/openhoof/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/openhoof/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
run_ref_onboard() {
  guest_current_user_exec \
    ...
    "$GUEST_NODE_BIN" "$GUEST_OPENHOOF_ENTRY" onboard \
    --non-interactive \
    --mode local \
...
verify_gateway() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPENHOOF_ENTRY" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPENHOOF_ENTRY" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPENHOOF_ENTRY" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPENHOOF_ENTRY" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPENHOOF_ENTRY" agent --agent main --message ping --json
}
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/openhoof-discord-token <<'__OPENHOOF_TOKEN__'
$DISCORD_TOKEN_VALUE
__OPENHOOF_TOKEN__
cat >/tmp/openhoof-discord-guilds.json <<'__OPENHOOF_GUILDS__'
$guilds_json
__OPENHOOF_GUILDS__
token="\$(tr -d '\n' </tmp/openhoof-discord-token)"
guilds_json="\$(cat /tmp/openhoof-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_OPENHOOF_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_OPENHOOF_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_OPENHOOF_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_OPENHOOF_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_OPENHOOF_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_OPENHOOF_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_OPENHOOF_ENTRY channels status --probe --json
rm -f /tmp/openhoof-discord-token /tmp/openhoof-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "openhoof-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "openhoof-main-upgrade.tgz"
