...
IMAGE_NAME="openhoof-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openhoof-net-e2e-$$"
GW_NAME="openhoof-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "OPENHOOF_GATEWAY_TOKEN=$TOKEN" \
  -e "OPENHOOF_SKIP_CHANNELS=1" \
  -e "OPENHOOF_SKIP_GMAIL_WATCHER=1" \
  -e "OPENHOOF_SKIP_CRON=1" \
  -e "OPENHOOF_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
