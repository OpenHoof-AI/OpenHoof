...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/openhoof-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/openhoof-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/openhoof-install.sh
  guest_exec /usr/bin/env OPENHOOF_NO_ONBOARD=1 bash /tmp/openhoof-install.sh "${version_args[@]}" --no-onboard
  guest_exec openhoof --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec openhoof --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" openhoof onboard \
    --non-interactive \
    --mode local \
...
verify_local_turn() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" openhoof agent \
    --local \
    --agent main \
    --message ping \
    --json
