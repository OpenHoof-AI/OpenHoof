...
IMAGE_NAME="openhoof-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/openhoof-bin:$PATH"
  mkdir -p /tmp/openhoof-bin
  cat > /tmp/openhoof-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/openhoof-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$OPENHOOF_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_openhoof_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/openhoof-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/openhoof-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$OPENHOOF_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/openhoof-e2e-$1.XXXXXX"
  }

  set_isolated_openhoof_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export OPENHOOF_HOME="$home_dir"
    export OPENHOOF_STATE_DIR="$home_dir/.openhoof"
    export OPENHOOF_CONFIG_PATH="$OPENHOOF_STATE_DIR/openhoof.json"
    mkdir -p "$OPENHOOF_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$OPENHOOF_STATE_DIR/workspace"
    config_path="$OPENHOOF_CONFIG_PATH"
    sessions_dir="$OPENHOOF_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_openhoof_env "$home_dir"
	    ...
	    node "$OPENHOOF_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$OPENHOOF_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$OPENHOOF_ENTRY\" configure --section skills" send_skills_flow
