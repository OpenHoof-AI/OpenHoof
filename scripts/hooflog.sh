...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "hooflog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
hooflog - OpenHoof Logging Utility

USAGE:
    hooflog [OPTIONS]

DESCRIPTION:
    View OpenHoof logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    OpenHoof logs flow through the macOS unified log (subsystem: ai.openhoof).
...
QUICK START:
    hooflog -n 100             Show last 100 lines from all components
    hooflog -f                 Follow logs in real-time
    hooflog -e                 Show only errors
    hooflog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    hooflog                   Show last 50 lines from past 5 minutes (default)
    hooflog -f                Stream logs continuously
    hooflog -n 100            Show last 100 lines
    hooflog -e                Show only recent errors
    hooflog -l 30m -n 200     Show last 200 lines from past 30 minutes
    hooflog -c ServerManager  Show recent ServerManager logs
    hooflog -s "fail"         Search for "fail" in recent logs
    hooflog --server -e       Show recent server errors
    hooflog -f -d             Stream debug logs continuously
