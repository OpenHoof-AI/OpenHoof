...
import { parseReleaseVersion } from "../openhoof-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const extensions = packageJson.openhoof?.extensions ?? [];

  if (!packageName.startsWith("@openhoof/")) {
    errors.push(
      `package name must start with "@openhoof/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("openhoof.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("openhoof.extensions must contain only non-empty strings.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const dir of dirs) {
    ...
    if (packageJson.openhoof?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: packageJson.openhoof?.install?.npmSpec?.trim() || undefined,
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "openhoof-plugin-npm-view-"));
