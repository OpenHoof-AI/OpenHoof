...
- regenerates apps/ios/OpenHoof.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${OPENHOOF_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid OPENHOOF_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid OPENHOOF_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid OPENHOOF_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid OPENHOOF_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing OPENHOOF_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(OPENHOOF_URL_SLASH)$(OPENHOOF_URL_SLASH)#g'
)"
...
OPENHOOF_CODE_SIGN_STYLE = Automatic
OPENHOOF_DEVELOPMENT_TEAM = ${TEAM_ID}
OPENHOOF_IOS_SELECTED_TEAM = ${TEAM_ID}
OPENHOOF_APP_BUNDLE_ID = ai.openhoof.client
OPENHOOF_SHARE_BUNDLE_ID = ai.openhoof.client.share
OPENHOOF_ACTIVITY_WIDGET_BUNDLE_ID = ai.openhoof.client.activitywidget
OPENHOOF_WATCH_APP_BUNDLE_ID = ai.openhoof.client.watchkitapp
OPENHOOF_WATCH_EXTENSION_BUNDLE_ID = ai.openhoof.client.watchkitapp.extension
OPENHOOF_APP_PROFILE =
OPENHOOF_SHARE_PROFILE =
OPENHOOF_PUSH_TRANSPORT = relay
OPENHOOF_PUSH_DISTRIBUTION = official
OPENHOOF_URL_SLASH = /
OPENHOOF_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
OPENHOOF_PUSH_APNS_ENVIRONMENT = production
EOF
