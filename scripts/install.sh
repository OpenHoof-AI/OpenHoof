...
# OpenHoof Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://openhoof.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one OpenHoof."
...
GUM_VERSION="${OPENHOOF_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🐴 OpenHoof Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://openhoof.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$OPENHOOF_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.openhoof.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_openhoof_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.openhoof-* "$npm_root"/openhoof 2>/dev/null || true
}

extract_openhoof_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_openhoof_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/openhoof" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/openhoof"|"/usr/local/bin/openhoof")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/openhoof/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale openhoof symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing openhoof binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing OpenHoof package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_openhoof_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "OpenHoof npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*openhoof" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_openhoof_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_openhoof_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "OpenHoof npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an openhoof binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES=()
TAGLINES+=("Your terminal just grew hooves—type something and let the bot stomp the busywork.")
...
TAGLINES+=("Automation with hooves: minimal fuss, maximal stomp.")
...
TAGLINES+=("Hooves out, commit in—let's ship something mildly responsible.")
TAGLINES+=("I'll butter your workflow like a mustang roll: messy, delicious, effective.")
...
HOLIDAY_CHRISTMAS="Christmas: Ho ho ho—Santa's little hoof-sistant is here to ship joy, roll back chaos, and stash the keys safely."
...
map_legacy_env "OPENHOOF_TAGLINE_INDEX" "HOOFBOT_TAGLINE_INDEX"
map_legacy_env "OPENHOOF_NO_ONBOARD" "HOOFBOT_NO_ONBOARD"
map_legacy_env "OPENHOOF_NO_PROMPT" "HOOFBOT_NO_PROMPT"
map_legacy_env "OPENHOOF_DRY_RUN" "HOOFBOT_DRY_RUN"
map_legacy_env "OPENHOOF_INSTALL_METHOD" "HOOFBOT_INSTALL_METHOD"
map_legacy_env "OPENHOOF_VERSION" "HOOFBOT_VERSION"
map_legacy_env "OPENHOOF_BETA" "HOOFBOT_BETA"
map_legacy_env "OPENHOOF_GIT_DIR" "HOOFBOT_GIT_DIR"
map_legacy_env "OPENHOOF_GIT_UPDATE" "HOOFBOT_GIT_UPDATE"
map_legacy_env "OPENHOOF_NPM_LOGLEVEL" "HOOFBOT_NPM_LOGLEVEL"
map_legacy_env "OPENHOOF_VERBOSE" "HOOFBOT_VERBOSE"
map_legacy_env "OPENHOOF_PROFILE" "HOOFBOT_PROFILE"
map_legacy_env "OPENHOOF_INSTALL_SH_NO_RUN" "HOOFBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${OPENHOOF_TAGLINE_INDEX:-}" ]]; then
        if [[ "${OPENHOOF_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((OPENHOOF_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${OPENHOOF_NO_ONBOARD:-0}
NO_PROMPT=${OPENHOOF_NO_PROMPT:-0}
DRY_RUN=${OPENHOOF_DRY_RUN:-0}
INSTALL_METHOD=${OPENHOOF_INSTALL_METHOD:-}
OPENHOOF_VERSION=${OPENHOOF_VERSION:-latest}
USE_BETA=${OPENHOOF_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/openhoof"
GIT_DIR=${OPENHOOF_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${OPENHOOF_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${OPENHOOF_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${OPENHOOF_VERBOSE:-0}"
VERIFY_INSTALL="${OPENHOOF_VERIFY_INSTALL:-0}"
OPENHOOF_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
OpenHoof installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://openhoof.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/openhoof)
...
Environment variables:
  OPENHOOF_INSTALL_METHOD=git|npm
  OPENHOOF_VERSION=latest|next|main|<semver>|<spec>
  OPENHOOF_BETA=0|1
  OPENHOOF_GIT_DIR=...
  OPENHOOF_GIT_UPDATE=0|1
  OPENHOOF_NO_PROMPT=1
  OPENHOOF_VERIFY_INSTALL=1
  OPENHOOF_DRY_RUN=1
  OPENHOOF_NO_ONBOARD=1
  OPENHOOF_VERBOSE=1
  OPENHOOF_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://openhoof.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://openhoof.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://openhoof.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://openhoof.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://openhoof.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected OpenHoof checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a OpenHoof source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_openhoof_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"openhoof"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://openhoof.ai/install.sh | bash"
}
...
ensure_openhoof_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/openhoof" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/openhoof" ]]; then
        ln -sf "$npm_root/openhoof/dist/entry.js" "${npm_bin}/openhoof"
        ui_info "Created openhoof bin link at ${npm_bin}/openhoof"
    fi
    return 0
}

# Check for existing OpenHoof installation
check_existing_openhoof() {
    if [[ -n "$(type -P openhoof 2>/dev/null || true)" ]]; then
        ui_info "Existing OpenHoof installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make openhoof show as \"command not found\" in new terminals."
...
warn_openhoof_not_found() {
    ui_warn "Installed, but openhoof is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t openhoof 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named openhoof; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_openhoof_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P openhoof 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/openhoof" ]]; then
        echo "${npm_bin}/openhoof"
        return 0
    fi
...
install_openhoof_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/openhoof/openhoof.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing OpenHoof from git checkout: ${repo_dir}"
    else
        ui_info "Installing OpenHoof from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning OpenHoof" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building OpenHoof" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/openhoof" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/openhoof"
    ui_success "OpenHoof wrapper installed to \$HOME/.local/bin/openhoof"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view openhoof dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:openhoof/openhoof#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_openhoof() {
    local package_name="openhoof"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            OPENHOOF_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="openhoof"
        else
            OPENHOOF_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${OPENHOOF_VERSION}" ]]; then
        OPENHOOF_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${OPENHOOF_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${OPENHOOF_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing OpenHoof v${resolved_version}"
    else
        ui_info "Installing OpenHoof (${OPENHOOF_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${OPENHOOF_VERSION}")"

    if ! install_openhoof_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_openhoof_paths
        install_openhoof_npm "${install_spec}"
    fi

    if [[ "${OPENHOOF_VERSION}" == "latest" && "${package_name}" == "openhoof" ]]; then
        if ! resolve_openhoof_bin &> /dev/null; then
            ui_warn "npm install openhoof@latest failed; retrying openhoof@next"
            cleanup_npm_openhoof_paths
            install_openhoof_npm "openhoof@next"
        fi
    fi

    ensure_openhoof_bin_link || true

    ui_success "OpenHoof installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local hoof="${OPENHOOF_BIN:-}"
    if [[ -z "$hoof" ]]; then
        hoof="$(resolve_openhoof_bin || true)"
    fi
    if [[ -z "$hoof" ]]; then
        ui_info "Skipping doctor (openhoof not on PATH yet)"
        warn_openhoof_not_found
        return 0
    fi
    run_quiet_step "Running doctor" "$hoof" doctor --non-interactive || true
...
maybe_open_dashboard() {
    ...
    if ! "$hoof" dashboard --help >/dev/null 2>&1; then
        return 0
    fi
    "$hoof" dashboard || true
}
...
resolve_workspace_dir() {
    local profile="${OPENHOOF_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.openhoof/workspace-${profile}"
    else
        echo "${HOME}/.openhoof/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${OPENHOOF_CONFIG_PATH:-$HOME/.openhoof/openhoof.json}"
    if [[ -f "${config_path}" || -f "$HOME/.hoofbot/hoofbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run openhoof onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$hoof" ]]; then
        ui_info "BOOTSTRAP.md found but openhoof not on PATH; skipping onboarding"
        warn_openhoof_not_found
        return
    fi

    "$hoof" onboard || {
        ui_error "Onboarding failed; run openhoof onboard to retry"
        return
    }
}
...
resolve_openhoof_version() {
    local version=""
    local raw_version_output=""
    local hoof="${OPENHOOF_BIN:-}"
    if [[ -z "$hoof" ]] && command -v openhoof &> /dev/null; then
        hoof="$(command -v openhoof)"
    fi
    if [[ -n "$hoof" ]]; then
        raw_version_output=$("$hoof" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_openhoof_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/openhoof/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/openhoof/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$("$hoof" daemon status --json 2>/dev/null || true)"
...
refresh_gateway_service_if_loaded() {
    ...
    if ! is_gateway_daemon_loaded "$hoof"; then
        return 0
    fi

    ui_info "Refreshing loaded gateway service"
    if run_quiet_step "Refreshing gateway service" "$hoof" gateway install --force; then
    ...
    if run_quiet_step "Restarting gateway service" "$hoof" gateway restart; then
    ...
    run_quiet_step "Probing gateway service" "$hoof" gateway status --deep || true
}
...
verify_installation() {
    ...
    if [[ -z "$hoof" ]]; then
        ui_error "Install verify failed: openhoof not on PATH yet"
        warn_openhoof_not_found
        return 1
    fi

    run_quiet_step "Checking OpenHoof version" "$hoof" --version || return 1

    if is_gateway_daemon_loaded "$hoof"; then
        run_quiet_step "Checking gateway service" "$hoof" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: openhoof gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_openhoof_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found OpenHoof checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set OPENHOOF_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_openhoof; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing OpenHoof"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g openhoof &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g openhoof 2>/dev/null || true
        ...
        install_openhoof_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/openhoof" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/openhoof"
    ...
    OPENHOOF_BIN="$(resolve_openhoof_bin || true)"
    ...
    installed_version=$(resolve_openhoof_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🐴 OpenHoof installed successfully (${installed_version})!"
    else
        ui_celebrate "🐴 OpenHoof installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            "Leveled up! New skills unlocked. You're welcome."
            "Fresh code, same mustang. Miss me?"
            ...
            "The mustang has molted. Harder shell, sharper hooves."
        ...
        local completion_messages=(
            ...
            "cracks hooves Alright, what are we building?"
            "The mustang has landed. Your terminal will never be the same."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/openhoof"
        ui_kv "Update command" "openhoof update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://openhoof.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            local -a doctor_args=()
            if [[ "$NO_ONBOARD" == "1" ]]; then
                if "$hoof" doctor --help 2>/dev/null | grep -q -- "--non-interactive"; then
                    doctor_args+=("--non-interactive")
                fi
            fi
            ui_info "Running openhoof doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                OPENHOOF_UPDATE_IN_PROGRESS=1 "$hoof" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                OPENHOOF_UPDATE_IN_PROGRESS=1 "$hoof" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                OPENHOOF_UPDATE_IN_PROGRESS=1 "$hoof" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run openhoof doctor and openhoof plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run openhoof onboard later"
        else
            local config_path="${OPENHOOF_CONFIG_PATH:-$HOME/.openhoof/openhoof.json}"
            if [[ -f "${config_path}" || -f "$HOME/.hoofbot/hoofbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$hoof" ]]; then
                    ui_info "Skipping onboarding (openhoof not on PATH yet)"
                    warn_openhoof_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run openhoof onboard to finish setup"
            return 0
        fi
    fi

    if command -v openhoof &> /dev/null; then
        local hoof="${OPENHOOF_BIN:-}"
        if [[ -z "$hoof" ]]; then
            hoof="$(resolve_openhoof_bin || true)"
        fi
        if [[ -n "$hoof" ]] && is_gateway_daemon_loaded "$hoof"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (openhoof daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if OPENHOOF_UPDATE_IN_PROGRESS=1 "$hoof" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: openhoof daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${OPENHOOF_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
