...
const EXPECTED_REPOSITORY_URL = "https://github.com/openhoof/openhoof";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "openhoof") {
    errors.push(`package.json name must be "openhoof"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.openhoof !== "openhoof.mjs") {
    errors.push(
      `package.json bin.openhoof must be "openhoof.mjs"; found "${pkg.bin?.openhoof ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`openhoof-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `openhoof-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
