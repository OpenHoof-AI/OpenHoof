...
# OpenHoof Auth Widget for Termux
...
SERVER="${OPENHOOF_SERVER:-${HOOFBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking OpenHoof auth..."

STATUS=$(ssh "$SERVER" '$HOME/openhoof/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/openhoof/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPENHOOF_EXPIRING|HOOFBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/openhoof/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPENHOOF_EXPIRED|HOOFBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: OpenHoof won't work until re-auth"
                ;;
        esac
        ;;
