...
# Build and bundle OpenHoof into a minimal .app we can open.
# Outputs to dist/OpenHoof.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenHoof.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenHoof"
BUNDLE_ID="${BUNDLE_ID:-ai.openhoof.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/openhoof/openhoof/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenHoof/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenHoofBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenHoofGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenHoof"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenHoof"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenHoof"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenHoof" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenHoof/Resources/OpenHoof.icns" "$APP_ROOT/Contents/Resources/OpenHoof.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenHoof/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenHoofKit resources"
OPENHOOFKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenHoofKit_OpenHoofKit.bundle"
if [ -d "$OPENHOOFKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenHoofKit_OpenHoofKit.bundle"
  cp -R "$OPENHOOFKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenHoofKit_OpenHoofKit.bundle"
else
  echo "WARN: OpenHoofKit resource bundle not found at $OPENHOOFKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenHoof"
killall -q OpenHoof 2>/dev/null || true
