...
@Suite struct WakeWordGateTests {
    @Test func matchRequiresGapAfterTrigger() {
        let transcript = "hey hoofd do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["hoofd"], minPostTriggerGap: 0.3)
    ...
    @Test func matchHandlesMultiWordTriggers() {
        let transcript = "hey hoofd do it"
        let segments = makeSegments(
            transcript: transcript,
            words: [
                ("hey", 0.0, 0.1),
        ...
        let config = WakeWordGateConfig(triggers: ["hey hoofd"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
    ...
    @Test func commandTextHandlesForeignRangeIndices() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "hoofd", start: 0.2, duration: 0.1, range: transcript.range(of: "hoofd")),
