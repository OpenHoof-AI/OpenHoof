...
const liveEnabled = HAS_OPENAI_KEY && process.env.OPENHOOF_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "openhoof-memory-test-" });
...
describeLive("memory plugin live tests", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "openhoof-memory-live-" });
