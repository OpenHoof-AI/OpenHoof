...
export const signalDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: OpenHoofConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "OpenHoof"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("openhoof gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
