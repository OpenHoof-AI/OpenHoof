...
export const imessageDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: OpenHoofConfig) => cfg.channels?.imessage?.dmPolicy ?? "pairing",
  setPolicy: (cfg: OpenHoofConfig, policy) =>
    setChannelDmPolicyWithAllowFrom({
      cfg,
      channel,
      dmPolicy: policy,
...
function resolveIMessageCliPath(params: { cfg: OpenHoofConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    ...
    "Ensure OpenHoof has Full Disk Access to Messages DB.",
