import type { AnyAgentTool, OpenHoofPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: OpenHoofPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<OpenHoofPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
