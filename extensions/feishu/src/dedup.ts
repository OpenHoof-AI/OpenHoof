...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.OPENHOOF_STATE_DIR?.trim() || env.HOOFBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["openhoof-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".openhoof");
}
