...
import type { ChannelSetupDmPolicy } from "openhoof/plugin-sdk/setup";
import type { ChannelSetupWizard } from "openhoof/plugin-sdk/setup";
import { formatDocsLink } from "openhoof/plugin-sdk/setup";
import type { WizardPrompter } from "openhoof/plugin-sdk/setup";
...
export const ircSetupWizard: ChannelSetupWizard = {
  ...
  textInputs: [
    ...
    },
    {
      ...
      initialValue: ({ cfg, accountId }) =>
        resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.realname || "OpenHoof",
      validate: ({ value }) => (String(value ?? "").trim() ? undefined : "Required"),
      normalizeValue: ({ value }) => String(value).trim(),
      applySet: async ({ cfg, accountId, value }) =>
        updateIrcAccountConfig(cfg as CoreConfig, accountId, {
  ...
  completionNote: {
    ...
    lines: [
      ...
      "Command: openhoof channels status --probe",
      `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
    ],
  },
