...
import type { OpenHoofPluginApi, OpenHoofPluginToolContext } from "../runtime-api.js";
import {
...
let createMustangTool: typeof import("./mustang-tool.js").createMustangTool;

function fakeApi(overrides: Partial<OpenHoofPluginApi> = {}): OpenHoofPluginApi {
  return {
    id: "mustang",
    name: "mustang",
    source: "test",
...
function fakeCtx(overrides: Partial<OpenHoofPluginToolContext> = {}): OpenHoofPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("mustang plugin tool", () => {
  let tempDir = "";
  const originalProcessState = snapshotPlatformPathEnv();

  beforeAll(async () => {
    ({ createMustangTool } = await import("./mustang-tool.js"));

    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-mustang-plugin-"));
  });
  ...
  it("runs mustang and returns parsed envelope in details", async () => {
    spawnState.queue.push({
      stdout: JSON.stringify({
        ok: true,
        status: "ok",
    ...
    const tool = createMustangTool(fakeApi());
  ...
  it("rejects invalid JSON from mustang", async () => {
  ...
  it("runs Windows cmd shims through Node without enabling shell", async () => {
    ...
    const shimScriptPath = path.join(tempDir, "shim-dist", "mustang-cli.cjs");
    const shimPath = path.join(tempDir, "shim-bin", "mustang.cmd");
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpenHoofPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
      return createMustangTool(api);
    };

    expect(factoryTool(fakeCtx({ sandboxed: true }))).toBeNull();
    expect(factoryTool(fakeCtx({ sandboxed: false }))?.name).toBe("mustang");
  });
});
