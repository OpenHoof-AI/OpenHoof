...
function ensureZalouserPluginEnabled(cfg: OpenHoofConfig): OpenHoofConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("openhoof directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
  ...
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ...
        : resolveDefaultZalouserAccountId(cfg as OpenHoofConfig);
    return await promptZalouserAllowFrom({
      cfg: cfg as OpenHoofConfig,
      prompter,
      accountId: id,
    });
  },
...
export const zalouserSetupWizard: ChannelSetupWizard = {
  ...
  groupAccess: {
    ...
    setPolicy: ({ cfg, accountId, policy }) =>
      setZalouserGroupPolicy(cfg as OpenHoofConfig, accountId, policy),
    resolveAllowlist: async ({ cfg, accountId, entries, prompter }) => {
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("openhoof directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
      ...
      const updatedAccount = resolveZalouserAccountSync({ cfg: cfg as OpenHoofConfig, accountId });
      try {
    ...
    applyAllowlist: ({ cfg, accountId, resolved }) =>
      setZalouserGroupAllowlist(cfg as OpenHoofConfig, accountId, resolved as string[]),
  },
