...
import type { OpenHoofConfig } from "openhoof/plugin-sdk/twitch";
...
describe("token", () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    delete process.env.OPENHOOF_TWITCH_ACCESS_TOKEN;
  });
  ...
  describe("resolveTwitchToken", () => {
    ...
    it("should prioritize config token over env var (simplified config)", () => {
      process.env.OPENHOOF_TWITCH_ACCESS_TOKEN = "oauth:env-token";
