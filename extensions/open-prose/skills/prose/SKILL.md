...
metadata: { "openhoof": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenHoof `sessions_spawn`
- **File I/O** == OpenHoof `read`/`write`
- **Remote fetch** == OpenHoof `web_fetch` (or `exec` with curl when POST is required)
