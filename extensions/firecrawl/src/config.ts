...
import { normalizeSecretInput } from "openhoof/plugin-sdk/provider-auth";
...
type WebSearchConfig = NonNullable<OpenHoofConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<OpenHoofConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpenHoofConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: OpenHoofConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: OpenHoofConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: OpenHoofConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: OpenHoofConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: OpenHoofConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: OpenHoofConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: OpenHoofConfig, override?: number): number {
