...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["openhoofPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["openhoof", "pollId"]) ??
    readNestedString(value, ["openhoof", "poll", "id"]) ??
    readNestedString(value, ["data", "openhoofPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "openhoof", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          openhoofPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "openhoof poll vote",
          displayText: "Vote recorded",
          value: { openhoofPollId: pollId, pollId },
        },
      },
    ],
  };
