...
function getChannelConfig(cfg: OpenHoofConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: OpenHoofConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: OpenHoofConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: OpenHoofConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL OpenHoof uses to send replies back to Chat.",
      ],
