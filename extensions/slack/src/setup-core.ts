...
import { formatDocsLink } from "openhoof/plugin-sdk/setup-tools";
...
function enableSlackAccount(cfg: OpenHoofConfig, accountId: string): OpenHoofConfig {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: OpenHoofConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: OpenHoofConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: OpenHoofConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: OpenHoofConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.allow !== false && value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: OpenHoofConfig; accountId: string }) =>
    ...
    disable: (cfg: OpenHoofConfig) => setSetupChannelEnabled(cfg, channel, false),
