...
import { createScopedChannelConfigAdapter } from "openhoof/plugin-sdk/channel-config-helpers";
import { createChannelPluginBase } from "openhoof/plugin-sdk/core";
import {
  formatDocsLink,
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OpenHoof";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OpenHoof`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "OpenHoof"): string[] {
  return [
