import type { SlackSlashCommandConfig } from "openhoof/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "openhoof");
  const name = normalizedName || "openhoof";
  return {
