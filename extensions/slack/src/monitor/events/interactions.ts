...
// Prefix for OpenHoof-generated action IDs to scope our handler
const OPENHOOF_ACTION_PREFIX = "openhoof:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${OPENHOOF_ACTION_PREFIX}`);

  // Handle OpenHoof modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
