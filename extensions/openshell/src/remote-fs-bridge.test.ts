...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("openhoof-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("openhoof-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("openhoof-openshell-remote-agent-");
