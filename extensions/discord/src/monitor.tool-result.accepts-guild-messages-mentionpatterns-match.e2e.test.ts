  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/openhoof",
      },
    },
    session: { store: "/tmp/openhoof-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bopenhoof\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "openhoof: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "OpenHoof" },
          },
        },
        eventPatch: {
