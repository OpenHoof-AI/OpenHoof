...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): OpenHoofConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("openhoof/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("openhoof/plugin-sdk/plugin-runtime")>(
    "openhoof/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
