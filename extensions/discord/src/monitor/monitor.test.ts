...
import { buildPluginBindingApprovalCustomId } from "openhoof/plugin-sdk/conversation-runtime";
...
describe("agent components", () => {
  const createCfg = (): OpenHoofConfig => ({}) as OpenHoofConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): OpenHoofConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/openhoof-sessions-test.json");
    ...
    resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
      ...
      binding: {
        ...
        pluginName: "OpenHoof App Server",
  ...
  it("does not fall through to Hoof when a plugin Discord interaction already replied", async () => {
    registerDiscordComponentEntries({
      entries: [createButtonEntry({ callbackData: "codex:approve" })],
      modals: [],
    });
  ...
  it("resolves plugin binding approvals without falling through to Hoof", async () => {
    registerDiscordComponentEntries({
      entries: [
        createButtonEntry({
