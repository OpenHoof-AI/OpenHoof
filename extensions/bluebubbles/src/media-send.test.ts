...
import type { OpenHoofConfig, PluginRuntime } from "openhoof/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): OpenHoofConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
