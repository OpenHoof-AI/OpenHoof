...
import type { PluginRuntime } from "openhoof/plugin-sdk/nostr";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENHOOF_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-nostr-"));
  process.env.OPENHOOF_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.OPENHOOF_STATE_DIR?.trim() || stateEnv.HOOFBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".openhoof");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENHOOF_STATE_DIR = previous;
    }
