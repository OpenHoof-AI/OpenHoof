...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OpenHoofConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OpenHoofConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OpenHoofConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OpenHoofConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: OpenHoofConfig, accountId = "media") {
    await handleTelegramAction(
