...
import { info, success } from "openhoof/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "openhoof/plugin-sdk/state-paths";
import type { WebChannel } from "openhoof/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "openhoof/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("openhoof channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
