...
import { generateSecureUuid } from "openhoof/plugin-sdk/infra-runtime";
...
import { getChildLogger } from "openhoof/plugin-sdk/text-runtime";
...
import { markdownToWhatsApp } from "openhoof/plugin-sdk/text-runtime";
import { toWhatsappJid } from "openhoof/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: OpenHoofConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
