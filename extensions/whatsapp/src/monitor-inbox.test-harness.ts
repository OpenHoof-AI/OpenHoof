...
import { resetLogger, setLoggerOverride } from "openhoof/plugin-sdk/runtime-env";
...
vi.mock("openhoof/plugin-sdk/conversation-runtime", () => getPairingStoreMocks());
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "openhoof-auth-"));
    } else {
      authDir = undefined;
    }
  });
