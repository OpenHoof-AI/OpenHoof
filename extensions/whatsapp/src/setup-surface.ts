...
import {
  ...
  type DmPolicy,
  type OpenHoofConfig,
} from "openhoof/plugin-sdk/setup";
import type { ChannelSetupWizard } from "openhoof/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "openhoof/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: OpenHoofConfig,
  patch: Partial<NonNullable<NonNullable<OpenHoofConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: OpenHoofConfig, dmPolicy: DmPolicy): OpenHoofConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OpenHoofConfig, allowFrom?: string[]): OpenHoofConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OpenHoofConfig, selfChatMode: boolean): OpenHoofConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: OpenHoofConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so OpenHoof can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for OpenHoof" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("openhoof channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
