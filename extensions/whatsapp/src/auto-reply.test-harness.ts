...
import * as ssrf from "openhoof/plugin-sdk/infra-runtime";
...
vi.mock("openhoof/plugin-sdk/agent-runtime", () => ({
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-session-"));
