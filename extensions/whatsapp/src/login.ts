...
import { danger, info, success } from "openhoof/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "openhoof/plugin-sdk/runtime-env";
import { logInfo } from "openhoof/plugin-sdk/text-runtime";
...
export async function loginWeb(
  ...
  try {
    ...
    if (code === DisconnectReason.loggedOut) {
      ...
      console.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("openhoof channels login")} and scan the QR again.`,
        ),
      );
