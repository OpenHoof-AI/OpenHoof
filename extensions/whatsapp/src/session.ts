...
import { VERSION } from "openhoof/plugin-sdk/cli-runtime";
import { danger, success } from "openhoof/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "openhoof/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "openhoof/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === DisconnectReason.loggedOut) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("openhoof channels login")}`,
              ),
            );
          }
        }
