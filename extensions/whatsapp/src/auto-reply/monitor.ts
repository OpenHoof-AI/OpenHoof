import { formatCliCommand } from "openhoof/plugin-sdk/cli-runtime";
import { waitForever } from "openhoof/plugin-sdk/cli-runtime";
import { loadConfig } from "openhoof/plugin-sdk/config-runtime";
import { createConnectedChannelStatusPatch } from "openhoof/plugin-sdk/gateway-runtime";
import { formatDurationPrecise } from "openhoof/plugin-sdk/infra-runtime";
...
import { resolveInboundDebounceMs } from "openhoof/plugin-sdk/reply-runtime";
import { getReplyFromConfig } from "openhoof/plugin-sdk/reply-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "openhoof/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "openhoof/plugin-sdk/routing";
import { logVerbose } from "openhoof/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "openhoof/plugin-sdk/runtime-env";
import { getChildLogger } from "openhoof/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "openhoof/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("openhoof channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("openhoof channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
