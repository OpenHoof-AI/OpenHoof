...
describe("web processMessage inbound context", () => {
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-process-message-"));
        ...
        list: [
          {
            id: "main",
            default: true,
            identity: { name: "Mainbot", emoji: "🐴", theme: "void horse" },
          },
        ],
      },
      messages: {},
  ...
  it("does not force an [openhoof] response prefix in self-chats when identity is unset", async () => {
    await processSelfDirectMessage({
      messages: {},
      session: { store: sessionStorePath },
    } as unknown as ReturnType<typeof import("../../../../../src/config/config.js").loadConfig>);
