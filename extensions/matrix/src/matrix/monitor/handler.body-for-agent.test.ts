...
import type { PluginRuntime, RuntimeEnv, RuntimeLogger } from "openhoof/plugin-sdk/matrix";
  ...
  it("stores sender-labeled BodyForAgent for group thread messages", async () => {
    ...
    const core = {
      channel: {
        ...
        session: {
          resolveStorePath: vi.fn().mockReturnValue("/tmp/openhoof-test-session.json"),
