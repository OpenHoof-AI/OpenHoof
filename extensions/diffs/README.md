...
Read-only diff viewer plugin for **OpenHoof** agents.
...
Set plugin-wide defaults in `~/.openhoof/openhoof.json`:
...
Before:
OpenHoof supports plugins.

After:
OpenHoof supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/openhoof-diffs`).
