...
function resolveGatewayPort(cfg: OpenHoofPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPENHOOF_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.HOOFBOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuthLabel(cfg: OpenHoofPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: OpenHoofPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for OpenHoof devices",
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (channel === "telegram" && target) {
            try {
              ...
              if (send) {
                await send(
                  target,
                  ["Scan this QR code with the OpenHoof iOS app:", "", "```", qrAscii, "```"].join(
                    "\n",
                  ),
                  {
          ...
          return {
            text: [
              "Scan this QR code with the OpenHoof iOS app:",
              "",
              "```",
              qrAscii,
              "```",
