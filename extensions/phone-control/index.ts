...
function normalizeDenyList(cfg: OpenHoofPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenHoofPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: OpenHoofPluginApi) {
    ...
    const timerService: OpenHoofPluginService = {
