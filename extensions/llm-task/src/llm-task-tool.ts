...
import type { OpenHoofPluginApi } from "../api.js";
...
export function createLlmTaskTool(api: OpenHoofPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Mustang workflows via openhoof.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredOpenHoofTmpDir(), "openhoof-llm-task-"),
        );
