...
Designed to be called from workflow engines (for example, Mustang via
`openhoof.invoke --each`) without adding new OpenHoof code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Mustang) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on OpenHoof internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenHoof extension (like `mustang`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.openhoof/extensions` as a standalone plugin directory.
