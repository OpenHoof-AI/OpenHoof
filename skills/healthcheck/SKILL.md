---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenHoof deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenHoof cron scheduling for periodic checks, or version status checks on a machine running OpenHoof (laptop, workstation, Pi, VPS).
---

# OpenHoof Host Hardening

## Overview

Assess and harden the host running OpenHoof, then align it to a user-defined risk tolerance without breaking access. Use OpenHoof security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenHoof changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenHoof gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenHoof can access sensitive data.
...
### 2) Run OpenHoof security audits (read-only)

As part of the default read-only checks, run `openhoof security audit --deep`. Only offer alternatives if the user requests them:

1. `openhoof security audit` (faster, non-probing)
2. `openhoof security audit --json` (structured output)

Offer to apply OpenHoof safe defaults (numbered):

1. `openhoof security audit --fix`

Be explicit that `--fix` only tightens OpenHoof defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenHoof version/update status (read-only)

As part of the default read-only checks, run `openhoof update status`.
...
- Credential hygiene notes (location of OpenHoof creds, prefer disk encryption)
...
- OpenHoof security audit (re-run)
...
After OpenHoof install or first hardening pass, run at least one baseline audit and version check:

- `openhoof security audit`
- `openhoof security audit --deep`
- `openhoof update status`

Ongoing monitoring is recommended. Use the OpenHoof cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `openhoof cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `openhoof update status`
...
Before creating, `openhoof cron list` and match on exact `name`. If found, `openhoof cron edit <id> ...`.
If not found, `openhoof cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `openhoof update status` (preferred for source checkouts and channels)
2. `npm view openhoof version` (published npm version)

## OpenHoof command accuracy

Use only supported commands and flags:

- `openhoof security audit [--deep] [--fix] [--json]`
- `openhoof status` / `openhoof status --deep`
- `openhoof health --json`
- `openhoof update status`
- `openhoof cron add|list|runs|run`

Do not invent CLI flags or imply OpenHoof enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenHoof compaction:
