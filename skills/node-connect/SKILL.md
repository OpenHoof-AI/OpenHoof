---
name: node-connect
description: Diagnose OpenHoof node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify OpenHoof is advertising that route, then fix pairing/auth.
...
- whether `openhoof devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `openhoof qr --json`. It uses the same setup-code payload Android scans.

```bash
openhoof config get gateway.mode
openhoof config get gateway.bind
openhoof config get gateway.tailscale.mode
openhoof config get gateway.remote.url
openhoof config get gateway.auth.mode
openhoof config get gateway.auth.allowTailscale
openhoof config get plugins.entries.device-pair.config.publicUrl
openhoof qr --json
openhoof devices list
openhoof nodes status
```

If this OpenHoof instance is pointed at a remote gateway, also run:

```bash
openhoof qr --remote --json
```
...
`openhoof qr --json` success means:
...
If `openhoof qr --json` says `Gateway is only bound to loopback`:
...
- `openhoof devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run openhoof qr again, rescan, then approve the pending device pairing.`
