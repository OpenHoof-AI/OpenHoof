...
const OPENHOOF_OPENSHELL_E2E = process.env.OPENHOOF_E2E_OPENSHELL === "1";
const OPENHOOF_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const OPENHOOF_OPENSHELL_COMMAND =
  process.env.OPENHOOF_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "openhoof-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && OPENHOOF_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: OPENHOOF_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(OPENHOOF_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-openshell-e2e-"));
      ...
      const gatewayName = `openhoof-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `openhoof-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        browser: {
          enabled: false,
          image: "openhoof-browser",
          containerPrefix: "openhoof-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: OPENHOOF_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("openhoof-openshell-e2e");
