...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openhoof-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openhoof");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENHOOF_SKIP_PROVIDERS: "1",
          OPENHOOF_SKIP_GMAIL_WATCHER: "1",
          OPENHOOF_SKIP_CRON: "1",
          OPENHOOF_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPENHOOF_SKIP_CANVAS_HOST: "1",
          OPENHOOF_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
