...
export function requireOpenHoofAgentDir(): string {
  const agentDir = process.env.OPENHOOF_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENHOOF_AGENT_DIR not set");
  }
  return agentDir;
}
