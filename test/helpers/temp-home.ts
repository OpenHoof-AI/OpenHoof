...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPENHOOF_HOME.
  delete process.env.OPENHOOF_HOME;
  process.env.OPENHOOF_STATE_DIR = path.join(base, ".openhoof");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "openhoof-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openhoof", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
