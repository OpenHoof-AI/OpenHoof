import type { OpenHoofConfig } from "openhoof/plugin-sdk/discord";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: OpenHoofConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): OpenHoofConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("openhoof/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("openhoof/plugin-sdk/acp-runtime")>(
    "openhoof/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("openhoof/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("openhoof/plugin-sdk/reply-runtime")>(
    "openhoof/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("openhoof/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("openhoof/plugin-sdk/config-runtime")>(
...
vi.mock("openhoof/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("openhoof/plugin-sdk/runtime-env")>(
    "openhoof/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("openhoof/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("openhoof/plugin-sdk/infra-runtime")>(
    "openhoof/plugin-sdk/infra-runtime",
  );
  return {
    ...actual,
