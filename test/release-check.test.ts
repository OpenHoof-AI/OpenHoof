        ...
        extensions: [
          {
            ...
            packageJson: {
              ...
              openhoof: {
                install: { npmSpec: "@openhoof/googlechat" },
                releaseChecks: {
          ...
          },
          {
            ...
            packageJson: {
              ...
              openhoof: { install: { npmSpec: "@openhoof/feishu" } },
            },
          },
        ],
      }),
...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | openhoof.install.npmSpec must be a non-empty string",
    ]);
  });
  ...
  it("flags invalid release-check allowlist metadata", () => {
    expect(
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            openhoof: {
              install: { npmSpec: "@openhoof/broken" },
              releaseChecks: {
      ...
      "bundled extension 'broken' manifest invalid | openhoof.releaseChecks.rootDependencyMirrorAllowlist must contain only non-empty strings",
    ]);
  });
});
  ...
  it("flags nested node_modules leaking into npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "node_modules/.bin/openhoof",
      ]),
    ).toEqual(["extensions/tlon/node_modules/.bin/tlon", "node_modules/.bin/openhoof"]);
  });
});
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("openhoof-2026.3.14.tgz", 120_354_302)]),
    ).toEqual([]);
  });

  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("openhoof-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "openhoof-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 167772160 bytes (160.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "openhoof-2026.3.14.tgz" },
        { filename: "openhoof-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
