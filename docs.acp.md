...
This document describes how the OpenHoof ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openhoof acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenHoof Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing OpenHoof Gateway
...
- ACP client spawns `openhoof acp` and speaks ACP messages over stdio.
...
`openhoof acp` resolves the Gateway URL and auth from CLI flags or config:
