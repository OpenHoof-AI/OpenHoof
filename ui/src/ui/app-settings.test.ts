...
const createHost = (tab: Tab): SettingsHost => ({
  ...
  theme: "hoof" as unknown as ThemeName & ThemeMode,
  themeMode: "system",
  themeResolved: "dark",
...
describe("applySettingsFromUrl", () => {
  ...
  it("hydrates query token params and strips them from the URL", () => {
    ...
    host.settings.gatewayUrl = "wss://control.example/openhoof";
  ...
  it("keeps query token params pending when a gatewayUrl confirmation is required", () => {
    setTestWindowUrl(
      "https://control.example/ui/overview?gatewayUrl=wss://other-gateway.example/openhoof&token=abc123",
    );
    const host = createHost("overview");
    host.settings.gatewayUrl = "wss://control.example/openhoof";
    ...
    expect(host.pendingGatewayUrl).toBe("wss://other-gateway.example/openhoof");
