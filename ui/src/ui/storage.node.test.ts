  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __OPENHOOF_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__OPENHOOF_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__OPENHOOF_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /openhoof/ ");

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/openhoof"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/openhoof/chat",
    });

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/openhoof"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("openhoof.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "openhoof.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/openhoof",
    ...
    const scopedKey = "openhoof.control.settings.v1:wss://gateway.example:8443/openhoof";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      borderRadius: 50,
      sessionsByGateway: {
        "wss://gateway.example:8443/openhoof": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", async () => {
    ...
    const scopedKey = `openhoof.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "hoof",
      themeMode: "system",
  ...
  it("caps persisted session scopes to the most recent gateways", async () => {
    ...
    const { saveSettings } = await import("./storage.ts");
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `openhoof.control.settings.v1:wss://gateway.example:8443`;
