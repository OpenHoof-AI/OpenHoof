...
export async function handleWhatsAppStart(host: OpenHoofApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OpenHoofApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpenHoofApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpenHoofApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OpenHoofApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpenHoofApp): string {
...
function resolveGatewayHttpAuthHeader(host: OpenHoofApp): string | null {
...
function buildGatewayHttpHeaders(host: OpenHoofApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OpenHoofApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpenHoofApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OpenHoofApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpenHoofApp) {
  const state = host.nostrProfileFormState;
