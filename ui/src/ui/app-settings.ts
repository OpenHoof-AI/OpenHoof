...
import type { OpenHoofApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as OpenHoofApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as OpenHoofApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as OpenHoofApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as OpenHoofApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as OpenHoofApp);
    await loadConfig(host as unknown as OpenHoofApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as OpenHoofApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as OpenHoofApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as OpenHoofApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as OpenHoofApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as OpenHoofApp);
    await loadDevices(host as unknown as OpenHoofApp);
    await loadConfig(host as unknown as OpenHoofApp);
    await loadExecApprovals(host as unknown as OpenHoofApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as OpenHoofApp);
    await loadConfig(host as unknown as OpenHoofApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as OpenHoofApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as OpenHoofApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__OPENHOOF_CONTROL_UI_BASE_PATH__;
...
export function syncThemeWithSettings(host: SettingsHost) {
  host.theme = host.settings.theme ?? "hoof";
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as OpenHoofApp;
...
async function loadOverviewLogs(host: OpenHoofApp) {
...
function buildAttentionItems(host: OpenHoofApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.openhoof.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OpenHoofApp, true),
    loadConfigSchema(host as unknown as OpenHoofApp),
    loadConfig(host as unknown as OpenHoofApp),
  ]);
}
