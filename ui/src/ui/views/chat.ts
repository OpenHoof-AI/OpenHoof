    ...
    <div class="agent-chat__welcome" style="--agent-color: var(--accent)">
      ...
      ${
        avatar
          ...
          : html`<div class="agent-chat__avatar agent-chat__avatar--logo"><img src=${logoUrl} alt="OpenHoof" /></div>`
      }
      <h2>${name}</h2>
...
function buildChatItems(props: ChatProps): Array<ChatItem | MessageGroup> {
  ...
  for (let i = historyStart; i < history.length; i++) {
    ...
    const marker = raw.__openhoof as Record<string, unknown> | undefined;
