...
import { OpenHoofApp } from "./app.ts";
import {
...
function resetChatStateForSessionSwitch(state: AppViewState, sessionKey: string) {
  ...
  (state as unknown as OpenHoofApp).chatStreamStartedAt = null;
  state.chatRunId = null;
  (state as unknown as OpenHoofApp).resetToolStream();
  (state as unknown as OpenHoofApp).resetChatScroll();
  state.applySettings({
    ...state.settings,
    sessionKey,
  ...
  return html`
    <div class="chat-controls">
      <button
        ...
        @click=${async () => {
          const app = state as unknown as OpenHoofApp;
