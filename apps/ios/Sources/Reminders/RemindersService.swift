...
final class RemindersService: RemindersServicing {
    func list(params: OpenHoofRemindersListParams) async throws -> OpenHoofRemindersListPayload {
        ...
        let payload: [OpenHoofReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OpenHoofReminderPayload(
        ...
        return OpenHoofRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenHoofRemindersAddParams) async throws -> OpenHoofRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenHoofReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenHoofRemindersAddPayload(reminder: payload)
    }
