...
struct IOSGatewayChatTransport: OpenHoofChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.openhoof", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> OpenHoofChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenHoofChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenHoofChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenHoofChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenHoofChatAttachmentPayload]) async throws -> OpenHoofChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenHoofChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OpenHoofChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OpenHoofGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenHoofGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenHoofChatEventPayload.self)
                        {
