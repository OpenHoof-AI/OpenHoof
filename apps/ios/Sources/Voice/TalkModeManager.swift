...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__OPENHOOF_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<OpenHoofTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.openhoof", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OpenHoofTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OpenHoofTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return OpenHoofTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OpenHoofTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OpenHoofTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return OpenHoofTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> OpenHoofTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OpenHoofTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OpenHoofTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: OpenHoofAgentEventPayload.self
            ) else {
                continue
            }
