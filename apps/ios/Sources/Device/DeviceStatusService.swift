...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> OpenHoofDeviceStatusPayload {
        ...
        return OpenHoofDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> OpenHoofDeviceInfoPayload {
        ...
        return OpenHoofDeviceInfoPayload(
    ...
    private func batteryStatus() -> OpenHoofBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: OpenHoofBatteryState = switch device.batteryState {
        ...
        return OpenHoofBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> OpenHoofThermalStatusPayload {
        let state: OpenHoofThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return OpenHoofThermalStatusPayload(state: state)
    }

    private func storageStatus() -> OpenHoofStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return OpenHoofStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
