...
final class MotionService: MotionServicing {
    func activities(params: OpenHoofMotionActivityParams) async throws -> OpenHoofMotionActivityPayload {
        ...
        let mapped: [OpenHoofMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return OpenHoofMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpenHoofPedometerParams) async throws -> OpenHoofPedometerPayload {
        ...
        let payload: OpenHoofPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpenHoofPedometerPayload(
