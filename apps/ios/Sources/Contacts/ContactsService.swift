...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpenHoofContactsSearchParams) async throws -> OpenHoofContactsSearchPayload {
        ...
        return OpenHoofContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpenHoofContactsAddParams) async throws -> OpenHoofContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpenHoofContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpenHoofContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpenHoofContactPayload {
        OpenHoofContactPayload(
