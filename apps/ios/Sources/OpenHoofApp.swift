...
@MainActor
final class OpenHoofAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.openhoof.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.openhoof.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.openhoof.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "openhoof.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "openhoof.watch.promptId"
    static let sessionKeyKey = "openhoof.watch.sessionKey"
    static let actionPrimaryIDKey = "openhoof.watch.action.primary.id"
    static let actionPrimaryLabelKey = "openhoof.watch.action.primary.label"
    static let actionSecondaryIDKey = "openhoof.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "openhoof.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "openhoof.watch.action.primary"
    static let actionSecondaryIdentifier = "openhoof.watch.action.secondary"
    static let actionIdentifierPrefix = "openhoof.watch.action."
    static let actionIDKeyPrefix = "openhoof.watch.action.id."
    static let actionLabelKeyPrefix = "openhoof.watch.action.label."
    static let categoryPrefix = "openhoof.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: OpenHoofWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> OpenHoofWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OpenHoofWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "OpenHoof" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [OpenHoofWatchAction]) -> [UNNotificationAction] {
...
@main
struct OpenHoofApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(OpenHoofAppDelegate.self) private var appDelegate
...
extension OpenHoofApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("OpenHoof: installing uncaught exception handler")
