...
import Foundation
import Testing
@testable import OpenHoofChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> OpenHoofChatHistoryPayload
{
    OpenHoofChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> OpenHoofChatSessionEntry {
    OpenHoofChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> OpenHoofChatSessionEntry
{
    OpenHoofChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> OpenHoofChatModelChoice {
    OpenHoofChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [OpenHoofChatHistoryPayload],
    sessionsResponses: [OpenHoofChatSessionsListResponse] = [],
    modelResponses: [[OpenHoofChatModelChoice]] = [],
    ...
    -> (TestChatTransport, OpenHoofChatViewModel)
{
...
private func sendUserMessage(_ vm: OpenHoofChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, OpenHoofChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenHoofChatHistoryPayload]
    private let sessionsResponses: [OpenHoofChatSessionsListResponse]
    private let modelResponses: [[OpenHoofChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<OpenHoofChatTransportEvent>
    private let continuation: AsyncStream<OpenHoofChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<OpenHoofChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenHoofChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenHoofChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [OpenHoofChatAttachmentPayload]) async throws -> OpenHoofChatSendResponse
    {
        ...
        return OpenHoofChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenHoofChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenHoofChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpenHoofChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = OpenHoofChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = OpenHoofChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: OpenHoofChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = OpenHoofChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: OpenHoofChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(OpenHoofChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == OpenHoofChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = OpenHoofChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: OpenHoofChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = OpenHoofChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = OpenHoofChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = OpenHoofChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { OpenHoofChatViewModel(sessionKey: "main", transport: transport) }
