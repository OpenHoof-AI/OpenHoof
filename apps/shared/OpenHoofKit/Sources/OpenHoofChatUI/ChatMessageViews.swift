...
struct ChatMessageBubble: View {
    let message: OpenHoofChatMessage
    let style: OpenHoofChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OpenHoofChatTheme.userText : OpenHoofChatTheme.assistantText
    ...
    private var inlineAttachments: [OpenHoofChatMessageContent] {
    ...
    private var toolCalls: [OpenHoofChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OpenHoofChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OpenHoofChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OpenHoofChatTheme.onboardingAssistantBubble
        }
        return OpenHoofChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OpenHoofChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OpenHoofChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OpenHoofChatTheme.userText : OpenHoofChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OpenHoofChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(OpenHoofChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OpenHoofChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OpenHoofChatTheme.assistantText)
            }
        }
    }
}
