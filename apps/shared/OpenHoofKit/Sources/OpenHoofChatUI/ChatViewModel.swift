...
private let chatUILogger = Logger(subsystem: "ai.openhoof", category: "OpenHoofChatUI")

@MainActor
@Observable
public final class OpenHoofChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [OpenHoofChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [OpenHoofChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [OpenHoofPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpenHoofChatPendingToolCall] = []
    public private(set) var sessions: [OpenHoofChatSessionEntry] = []
    private let transport: any OpenHoofChatTransport
    private var sessionDefaults: OpenHoofChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: OpenHoofChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any OpenHoofChatTransport,
    ...
    public var sessionChoices: [OpenHoofChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [OpenHoofChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpenHoofPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenHoofChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpenHoofChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OpenHoofChatMessage) -> OpenHoofChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OpenHoofChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OpenHoofChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: OpenHoofChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [OpenHoofChatMessage],
        incoming: [OpenHoofChatMessage]) -> [OpenHoofChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OpenHoofChatMessage]) -> [OpenHoofChatMessage] {
        var result: [OpenHoofChatMessage] = []
    ...
    private static func dedupeKey(for message: OpenHoofChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpenHoofChatMessageContent] = [
            OpenHoofChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OpenHoofChatAttachmentPayload in
            OpenHoofChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpenHoofChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = OpenHoofChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: OpenHoofChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpenHoofChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpenHoofAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpenHoofChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpenHoofPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
