...
@MainActor
public struct OpenHoofChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: OpenHoofChatViewModel
    ...
    public init(
        viewModel: OpenHoofChatViewModel,
    ...
    private var visibleMessages: [OpenHoofChatMessage] {
        let base: [OpenHoofChatMessage]
    ...
    private func mergeToolResults(in messages: [OpenHoofChatMessage]) -> [OpenHoofChatMessage] {
        ...
        for message in messages {
            ...
            let merged = OpenHoofChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: OpenHoofChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: OpenHoofChatMessage) -> Bool {
    ...
    private func primaryText(in message: OpenHoofChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: OpenHoofChatMessage) -> Bool {
    ...
    private func toolCalls(in message: OpenHoofChatMessage) -> [OpenHoofChatMessageContent] {
    ...
    private func inlineToolResults(in message: OpenHoofChatMessage) -> [OpenHoofChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: OpenHoofChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: OpenHoofChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(OpenHoofChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
