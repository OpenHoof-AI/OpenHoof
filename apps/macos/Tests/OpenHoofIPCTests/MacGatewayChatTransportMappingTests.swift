...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: OpenHoofProtocol.AnyCodable(["ok": OpenHoofProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: OpenHoofProtocol.AnyCodable(["ok": OpenHoofProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = OpenHoofProtocol.AnyCodable([
            "runId": OpenHoofProtocol.AnyCodable("run-1"),
            "sessionKey": OpenHoofProtocol.AnyCodable("main"),
            "state": OpenHoofProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: OpenHoofProtocol.AnyCodable(["a": OpenHoofProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
