...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func `gateway entrypoint prefers dist over bin`() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/openhoof.js")
    ...
    @Test func `log locator picks newest log file`() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/openhoof", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("openhoof-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("openhoof-new-\(UUID().uuidString).log")
