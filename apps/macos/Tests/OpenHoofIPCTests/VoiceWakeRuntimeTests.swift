...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["openhoof"]
        let text = "hey openhoof"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey openhoof do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["openhoof"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "openhoof", start: 0.2, duration: 0.1, range: transcript.range(of: "openhoof")),
