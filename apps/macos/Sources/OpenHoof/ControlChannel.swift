...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenHoofProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.openhoof", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenHoofKit.AnyCodable]()) {
                $0[$1.key] = OpenHoofKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenHoofProtocol.AnyCodable?) -> [String: OpenHoofProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenHoofProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenHoofKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenHoofProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("openhoof.control.heartbeat")
    static let controlAgentEvent = Notification.Name("openhoof.control.agent")
}
