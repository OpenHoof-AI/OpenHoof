...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.openhoof", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OpenHoofPeekabooBridgeServices?

    private static let legacySocketDirectoryNames = ["hoofbot", "hoofdis", "moltbot"]

    private static var openhoofSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        return Self.makeSocketPath(for: "OpenHoof", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = OpenHoofPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.openhoofSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.openhoofSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.openhoofSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.openhoofSocketPath)
        } catch {
...
@MainActor
private final class OpenHoofPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.openhoof.peekaboo")
