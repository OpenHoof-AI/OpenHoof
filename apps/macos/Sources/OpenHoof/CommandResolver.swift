...
enum CommandResolver {
    private static let projectRootDefaultsKey = "openhoof.gatewayProjectRootPath"
    private static let helperName = "openhoof"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openhoofEntry = root.appendingPathComponent("openhoof.mjs").path
        if FileManager().isReadableFile(atPath: openhoofEntry) { return openhoofEntry }
        let binEntry = root.appendingPathComponent("bin/openhoof.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/openhoof")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let openhoofPaths = self.openhoofManagedPaths(home: home)
        if !openhoofPaths.isEmpty {
            extras.insert(contentsOf: openhoofPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + openhoofPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func openhoofManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".openhoof"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func openhoofExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOpenHoofExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("openhoof.mjs").path,
            root.appendingPathComponent("bin/openhoof.js").path,
        ]
    ...
    static func hasAnyOpenHoofInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.openhoofExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func openhoofNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let openhoofPath = self.projectOpenHoofExecutable(projectRoot: root) {
            return [openhoofPath, subcommand] + extraArgs
        }
        if let openhoofPath = self.openhoofExecutable(searchPaths: searchPaths) {
            return [openhoofPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "openhoof", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            openhoof entrypoint missing (looked for dist/index.js or openhoof.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real openhoof CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/openhoof"
        ...
        \(cliSection)
        \(projectSection)
        if command -v openhoof >/dev/null 2>&1; then
          CLI="$(command -v openhoof)"
          openhoof \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/openhoof.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/openhoof.mjs"
            node "$PRJ/openhoof.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/openhoof.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/openhoof.js"
            node "$PRJ/bin/openhoof.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent openhoof"
          pnpm --silent openhoof \(quotedArgs);
        else
          echo "openhoof CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenHoofConfigFile.loadDict()
