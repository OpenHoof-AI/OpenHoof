...
enum DebugActions {
    private static let verboseDefaultsKey = "openhoof.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "openhoof.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenHoofPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenHoof", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenHoofPaths.configURL
        guard
