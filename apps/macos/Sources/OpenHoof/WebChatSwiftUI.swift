...
private let webChatSwiftLogger = Logger(subsystem: "ai.openhoof", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "openhoof.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: OpenHoofChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> OpenHoofChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(OpenHoofChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            OpenHoofChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return OpenHoofChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpenHoofChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OpenHoofGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    OpenHoofAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: OpenHoofProtocol.ModelChoice) -> OpenHoofChatModelChoice {
        OpenHoofChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpenHoofChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpenHoofChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpenHoofChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: OpenHoofChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpenHoofChatView>) -> NSViewController
    {
