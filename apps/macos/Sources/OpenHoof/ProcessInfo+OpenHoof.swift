...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.openhoof.mac`) even if the shipped
    ...
    {
        if environment["OPENHOOF_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "openhoof.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "openhoof.nixMode") { return true }
