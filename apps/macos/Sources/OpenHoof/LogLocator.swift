...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENHOOF_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/openhoof")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("openhoof-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("openhoof-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/openhoof/ (rolling or stdout), or nil if none exist.
