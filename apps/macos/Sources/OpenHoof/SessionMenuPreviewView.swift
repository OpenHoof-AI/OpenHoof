...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.openhoof", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenHoofSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenHoofSessionPreviewEntry,
    ...
    private static func cache(payload: OpenHoofSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenHoofChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenHoofKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenHoofKit.AnyCodable]) -> [OpenHoofChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenHoofChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenHoofChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenHoofChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenHoofChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenHoofChatMessage) -> String? {
