...
private let deepLinkLogger = Logger(subsystem: "ai.openhoof", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "OpenHoof is paused", message: "Unpause OpenHoof to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run OpenHoof agent?", message: body) else { return }
        }
