...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.openhoof/openhoof.json. Prefer OPENHOOF_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.openhoof/openhoof.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.openhoof/openhoof.json. Restart the gateway to apply."
        }
