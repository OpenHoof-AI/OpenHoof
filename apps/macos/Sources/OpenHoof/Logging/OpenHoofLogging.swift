...
enum OpenHoofLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenHoofOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenHoofFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenHoofLogging.bootstrapIfNeeded()
        let label = OpenHoofLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenHoofOSLogHandler: AppLogLevelBackedHandler {
...
struct OpenHoofFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenHoofLogging.parseLabel(self.label)
