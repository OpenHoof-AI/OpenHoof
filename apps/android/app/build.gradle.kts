...
val androidStoreFile = providers.gradleProperty("OPENHOOF_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("OPENHOOF_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("OPENHOOF_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("OPENHOOF_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set OPENHOOF_ANDROID_STORE_FILE, " +
            "OPENHOOF_ANDROID_STORE_PASSWORD, OPENHOOF_ANDROID_KEY_ALIAS, and " +
            "OPENHOOF_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/OpenHoofKit/Sources/OpenHoofKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.openhoof.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "openhoof-$versionName-$buildType.apk"
