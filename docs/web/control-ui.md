...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/openhoof`)
...
If the page fails to load, start the Gateway first: `openhoof gateway`.
...
you revoke it with `openhoof devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.openhoof/openhoof.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop openhoof`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenHoof
...
OpenHoof **blocks** Control UI connections without device identity.
