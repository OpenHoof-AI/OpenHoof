...
```bash
openhoof tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openhoof tui --deliver`
...
- Local shell commands receive `OPENHOOF_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENHOOF_THEME=light` before launching `openhoof tui`.
- To force the original dark palette instead, set `OPENHOOF_THEME=dark`.
...
- Check the Gateway logs: `openhoof logs --follow`.
- Confirm the agent can run: `openhoof status` and `openhoof models status`.
...
- No agents in picker: check `openhoof agents list` and your routing config.
