...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openhoof sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openhoof doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openhoof-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENHOOF_STATE_DIR:-$HOME/.openhoof}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
