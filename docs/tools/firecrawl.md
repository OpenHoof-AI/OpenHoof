...
OpenHoof can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `openhoof configure --section web` enables the bundled Firecrawl plugin automatically.
...
OpenHoof always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
