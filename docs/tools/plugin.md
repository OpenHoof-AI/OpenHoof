---
summary: "OpenHoof plugins/extensions: discovery, config, and safety"
read_when:
...
- a native **OpenHoof plugin** (`openhoof.plugin.json` + runtime module), or
- a compatible **bundle** (`.codex-plugin/plugin.json` or `.claude-plugin/plugin.json`)

Both show up under `openhoof plugins`, but only native OpenHoof plugins execute
runtime code in-process.

Most of the time, you’ll use plugins when you want a feature that’s not built
into core OpenHoof yet (or you want to keep optional features out of your main
install).
...
```bash
openhoof plugins list
```

2. Install an official plugin (example: Voice Call):

```bash
openhoof plugins install @openhoof/voice-call
```
...
```bash
openhoof plugins install ./my-bundle
openhoof plugins install ./my-bundle.tgz
```
...
```bash
openhoof plugins marketplace list <marketplace-name>
openhoof plugins install <plugin-name>@<marketplace-name>
```

OpenHoof resolves known Claude marketplace names from
...
Capabilities are the public **native plugin** model inside OpenHoof. Every
native OpenHoof plugin registers against one or more capability types:
...
OpenHoof classifies every loaded plugin into a shape based on its actual
...
Use `openhoof plugins inspect <id>` to see a plugin's shape and capability
...
When you run `openhoof doctor` or `openhoof plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `openhoof status --all` and `openhoof plugins doctor`.

## Architecture

OpenHoof's plugin system has four layers:

1. **Manifest + discovery**
   OpenHoof finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `openhoof.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native OpenHoof plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of OpenHoof reads the registry to expose tools, channels, provider
...
That split lets OpenHoof validate config, explain missing/disabled plugins, and
...
normal chat actions. OpenHoof keeps one shared `message` tool in core, and
...
OpenHoof treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's OpenHoof-facing
  surfaces
...
So if OpenHoof adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If OpenHoof has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { OpenHoofPluginDefinition } from "openhoof/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "openhoof/plugin-sdk";

const plugin: OpenHoofPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
OpenHoof already treats image/audio/video understanding as one shared
...
If OpenHoof adds a new domain later, such as video generation, use the same
...
OpenHoof also recognizes two compatible external bundle layouts:
...
native OpenHoof plugin sources. OpenHoof resolves the marketplace entry first,
...
- supported now: Cursor `.cursor/commands/*.md` roots, mapped into the normal
  OpenHoof skill loader
- supported now: Codex bundle hook directories that use the OpenHoof hook-pack
...
Bundle hook support is limited to the normal OpenHoof hook directory format
...
Native OpenHoof plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the OpenHoof process

Compatible bundles are safer by default because OpenHoof currently treats them
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@openhoof/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@openhoof/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@openhoof/zalouser`
- [Matrix](/channels/matrix) — `@openhoof/matrix`
- [Nostr](/channels/nostr) — `@openhoof/nostr`
- [Zalo](/channels/zalo) — `@openhoof/zalo`
- [Microsoft Teams](/channels/msteams) — `@openhoof/msteams`
...
Native OpenHoof plugins are **TypeScript modules** loaded at runtime via jiti.
...
Native OpenHoof plugins can register capabilities and surfaces:
...
Native OpenHoof plugins run in-process with the Gateway (see
...
against explicit surfaces that OpenHoof understands, validates, and can expose
...
`OpenHoofPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   OpenHoof can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that OpenHoof knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `OpenHoofPluginApi` or
  `api.runtime`
...
OpenHoof exports capabilities, not implementation convenience.
...
Use `openhoof plugins inspect <id>` for deep plugin introspection. This is the
canonical command for understanding a plugin's shape and registration behavior.

```bash
openhoof plugins inspect openai
openhoof plugins inspect openai --json
```
...
OpenHoof still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, OpenHoof calls hooks in this rough order.
...
| —   | _(built-in model lookup)_     | OpenHoof tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on OpenHoof's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before OpenHoof's static catalog updates.
...
At startup, OpenHoof does roughly this:
...
The manifest is the control-plane source of truth. OpenHoof uses it to:
...
OpenHoof keeps short in-process caches for:
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as OpenHoof adds those
...
- If OpenHoof adds a new capability such as video generation later, define the
...
- OpenHoof only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "OpenHoof plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `openhoof/plugin-sdk` import when
authoring plugins:

- `openhoof/plugin-sdk/core` for the smallest generic plugin-facing contract.
...
- Domain subpaths such as `openhoof/plugin-sdk/channel-config-helpers`,
  `openhoof/plugin-sdk/channel-config-schema`,
  `openhoof/plugin-sdk/channel-policy`,
  `openhoof/plugin-sdk/channel-runtime`,
  `openhoof/plugin-sdk/config-runtime`,
  `openhoof/plugin-sdk/agent-runtime`,
  `openhoof/plugin-sdk/lazy-runtime`,
  `openhoof/plugin-sdk/reply-history`,
  `openhoof/plugin-sdk/routing`,
  `openhoof/plugin-sdk/runtime-store`, and
  `openhoof/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- Narrow channel-core subpaths such as `openhoof/plugin-sdk/discord-core`,
  `openhoof/plugin-sdk/telegram-core`, `openhoof/plugin-sdk/whatsapp-core`,
  and `openhoof/plugin-sdk/line-core` for channel-specific primitives that
  should stay smaller than the full channel helper barrels.
- `openhoof/plugin-sdk/compat` remains as a legacy migration surface for older
...
- Bundled extension internals remain private. External plugins should use only
  `openhoof/plugin-sdk/*` subpaths. OpenHoof core/test code may use the repo
...
- `openhoof/plugin-sdk/telegram` for Telegram channel plugin types and shared channel-facing helpers. Built-in Telegram implementation internals stay private to the bundled extension.
- `openhoof/plugin-sdk/discord` for Discord channel plugin types and shared channel-facing helpers. Built-in Discord implementation internals stay private to the bundled extension.
- `openhoof/plugin-sdk/slack` for Slack channel plugin types and shared channel-facing helpers. Built-in Slack implementation internals stay private to the bundled extension.
- `openhoof/plugin-sdk/signal` for Signal channel plugin types and shared channel-facing helpers. Built-in Signal implementation internals stay private to the bundled extension.
- `openhoof/plugin-sdk/imessage` for iMessage channel plugin types and shared channel-facing helpers. Built-in iMessage implementation internals stay private to the bundled extension.
- `openhoof/plugin-sdk/whatsapp` for WhatsApp channel plugin types and shared channel-facing helpers. Built-in WhatsApp implementation internals stay private to the bundled extension.
- `openhoof/plugin-sdk/line` for LINE channel plugins.
- `openhoof/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Additional bundled extension-specific subpaths remain available where OpenHoof
  intentionally exposes extension-facing helpers:
  `openhoof/plugin-sdk/acpx`, `openhoof/plugin-sdk/bluebubbles`,
  `openhoof/plugin-sdk/feishu`, `openhoof/plugin-sdk/googlechat`,
  `openhoof/plugin-sdk/irc`, `openhoof/plugin-sdk/mustang`,
  `openhoof/plugin-sdk/matrix`,
  `openhoof/plugin-sdk/mattermost`, `openhoof/plugin-sdk/memory-core`,
  `openhoof/plugin-sdk/minimax-portal-auth`,
  `openhoof/plugin-sdk/nextcloud-talk`, `openhoof/plugin-sdk/nostr`,
  `openhoof/plugin-sdk/synology-chat`, `openhoof/plugin-sdk/test-utils`,
  `openhoof/plugin-sdk/tlon`, `openhoof/plugin-sdk/twitch`,
  `openhoof/plugin-sdk/voice-call`,
  `openhoof/plugin-sdk/zalo`, and `openhoof/plugin-sdk/zalouser`.
...
`openhoof/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape OpenHoof writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to OpenHoof's
...
- if both `catalog` and `discovery` are registered, OpenHoof uses `catalog`

Compatibility note:

- `openhoof/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `openhoof status`, `openhoof status --all`,
  `openhoof channels status`, `openhoof channels resolve`, and doctor/config
...
- Set `OPENHOOF_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `OPENHOOF_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `OPENHOOF_PLUGIN_DISCOVERY_CACHE_MS` and
  `OPENHOOF_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

OpenHoof scans, in order:
...
- `<workspace>/.openhoof/extensions/*.ts`
- `<workspace>/.openhoof/extensions/*/index.ts`

3. Global extensions

- `~/.openhoof/extensions/*.ts`
- `~/.openhoof/extensions/*/index.ts`

4. Bundled extensions (shipped with OpenHoof; mixed default-on/default-off)
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, OpenHoof logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. OpenHoof blocks candidates when:
...
Each native OpenHoof plugin must include a `openhoof.plugin.json` file in its
...
A plugin directory may include a `package.json` with `openhoof.extensions`:
...
Security guardrail: every `openhoof.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `openhoof plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `openhoof.setupEntry` can point at a lightweight setup-only module.
When OpenHoof needs setup surfaces for a disabled channel plugin, or
...
Optional: `openhoof.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let OpenHoof load the
...
Channel plugins can advertise setup/discovery metadata via `openhoof.channel` and
install hints via `openhoof.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@openhoof/nextcloud-talk",
  "openhoof": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@openhoof/nextcloud-talk",
...
OpenHoof can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.openhoof/mpm/plugins.json`
- `~/.openhoof/mpm/catalog.json`
- `~/.openhoof/plugins/catalog.json`

Or point `OPENHOOF_PLUGIN_CATALOG_PATHS` (or `OPENHOOF_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "openhoof": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, OpenHoof uses it but warns when it doesn’t match the
configured id.
...
- Native plugin config is validated using the JSON Schema embedded in
  `openhoof.plugin.json` (`configSchema`).
- Compatible bundles currently do not expose native OpenHoof config schemas.
...
OpenHoof preserves config for disabled plugins so toggling them back on is not
destructive.
...
```json5
{
  plugins: {
    slots: {
      ...
      contextEngine: "legacy", // or a plugin id such as "lossless-hoof"
    },
  },
}
```
...
OpenHoof augments `uiHints` at runtime based on discovered plugins:
...
```bash
openhoof plugins list
openhoof plugins inspect <id>
openhoof plugins install <path>                 # copy a local file/dir into ~/.openhoof/extensions/<id>
openhoof plugins install ./extensions/voice-call # relative path ok
openhoof plugins install ./plugin.tgz           # install from a local tarball
openhoof plugins install ./plugin.zip           # install from a local zip
openhoof plugins install -l ./extensions/voice-call # link (no copy) for dev
openhoof plugins install @openhoof/voice-call   # install from npm
openhoof plugins install @openhoof/voice-call --pin # store exact resolved name@version
openhoof plugins update <id>
openhoof plugins update --all
openhoof plugins enable <id>
openhoof plugins disable <id>
openhoof plugins doctor
```

See [`openhoof plugins` CLI reference](/cli/plugins) for full details on each
...
2. add typed plugin registration/runtime surfaces
   Extend `OpenHoofPluginApi` and/or `api.runtime` with the smallest useful
...
This is how OpenHoof stays opinionated without becoming hardcoded to one
...
export default function (api) {
  api.registerContextEngine("lossless-hoof", () => ({
    info: { id: "lossless-hoof", name: "Lossless Hoof", ownsCompaction: true },
    async ingest() {
...
```ts
import { delegateCompactionToRuntime } from "openhoof/plugin-sdk/core";
...
- Plugin-managed hooks show up in `openhoof hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `openhoof hooks`; enable/disable the plugin instead.
...
- When disabled, OpenHoof blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
setup inside OpenHoof, surface provider setup in onboarding/model-pickers, and
...
- `configPatch`: optional `openhoof.json` changes
...
explicit method or let OpenHoof synthesize per-method choices.

OpenHoof validates provider wizard metadata when the plugin registers:
...
- `openhoof models auth login --provider <id> [--method <id>]`
- `openhoof onboard`
...
- `run` receives a `ProviderAuthContext` with `prompter`, `runtime`,
  ...
  capture, while `openhoof models auth` keeps a tighter prompt surface.
...
- `wizard.setup` adds a provider choice to onboarding surfaces such as
  `openhoof onboard` / `openhoof setup --wizard`.
...
- `deprecatedProfileIds` lets the provider own `openhoof doctor` cleanup for
...
- `config`: The current OpenHoof config
...
- Main package: `openhoof` (this repo)
- Plugins: separate npm packages under `@openhoof/*` (example: `@openhoof/voice-call`)

Publishing contract:

- Plugin `package.json` must include `openhoof.extensions` with one or more entry files.
- Optional: `openhoof.setupEntry` may point at a lightweight setup-only entry for disabled or still-unconfigured channel setup.
- Optional: `openhoof.startup.deferConfiguredChannelFullLoadUntilAfterListen` may opt a channel plugin into using `setupEntry` during pre-listen gateway startup, but only when that setup entry completely covers the plugin's startup-critical surface.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `openhoof plugins install <npm-spec>` uses `npm pack`, extracts into `~/.openhoof/extensions/<id>/`, and enables it in config.
...
- CLI: `openhoof voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `openhoof.extensions` points at the built entrypoint (`dist/index.js`).
