---
summary: "Agent tool surface for OpenHoof (browser, canvas, nodes, message, cron) replacing legacy `openhoof-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openhoof-*` skills
title: "Tools"
---

# Tools (OpenHoof)

OpenHoof exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openhoof-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openhoof.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenHoof logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openhoof`: all built-in OpenHoof tools (excludes provider plugins)
...
- [Mustang](/tools/mustang): typed workflow runtime with resumable approvals (requires the Mustang CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`openhoof node run`).
...
OpenHoof tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `openhoof configure --section web`).
...
Control the dedicated OpenHoof-managed browser.
...
- Omit `profile` for the safe default: isolated OpenHoof-managed browser (`openhoof`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `openhoof`).
...
- Quick smoke: `openhoof nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- Available when `agents.defaults.imageGenerationModel` is configured, or when OpenHoof can infer a compatible image-generation default from your enabled providers plus available auth.
...
- When editing and `resolution` is omitted, OpenHoof infers a draft/final resolution from the input image size.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openhoof gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, OpenHoof uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.openhoof/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OpenHoof clamps `tools.sessions.visibility` to `tree`.
