...
- **gateway host** → `openhoof` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, OpenHoof also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openhoof/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.openhoof/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `openhoof approvals allowlist ...`).
- `openhoof security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `openhoof doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.openhoof/exec-approvals.json` directly.

CLI: `openhoof approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, OpenHoof does not treat
...
topics, OpenHoof preserves the topic for the approval prompt and the post-approval follow-up.
