...
- `commands.config` (default `false`) enables `/config` (reads/writes `openhoof.json`).
- `commands.mcp` (default `false`) enables `/mcp` (reads/writes OpenHoof-managed MCP config under `mcp.servers`).
...
- `/mcp show|get|set|unset` (manage OpenHoof MCP server config, owner-only; requires `commands.mcp: true`)
...
- For full provider usage breakdown, use `openhoof status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from OpenHoof session logs.
...
```
/debug show
/debug set messages.responsePrefix="[openhoof]"
...
- Overrides apply immediately to new config reads, but do **not** write to `openhoof.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`openhoof.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[openhoof]"
...
`/mcp` writes OpenHoof-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
- `/mcp` stores config in OpenHoof config, not Pi-owned project settings.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/openhoof`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
