...
read_when:
  - Running OpenHoof Gateway in WSL2 while Chrome lives on Windows
...
- OpenHoof Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/openhoof/openhoof/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- OpenHoof and Chrome are on the same machine
...
- OpenHoof points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, OpenHoof is not the problem yet.
...
Fix that before touching OpenHoof config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point OpenHoof at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting OpenHoof to succeed
...
```bash
openhoof browser open https://example.com --browser-profile remote
openhoof browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `openhoof browser tabs` returns the target
...
3. OpenHoof config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug OpenHoof config or Control UI auth
