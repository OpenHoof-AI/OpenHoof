...
read_when:
  - You want to place an outbound voice call from OpenHoof
...
Voice calls for OpenHoof via a plugin. Supports outbound notifications and
...
- Use `openhoof voicecall ...` or the `voice_call` tool
...
```bash
openhoof plugins install ./extensions/voice-call
...
openhoof voicecall start --to "+15555550123"   # alias for call
...
openhoof voicecall latency                     # summarize turn latency from logs
