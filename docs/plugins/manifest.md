...
read_when:
  - You are building an OpenHoof plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (openhoof.plugin.json)

This page is for the **native OpenHoof plugin manifest** only.
...
OpenHoof auto-detects those bundle layouts too, but they are not validated
against the `openhoof.plugin.json` schema described here.

For compatible bundles, OpenHoof currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches OpenHoof runtime expectations.

Every native OpenHoof plugin **must** ship a `openhoof.plugin.json` file in the
**plugin root**. OpenHoof uses this manifest to validate configuration
...
- `providerAuthEnvVars` (object): auth env vars keyed by provider id. Use this
  when OpenHoof should resolve provider credentials from env without loading
  plugin runtime first.
- `providerAuthChoices` (array): cheap onboarding/auth-choice metadata keyed by
  provider + auth method. Use this when OpenHoof should show a provider in
...
- The manifest is **required for native OpenHoof plugins**, including local filesystem loads.
