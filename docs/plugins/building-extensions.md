...
summary: "Step-by-step guide for creating OpenHoof channel and provider extensions"
read_when:
  - You want to create a new OpenHoof plugin or extension
  - You need to understand the plugin SDK import patterns
  - You are adding a new channel or provider to OpenHoof
---

# Building Extensions

This guide walks through creating an OpenHoof extension from scratch. Extensions
can add channels, model providers, tools, or other capabilities.

## Prerequisites

- OpenHoof repository cloned and dependencies installed (`pnpm install`)
...
├── package.json          # npm metadata + openhoof config
...
```json
{
  "name": "@openhoof/my-channel",
  "version": "2026.1.1",
  "description": "OpenHoof My Channel plugin",
  "type": "module",
  "dependencies": {},
  "openhoof": {
    "extensions": ["./index.ts"],
...
The `openhoof` field tells the plugin system what your extension provides.
...
```typescript
import { defineChannelPluginEntry } from "openhoof/plugin-sdk/core";

export default defineChannelPluginEntry({
  id: "my-channel",
  name: "My Channel",
  description: "Connects OpenHoof to My Channel",
  plugin: {
...
import { defineChannelPluginEntry } from "openhoof/plugin-sdk/core";
import { resolveOutboundSendDep } from "openhoof/plugin-sdk/channel-runtime";
...
import { ... } from "openhoof/plugin-sdk";
```
...
`openhoof/plugin-sdk/my-channel` from production files. Route internal imports
through `./api.ts` or `./runtime-api.ts` instead. The SDK subpath is the
external contract only.

## Step 5: Add a plugin manifest

Create `openhoof.plugin.json` in your extension root:
...
OpenHoof runs contract tests against all registered plugins. After adding your
extension, run:
...
```typescript
import { createTestRuntime } from "openhoof/plugin-sdk/testing";
```
...
1. **No monolithic root imports** — `openhoof/plugin-sdk` root is rejected
...
- [ ] `package.json` has correct `openhoof` metadata
...
- [ ] `openhoof.plugin.json` manifest is present and valid
