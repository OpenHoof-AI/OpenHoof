...
read_when:
  - Running OpenHoof behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OpenHoof
...
- You run OpenHoof behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OpenHoof checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OpenHoof extracts the user identity from the configured header
...
- OpenHoof can stay on loopback HTTP behind the proxy.
...
If OpenHoof itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://openhoof.example.com
    to: http://openhoof-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
openhoof.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy openhoof:18789 {
...
location / {
    ...
    proxy_pass http://openhoof:18789;
...
`openhoof security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OpenHoof config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `openhoof security audit` and review findings
