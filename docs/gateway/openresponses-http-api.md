...
OpenHoof’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`openhoof agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "openhoof:<agentId>"`, `model: "agent:<agentId>"`, or `x-openhoof-agent-id`
- use `x-openhoof-session-key` for explicit session routing
