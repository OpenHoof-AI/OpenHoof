...
- `openhoof gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `openhoof doctor` reports no blocking config/service issues.
- `openhoof channels status --probe` shows connected/ready channels.
...
openhoof config get agents.defaults.models
```
...
openhoof status
openhoof channels status --probe
openhoof pairing list --channel <channel> [--account <id>]
openhoof config get channels
openhoof logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `openhoof config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `openhoof devices list` then `openhoof devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `openhoof configure`). If you are running OpenHoof via Podman using the dedicated `openhoof` user, the config lives at `~openhoof/.openhoof/openhoof.json`.
...
```bash
openhoof cron status
openhoof cron list
openhoof cron runs --id <jobId> --limit 20
openhoof system heartbeat last
openhoof logs --follow
```
...
openhoof approvals get --node <idOrNameOrIp>
openhoof logs --follow
openhoof status
```
...
openhoof browser start --browser-profile openhoof
...
```bash
openhoof gateway status
openhoof config get gateway.mode
openhoof config get gateway.remote.url
openhoof config get gateway.auth.mode
```
...
```bash
openhoof config get gateway.bind
