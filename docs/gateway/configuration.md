...
read_when:
  - Setting up OpenHoof for the first time
...
OpenHoof reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.openhoof/openhoof.json`.

If the file is missing, OpenHoof uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `openhoof onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    openhoof onboard       # full onboarding flow
    openhoof configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    openhoof config get agents.defaults.workspace
    openhoof config set agents.defaults.heartbeat.every "2h"
    openhoof config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.openhoof/openhoof.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
OpenHoof only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`openhoof doctor`, `openhoof logs`, `openhoof health`, `openhoof status`)
- Run `openhoof doctor` to see exact issues
- Run `openhoof doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@openhoof", "openhoof"],
            },
          },
        ],
      },
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `openhoof.json`.
    ...
    ```bash
    openhoof config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `OPENHOOF_APNS_RELAY_BASE_URL` and `OPENHOOF_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `OPENHOOF_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.openhoof/openhoof.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `openhoof config set` for single keys.
    </Warning>
    ...
    ```bash
    openhoof gateway call config.get --params '{}'  # capture payload.hash
    openhoof gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.openhoof/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    openhoof gateway call config.patch --params '{
...
OpenHoof reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.openhoof/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, OpenHoof runs your login shell and imports only the missing keys:
...
Env var equivalent: `OPENHOOF_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${OPENHOOF_GATEWAY_TOKEN}" } },
