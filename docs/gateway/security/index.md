...
> OpenHoof is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
OpenHoof security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `openhoof security audit`
...
```bash
openhoof security audit
openhoof security audit --deep
openhoof security audit --fix
openhoof security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

OpenHoof is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
OpenHoof assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.openhoof`, including `openhoof.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- OpenHoof can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want OpenHoof, use one VPS/host per user.
...
OpenHoof is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/openhoof/openhoof/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, OpenHoof also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.openhoof/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.openhoof/credentials/<channel>-allowFrom.json` (default account)
  - `~/.openhoof/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.openhoof/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.openhoof/secrets.json`
- **Legacy OAuth import**: `~/.openhoof/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full OpenHoof state                                 | filesystem perms on `~/.openhoof`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.openhoof/openhoof.json`                                                   | yes      |
...
`openhoof security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`openhoof security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in OpenHoof config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${OPENHOOF_GATEWAY_PASSWORD}
```
...
- OpenHoof gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from OpenHoof responses.
...
OpenHoof stores session transcripts on disk under `~/.openhoof/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.openhoof` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If OpenHoof cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
- If you don’t want remote execution, set security to **deny** and remove node pairing for that Mac.

## Dynamic skills (watcher / remote nodes)

OpenHoof can refresh the skills list mid-session:
...
- If you install plugins from npm (`openhoof plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openhoof/extensions/<pluginId>/` (or `$OPENHOOF_STATE_DIR/extensions/<pluginId>/`).
  - OpenHoof uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
openhoof pairing list <channel>
openhoof pairing approve <channel> <code>
```
...
By default, OpenHoof routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
OpenHoof has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.openhoof/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.openhoof or your logs.”

## Unsafe external content bypass flags

OpenHoof includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.openhoof/openhoof.json`: `600` (user read/write only)
- `~/.openhoof`: `700` (user only)

`openhoof doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `OPENHOOF_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__openhoof__/canvas/` and `/__openhoof__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run OpenHoof with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_openhoof-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `OPENHOOF_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `openhoof doctor --generate-gateway-token`.
...
paths, set `OPENHOOF_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `OPENHOOF_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `OPENHOOF_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), OpenHoof
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. OpenHoof verifies the identity by resolving the
...
- OpenHoof will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.openhoof/` (or `$OPENHOOF_STATE_DIR/`) may contain secrets or private data:

- `openhoof.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `openhoof status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@openhoof", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.openhoof`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.openhoof/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `openhoof` profile).
...
OpenHoof’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default OpenHoof limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `openhoof gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `OPENHOOF_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/openhoof/openhoof-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.openhoof/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `openhoof security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + OpenHoof version
...
Found a vulnerability in OpenHoof? Please report responsibly:

1. Email: [security@openhoof.ai](mailto:security@openhoof.ai)
