...
- During heartbeat runs, OpenHoof treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openhoof sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenHoof skips the heartbeat run entirely (no model call).
...
```bash
openhoof system event --text "Check for urgent follow-ups" --mode now
```
