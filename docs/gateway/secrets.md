...
OpenHoof supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `OPENHOOF_GATEWAY_TOKEN` (or `HOOFBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, OpenHoof runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). OpenHoof validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/OpenHoof QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/openhoof"],
...
- OpenHoof resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `openhoof.json` refs)
...
When reload-time activation fails after a healthy state, OpenHoof enters degraded secrets state.
...
- Strict command paths (for example `openhoof memory` remote-memory paths and `openhoof qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `openhoof status`, `openhoof status --all`, `openhoof channels status`, `openhoof channels resolve`, `openhoof security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `openhoof secrets reload`.
...
```bash
openhoof secrets audit --check
openhoof secrets configure
openhoof secrets audit --check
```
...
- plaintext values at rest (`openhoof.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `openhoof.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `openhoof secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `openhoof.json` plus `auth-profiles.json` for one agent scope
...
- `openhoof secrets configure --providers-only`
- `openhoof secrets configure --skip-provider-setup`
- `openhoof secrets configure --agent <id>`
...
```bash
openhoof secrets apply --from /tmp/openhoof-secrets-plan.json
openhoof secrets apply --from /tmp/openhoof-secrets-plan.json --allow-exec
openhoof secrets apply --from /tmp/openhoof-secrets-plan.json --dry-run
openhoof secrets apply --from /tmp/openhoof-secrets-plan.json --dry-run --allow-exec
```
...
OpenHoof intentionally does not write rollback backups containing historical plaintext secret values.
