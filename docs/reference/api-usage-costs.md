...
OpenHoof features that can generate provider usage or paid API calls.
...
- `openhoof status --usage` and `openhoof channels list` show provider **usage windows**
...
OpenHoof can pick up credentials from:
...
- `openhoof status --usage`
- `openhoof models status --json`
...
`openhoof models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
