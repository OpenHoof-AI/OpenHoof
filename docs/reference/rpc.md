...
OpenHoof integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenHoof owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenHoof spawns `imsg rpc` as a child process (legacy iMessage integration).
