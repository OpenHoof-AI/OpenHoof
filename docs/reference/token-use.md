---
summary: "How OpenHoof builds prompt context and reports token usage + costs"
read_when:
...
OpenHoof tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OpenHoof assembles its own system prompt on every run. It includes:
...
For images, OpenHoof downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OpenHoof session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `openhoof status --usage` and `openhoof channels list` show
...
`cacheWrite`. If pricing is missing, OpenHoof shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OpenHoof can
...
Anthropic's 1M context window is currently beta-gated. OpenHoof can inject the
...
OpenHoof skips the `context-1m-*` beta header because Anthropic currently
