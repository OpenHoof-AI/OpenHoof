...
- With Anthropic API-key auth profiles, OpenHoof seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, OpenHoof injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
OpenHoof exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.openhoof/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$OPENHOOF_STATE_DIR/logs/cache-trace.jsonl`
...
- `OPENHOOF_CACHE_TRACE=1` enables cache tracing.
- `OPENHOOF_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `OPENHOOF_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `OPENHOOF_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `OPENHOOF_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
