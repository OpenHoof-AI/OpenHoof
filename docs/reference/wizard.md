...
This is the full reference for `openhoof onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.openhoof/openhoof.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `openhoof doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.openhoof/credentials/oauth.json`; auth profiles live in `~/.openhoof/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.openhoof/credentials/oauth.json` (or `$OPENHOOF_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.openhoof/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `openhoof pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `openhoof configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `openhoof health`.
    - Tip: `openhoof status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export OPENHOOF_GATEWAY_TOKEN="your-token"
openhoof onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env OPENHOOF_GATEWAY_TOKEN
```
...
- Stores it under `~/.openhoof/tools/signal-cli/<version>/`.
...
Typical fields in `~/.openhoof/openhoof.json`:
...
`openhoof agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.openhoof/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.openhoof/agents/<agentId>/sessions/`.
