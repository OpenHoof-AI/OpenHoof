...
service via the external `openhoof` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.openhoof.gateway`
  (or `ai.openhoof.<profile>` when using `--profile`/`OPENHOOF_PROFILE`; legacy `com.openhoof.*` is supported).
