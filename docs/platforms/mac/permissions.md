...
- Same path: run the app from a fixed location (for OpenHoof, `dist/OpenHoof.app`).
...
```bash
sudo tccutil reset Accessibility ai.openhoof.mac
sudo tccutil reset ScreenCapture ai.openhoof.mac
...
Workaround: move files into the OpenHoof workspace (`~/.openhoof/workspace`) if you want to avoid per-folder grants.
