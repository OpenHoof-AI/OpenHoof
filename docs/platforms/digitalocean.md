---
summary: "OpenHoof on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up OpenHoof on DigitalOcean
  - Looking for cheap VPS hosting for OpenHoof
title: "DigitalOcean"
---

# OpenHoof on DigitalOcean

## Goal

Run a persistent OpenHoof Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status openhoof-gateway.service

# View logs
journalctl --user -u openhoof-gateway.service -f
```
...
```bash
openhoof config set gateway.bind tailnet
openhoof gateway restart
```
...
```bash
openhoof channels login whatsapp
# Scan QR code
```
...
- `~/.openhoof/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u openhoof --no-pager -n 50
```
