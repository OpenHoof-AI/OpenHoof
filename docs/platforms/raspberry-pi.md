---
summary: "OpenHoof on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenHoof on a Raspberry Pi
  - Running OpenHoof on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenHoof on Raspberry Pi

## Goal

Run a persistent, always-on OpenHoof Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u openhoof -f
```

## 9) Access the OpenHoof Dashboard
...
```bash
ssh user@gateway-host 'openhoof dashboard --no-open'
```
...
(or `OPENHOOF_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/openhoof-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/openhoof-compile-cache
mkdir -p /var/tmp/openhoof-compile-cache
export OPENHOOF_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `OPENHOOF_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running OpenHoof, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit openhoof
```

```ini
[Service]
Environment=OPENHOOF_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/openhoof-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep OpenHoof state/cache on SSD-backed storage to avoid SD-card
...
Most OpenHoof features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openhoof

# Enable if not
sudo systemctl enable openhoof

# Start on boot
sudo systemctl start openhoof
```
...
```bash
# Check logs
journalctl -u openhoof --no-pager -n 100

# Common fix: rebuild
cd ~/openhoof  # if using hackable install
npm run build
sudo systemctl restart openhoof
```
