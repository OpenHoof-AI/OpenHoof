...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenHoof
...
Running OpenHoof via Node with `tsx` fails at startup with:

```
[openhoof] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openhoof status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openhoof.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openhoof.mjs status` works on Node 25.
