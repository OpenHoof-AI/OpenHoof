...
OpenHoof supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, OpenHoof also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `openhoof configure --section web`. It stores the key in
`~/.openhoof/openhoof.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.openhoof/.env` (or your service environment). See [Env vars](/help/faq#how-does-openhoof-load-environment-variables).
