...
- Jobs persist under `~/.openhoof/cron/` so restarts don’t lose schedules.
...
- For upgrades, `openhoof doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
openhoof cron list
openhoof cron run <job-id>
openhoof cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.openhoof/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openhoof cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, OpenHoof applies a
...
If `delivery` is omitted for isolated jobs, OpenHoof defaults to `announce`.
...
- CLI equivalents: `openhoof cron add --light-context ...` and `openhoof cron edit --light-context`.
...
- Job store: `~/.openhoof/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openhoof/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, OpenHoof classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, OpenHoof archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `openhoof cron runs` and adjust retention before logs become large
...
openhoof cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openhoof cron edit <jobId> --agent ops
openhoof cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
openhoof cron run <jobId>
openhoof cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `openhoof cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
openhoof cron edit <jobId> \
...
```bash
openhoof cron edit <jobId> --exact
```
...
```bash
openhoof system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openhoof system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `OPENHOOF_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenHoof applies exponential retry backoff for recurring jobs after consecutive errors:
