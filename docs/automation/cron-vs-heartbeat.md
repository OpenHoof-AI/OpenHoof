...
openhoof cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
openhoof cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
openhoof cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```

## Mustang: Deterministic workflows with approvals

Mustang is the workflow runtime for **multi-step tool pipelines** that need deterministic execution and explicit approvals.
...
- **Mustang** defines _what steps_ happen once the run starts.

For scheduled workflows, use cron or heartbeat to trigger an agent turn that calls Mustang.
For ad-hoc workflows, call Mustang directly.

### Operational notes (from the code)

- Mustang runs as a **local subprocess** (`mustang` CLI) in tool mode and returns a **JSON envelope**.
- If the tool returns `needs_approval`, you resume with a `resumeToken` and `approve` flag.
- The tool is an **optional plugin**; enable it additively via `tools.alsoAllow: ["mustang"]` (recommended).
- Mustang expects the `mustang` CLI to be available on `PATH`.

See [Mustang](/tools/mustang) for full usage and examples.
