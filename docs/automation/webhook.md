...
read_when:
  ...
  - Wiring external systems into OpenHoof
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your OpenHoof config directory (typically `~/.openhoof/hooks/transforms`).
...
- `openhoof webhooks gmail setup` writes `hooks.gmail` config for `openhoof webhooks gmail run`.
