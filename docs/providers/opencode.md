---
summary: "Use OpenCode Zen and Go catalogs with OpenHoof"
read_when:
...
OpenCode exposes two hosted catalogs in OpenHoof:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. OpenHoof keeps the runtime provider ids
...
openhoof onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
openhoof onboard --auth-choice opencode-go
openhoof onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
