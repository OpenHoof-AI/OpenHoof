---
summary: "Use OpenAI via API keys or Codex subscription in OpenHoof"
read_when:
  - You want to use OpenAI models in OpenHoof
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like OpenHoof.
...
```bash
openhoof onboard --auth-choice openai-api-key
# or non-interactive
openhoof onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. OpenHoof forwards both through the `openai/*` Responses path.
OpenHoof intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

OpenHoof does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in OpenHoof.
...
openhoof onboard --auth-choice openai-codex

# Or run OAuth directly
openhoof models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. OpenHoof
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, OpenHoof also supports:

- `openai-codex/gpt-5.3-codex-spark`

OpenHoof treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

OpenHoof also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
OpenHoof uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), OpenHoof also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. OpenHoof enables it by default for
...
OpenHoof, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
OpenHoof exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, OpenHoof applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), OpenHoof now auto-enables OpenAI server-side
