---
summary: "Use Synthetic's Anthropic-compatible API in OpenHoof"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OpenHoof registers it as the
...
```bash
openhoof onboard --auth-choice synthetic-api-key
```
...
Note: OpenHoof's Anthropic client appends `/v1` to the base URL, so use
