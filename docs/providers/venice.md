---
summary: "Use Venice AI privacy-focused models in OpenHoof"
read_when:
  - You want privacy-focused inference in OpenHoof
...
```bash
openhoof onboard --auth-choice venice-api-key
```
...
```bash
openhoof agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, OpenHoof shows all available Venice models. Pick based on your needs:
...
```bash
openhoof models set venice/kimi-k2-5
openhoof models set venice/claude-opus-4-6
```

List all available models:

```bash
openhoof models list | grep venice
```

## Configure via `openhoof configure`

1. Run `openhoof configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenHoof automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openhoof agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
openhoof agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
openhoof agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
openhoof agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
openhoof agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `openhoof models list` to see currently available models. Some models may be temporarily offline.
