---
summary: "Use MiniMax M2.5 in OpenHoof"
read_when:
  - You want MiniMax models in OpenHoof
...
```bash
openhoof plugins enable minimax  # skip if already loaded.
openhoof gateway restart  # restart if gateway is already running
openhoof onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/openhoof/openhoof/tree/main/extensions/minimax) for details.
...
Configure manually via `openhoof.json`:
...
- Use `openhoof models list` and `openhoof models set minimax/MiniMax-M2.5` to switch.
...
- Running `openhoof configure` and selecting **MiniMax M2.5**, or
