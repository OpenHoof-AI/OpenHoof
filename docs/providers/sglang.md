---
summary: "Run OpenHoof with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run OpenHoof against a local SGLang server
...
OpenHoof can connect to SGLang using the `openai-completions` API.

OpenHoof can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, OpenHoof will query:
