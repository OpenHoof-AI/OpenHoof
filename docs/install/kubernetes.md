---
summary: "Deploy OpenHoof Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run OpenHoof on a Kubernetes cluster
  - You want to test OpenHoof in a Kubernetes environment
title: "Kubernetes"
---

# OpenHoof on Kubernetes

A minimal starting point for running OpenHoof on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

OpenHoof is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/openhoof 18789:18789 -n openhoof
...
```bash
kubectl get secret openhoof-secrets -n openhoof -o jsonpath='{.data.OPENHOOF_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: openhoof (configurable via OPENHOOF_NAMESPACE)
├── Deployment/openhoof        # Single pod, init container + gateway
├── Service/openhoof           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/openhoof-config  # openhoof.json + AGENTS.md
└── Secret/openhoof-secrets    # Gateway token + API keys
```
...
Edit `openhoof.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret openhoof-secrets -n openhoof \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/openhoof -n openhoof
```

### Custom namespace

```bash
OPENHOOF_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/openhoof/openhoof:2026.3.1
```
...
└── manifests/
    ...
    ├── configmap.yaml          # openhoof.json + AGENTS.md
