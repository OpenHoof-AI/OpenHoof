...
Deploy OpenHoof on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/hoofbot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/openhoof` — Control UI

## What you get

- Hosted OpenHoof Gateway + Control UI
...
- `OPENHOOF_STATE_DIR=/data/.openhoof` (recommended)
- `OPENHOOF_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENHOOF_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OpenHoof state + workspace so you can migrate to another host without losing config or memory.
