...
Deploy OpenHoof on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/openhoof/openhoof)
...
repository configures everything needed to run OpenHoof:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENHOOF_STATE_DIR
        value: /data/.openhoof
      - key: OPENHOOF_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENHOOF_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/openhoof`.
...
If you use the original OpenHoof repository, Render will not auto-deploy your OpenHoof. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenHoof, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenHoof host.
