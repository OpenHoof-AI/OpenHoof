---
read_when:
  - 你想在 OpenHoof 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 OpenHoof 中使用 MiniMax M2.5
title: MiniMax
x-i18n:
...
```bash
openhoof plugins enable minimax  # 如果已加载则跳过。
openhoof gateway restart  # 如果 gateway 已在运行，则重启
...
详情请参阅 [MiniMax plugin README](https://github.com/openhoof/openhoof/tree/main/extensions/minimax)。
...
- 使用 `openhoof models list` 和 `openhoof models set minimax/MiniMax-M2.5` 进行切换。
...
- 运行 `openhoof configure` 并选择 **MiniMax M2.5**，或者
