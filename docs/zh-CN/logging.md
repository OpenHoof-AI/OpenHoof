...
你可以在 `~/.openhoof/openhoof.json` 中覆盖此设置：
...
所有日志配置都在 `~/.openhoof/openhoof.json` 的 `logging` 下。
...
- OpenHoof 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `openhoof plugins enable diagnostics-otel` 启用插件。
...
- `openhoof.tokens`（计数器，属性：`openhoof.token`、`openhoof.channel`、`openhoof.provider`、`openhoof.model`）
- `openhoof.cost.usd`（计数器，属性：`openhoof.channel`、`openhoof.provider`、`openhoof.model`）
- `openhoof.run.duration_ms`（直方图，属性：`openhoof.channel`、`openhoof.provider`、`openhoof.model`）
- `openhoof.context.tokens`（直方图，属性：`openhoof.context`、`openhoof.channel`、`openhoof.provider`、`openhoof.model`）

消息流：

- `openhoof.webhook.received`（计数器，属性：`openhoof.channel`、`openhoof.webhook`）
- `openhoof.webhook.error`（计数器，属性：`openhoof.channel`、`openhoof.webhook`）
- `openhoof.webhook.duration_ms`（直方图，属性：`openhoof.channel`、`openhoof.webhook`）
- `openhoof.message.queued`（计数器，属性：`openhoof.channel`、`openhoof.source`）
- `openhoof.message.processed`（计数器，属性：`openhoof.channel`、`openhoof.outcome`）
- `openhoof.message.duration_ms`（直方图，属性：`openhoof.channel`、`openhoof.outcome`）

队列 + 会话：

- `openhoof.queue.lane.enqueue`（计数器，属性：`openhoof.lane`）
- `openhoof.queue.lane.dequeue`（计数器，属性：`openhoof.lane`）
- `openhoof.queue.depth`（直方图，属性：`openhoof.lane` 或 `openhoof.channel=heartbeat`）
- `openhoof.queue.wait_ms`（直方图，属性：`openhoof.lane`）
- `openhoof.session.state`（计数器，属性：`openhoof.state`、`openhoof.reason`）
- `openhoof.session.stuck`（计数器，属性：`openhoof.state`）
- `openhoof.session.stuck_age_ms`（直方图，属性：`openhoof.state`）
- `openhoof.run.attempt`（计数器，属性：`openhoof.attempt`）
...
- `openhoof.webhook.error`
  - `openhoof.channel`、`openhoof.webhook`、`openhoof.chatId`、`openhoof.error`
- `openhoof.message.processed`
  - `openhoof.channel`、`openhoof.outcome`、`openhoof.chatId`、`openhoof.messageId`、`openhoof.sessionKey`、`openhoof.sessionId`、`openhoof.reason`
- `openhoof.session.stuck`
  - `openhoof.state`、`openhoof.ageMs`、`openhoof.queueDepth`、`openhoof.sessionKey`、`openhoof.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `openhoof doctor`。
