...
OpenHoof 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`openhoof gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`openhoof onboard --install-daemon`
- 直接安装：`openhoof gateway install`
- 配置流程：`openhoof configure` → 选择 **Gateway 服务**
- 修复/迁移：`openhoof doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.openhoof.gateway` 或 `ai.openhoof.<profile>`；旧版为 `com.openhoof.*`）
- Linux/WSL2：systemd 用户服务（`openhoof-gateway[-<profile>].service`）
