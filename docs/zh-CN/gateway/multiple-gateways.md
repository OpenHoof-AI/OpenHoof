...
summary: 在一台主机上运行多个 OpenHoof Gateway 网关（隔离、端口和配置档案）
title: 多个 Gateway 网关
x-i18n:
...
- `OPENHOOF_CONFIG_PATH` — 每个实例单独的配置文件
- `OPENHOOF_STATE_DIR` — 每个实例单独的会话、凭证、缓存
...
配置档案会自动限定 `OPENHOOF_STATE_DIR` + `OPENHOOF_CONFIG_PATH`，并为服务名称添加后缀。
