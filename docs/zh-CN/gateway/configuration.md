...
OpenHoof 会从 `~/.openhoof/openhoof.json` 读取可选的 <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> 配置。

如果该文件缺失，OpenHoof 会使用安全的默认值。添加配置的常见原因包括：
...
<Tip>
**刚接触配置？** 从 `openhoof onboard` 开始进行交互式设置，或者查看 [Configuration Examples](/gateway/configuration-examples) 指南，获取完整的可复制粘贴配置。
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    openhoof onboard       # 完整设置向导
    openhoof configure     # 配置向导
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    直接编辑 `~/.openhoof/openhoof.json`。Gateway 网关会监视该文件并自动应用更改（参见[热重载](#config-hot-reload)）。
  </Tab>
</Tabs>
...
<Warning>
OpenHoof 只接受完全符合 schema 的配置。未知键、类型格式错误或无效值都会导致 Gateway 网关**拒绝启动**。唯一的根级例外是 `$schema`（字符串），这样编辑器就可以附加 JSON Schema 元数据。
</Warning>

当校验失败时：

- Gateway 网关不会启动
- 只有诊断命令可用（`openhoof doctor`、`openhoof logs`、`openhoof health`、`openhoof status`）
- 运行 `openhoof doctor` 以查看具体问题
- 运行 `openhoof doctor --fix`（或 `--yes`）以应用修复
...
<AccordionGroup>
  ...
  <Accordion title="为官方 iOS 构建启用基于 relay 的推送">
    基于 relay 的推送在 `openhoof.json` 中配置。
    ...
    - `OPENHOOF_APNS_RELAY_BASE_URL` 和 `OPENHOOF_APNS_RELAY_TIMEOUT_MS` 仍可作为临时环境变量覆盖使用。
    - `OPENHOOF_APNS_RELAY_ALLOW_HTTP=true` 仍是仅限 loopback 的开发逃生口；不要在配置中持久化 HTTP relay URL。
...
Gateway 网关会监视 `~/.openhoof/openhoof.json` 并自动应用更改 —— 对于大多数设置，无需手动重启。
...
<AccordionGroup>
  <Accordion title="config.apply（完整替换）">
    ...
    <Warning>
    `config.apply` 会替换**整个配置**。部分更新请使用 `config.patch`，单个键请使用 `openhoof config set`。
    </Warning>
    ...
    ```bash
    openhoof gateway call config.get --params '{}'  # 捕获 payload.hash
...
<Accordion title="Shell 环境变量导入（可选）">
  如果启用，并且预期键名尚未设置，OpenHoof 会运行你的登录 shell，并且只导入缺失的键：
...
环境变量等价项：`OPENHOOF_LOAD_SHELL_ENV=1`
</Accordion>
