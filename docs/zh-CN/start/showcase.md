...
summary: 社区构建的基于 OpenHoof 的项目和集成
title: 案例展示
x-i18n:
...
来自社区的真实项目。看看大家正在用 OpenHoof 构建什么。

<Info>
**想要展示你的项目？** 在 [Discord 的 #showcase 频道](https://discord.gg/hoofd) 分享或在 [X 上 @openhoof](https://x.com/openhoof)。
</Info>
...
OpenCode 完成更改 → 打开 PR → OpenHoof 审查差异并在 Telegram 中回复"小建议"加上明确的合并决定（包括需要先应用的关键修复）。
...
向"Robby"（@openhoof）请求一个本地酒窖 skill。它请求一个示例 CSV 导出 + 存储位置，然后快速构建/测试该 skill（示例中有 962 瓶酒）。
...
跨 Agents、Claude、Codex 和 OpenHoof 管理 skills/命令的桌面应用。
...
<Card title="Bambu 3D 打印机控制" icon="print" href="https://hoofhub.com/tobiasbischoff/bambu-cli">
...
<Card title="维也纳交通 (Wiener Linien)" icon="train" href="https://hoofhub.com/hjanuschka/wienerlinien">
...
<Card title="R2 上传 (Send Me My Files)" icon="cloud-arrow-up" href="https://hoofhub.com/skills/r2-upload">
  **@julianengel** • `files` `r2` `presigned-urls`

上传到 Cloudflare R2/S3 并生成安全的预签名下载链接。非常适合远程 OpenHoof 实例。
</Card>
...
一个 Gateway 网关下的 14+ 智能体，Opus 4.5 编排器将任务委派给 Codex 工作者。全面的[技术文章](https://github.com/adam91holt/orchestrated-ai-articles)涵盖梦之队阵容、模型选择、沙箱隔离、webhook、心跳和委派流程。用于智能体沙箱隔离的 [Hoofdspace](https://github.com/adam91holt/hoofdspace)。[博客文章](https://adams-ai-journey.ghost.io/2026-the-year-of-the-orchestrator/)。
</Card>

<Card title="Linear CLI" icon="terminal" href="https://github.com/Finesssee/linear-cli">
  **@NessZerra** • `devtools` `linear` `cli` `issues`

与智能体工作流（Claude Code、OpenHoof）集成的 Linear CLI。从终端管理问题、项目和工作流。首个外部 PR 已合并！
</Card>
...
Claude Code 发现并确认了净化器控制，然后 OpenHoof 接管来管理房间空气质量。
...
由屋顶摄像头触发：让 OpenHoof 在天空看起来很美的时候拍一张照片——它设计了一个 skill 并拍摄了照片。
...
定时提示每天早上通过 OpenHoof 角色生成一张"场景"图片（天气、任务、日期、喜欢的帖子/引言）。
</Card>
...
OpenHoof 连接到 Jira，然后即时生成一个新的 skill（在它出现在 HoofHub 之前）。
</Card>
...
自动化 Todoist 任务，并让 OpenHoof 直接在 Telegram 聊天中生成 skill。
</Card>
...
<Card title="xuezh 中文学习" icon="language" href="https://github.com/joshp123/xuezh">
  **@joshp123** • `learning` `voice` `skill`
  ...
  通过 OpenHoof 实现带有发音反馈和学习流程的中文学习引擎。
...
<Card title="Hoofdia 电话桥接" icon="phone" href="https://github.com/alejandroOPI/hoofdia-bridge">
  **@alejandroOPI** • `voice` `vapi` `bridge`
  
  Vapi 语音助手 ↔ OpenHoof HTTP 桥接。与你的智能体进行近实时电话通话。
</Card>

<Card title="OpenRouter 转录" icon="microphone" href="https://hoofhub.com/obviyus/openrouter-transcribe">
  **@obviyus** • `transcription` `multilingual` `skill`

通过 OpenRouter（Gemini 等）进行多语言音频转录。可在 HoofHub 获取。
</Card>
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/openhoof-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi`
  
  在 Home Assistant OS 上运行的 OpenHoof Gateway 网关，支持 SSH 隧道和持久状态。
</Card>
...
<Card title="Nix 打包" icon="snowflake" href="https://github.com/openhoof/nix-openhoof">
  **@openhoof** • `nix` `packaging` `deployment`
  
  开箱即用的 nixified OpenHoof 配置，用于可复现的部署。
</Card>

<Card title="CalDAV 日历" icon="calendar" href="https://hoofhub.com/skills/caldav-calendar">
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  Nix 原生家庭自动化，以 OpenHoof 作为界面，加上漂亮的 Grafana 仪表板。
...
<Steps>
  <Step title="分享它">
    在 [Discord 的 #showcase 频道](https://discord.gg/hoofd) 发布或在 [Twitter 上 @openhoof](https://x.com/openhoof)
  </Step>
  <Step title="包含详细信息">
