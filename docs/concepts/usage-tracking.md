...
- `/usage cost` in chats: local cost summary aggregated from OpenHoof session logs.
- CLI: `openhoof status --usage` prints a full per-provider breakdown.
- CLI: `openhoof channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
