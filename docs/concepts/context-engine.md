...
read_when:
  - You want to understand how OpenHoof assembles model context
...
A **context engine** controls how OpenHoof builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

OpenHoof ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.openhoof/openhoof.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other OpenHoof plugin. Install
first, then select the engine in the slot:

```bash
# Install from npm
openhoof plugins install @martian-engineering/lossless-hoof

# Or install from a local path (for development)
openhoof plugins install -l ./my-context-engine
```
...
// openhoof.json
{
  plugins: {
    slots: {
      contextEngine: "lossless-hoof", // must match the plugin's registered engine id
    },
    entries: {
      "lossless-hoof": {
        enabled: true,
...
Every time OpenHoof runs a model prompt, the context engine participates at
...
OpenHoof currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. OpenHoof
...
The built-in `legacy` engine preserves OpenHoof's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. OpenHoof uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. OpenHoof disables Pi's built-in
...
`ownsCompaction: false` does **not** mean OpenHoof automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `openhoof/plugin-sdk/core` to use
  OpenHoof's built-in compaction behavior.
...
OpenHoof resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to OpenHoof's built-in summarization. Plugin engines can implement
...
- Use `openhoof doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, OpenHoof
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `openhoof plugins install -l ./my-engine` to link a
