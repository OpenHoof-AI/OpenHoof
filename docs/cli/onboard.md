---
summary: "CLI reference for `openhoof onboard` (interactive onboarding)"
read_when:
...
```bash
openhoof onboard
openhoof onboard --flow quickstart
openhoof onboard --flow manual
openhoof onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`OPENHOOF_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `openhoof gateway run`.
...
- Fastest first chat: `openhoof dashboard` (Control UI, no channel setup).
