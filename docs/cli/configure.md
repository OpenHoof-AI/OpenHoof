---
summary: "CLI reference for `openhoof configure` (interactive configuration prompts)"
read_when:
...
Tip: `openhoof config` without a subcommand opens the same wizard. Use
`openhoof config get|set|unset` for non-interactive edits.
...
```bash
openhoof configure
openhoof configure --section model --section channels
```
