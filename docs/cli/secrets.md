---
summary: "CLI reference for `openhoof secrets` (reload, audit, configure, apply)"
read_when:
...
Use `openhoof secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
Scan OpenHoof state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `openhoof.json` refs)
...
```bash
openhoof secrets audit
openhoof secrets audit --check
openhoof secrets audit --json
openhoof secrets audit --allow-exec
```
...
```bash
openhoof secrets configure
openhoof secrets configure --plan-out /tmp/openhoof-secrets-plan.json
openhoof secrets configure --apply --yes
openhoof secrets configure --providers-only
openhoof secrets configure --skip-provider-setup
openhoof secrets configure --agent ops
openhoof secrets configure --json
```
...
- `configure` targets secret-bearing fields in `openhoof.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, OpenHoof fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
openhoof secrets apply --from /tmp/openhoof-secrets-plan.json --json
```
...
- `openhoof.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.openhoof/.env` known secret keys whose values were migrated
