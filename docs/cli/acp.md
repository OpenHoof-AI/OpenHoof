...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an OpenHoof Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`openhoof acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the OpenHoof gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
openhoof acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
openhoof acp --url wss://gateway-host:18789 --token-file ~/.openhoof/gateway.token

# Attach to an existing session key
openhoof acp --session agent:main:main

# Attach by label (must already exist)
openhoof acp --session-label "support inbox"

# Reset the session key before the first prompt
openhoof acp --session agent:main:main --reset-session
```
...
openhoof acp client --server-args --url wss://gateway-host:18789 --token-file ~/.openhoof/gateway.token

# Override the server command (default: openhoof)
openhoof acp client --server "node" --server-args openhoof.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an OpenHoof Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `openhoof acp` over stdio.

Example config (persisted):

```bash
openhoof config set gateway.remote.url wss://gateway-host:18789
openhoof config set gateway.remote.token <token>
```
...
```bash
openhoof acp --session agent:main:main
openhoof acp --session agent:design:main
openhoof acp --session agent:qa:bug-123
```
...
OpenHoof bot over ACP, use `acpx` with its built-in `openhoof` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx openhoof` at `openhoof acp`.
3. Target the OpenHoof session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default OpenHoof ACP session
acpx openhoof exec "Summarize the active OpenHoof session state."

# Persistent named session for follow-up turns
acpx openhoof sessions ensure --name codex-bridge
acpx openhoof -s codex-bridge --cwd /path/to/repo \
  "Ask my OpenHoof work agent for recent context relevant to this repo."
```

If you want `acpx openhoof` to target a specific Gateway and session key every
time, override the `openhoof` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "openhoof": {
      "command": "env OPENHOOF_HIDE_BANNER=1 OPENHOOF_SUPPRESS_NOTES=1 openhoof acp --url ws://127.0.0.1:18789 --token-file ~/.openhoof/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local OpenHoof checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env OPENHOOF_HIDE_BANNER=1 OPENHOOF_SUPPRESS_NOTES=1 node openhoof.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an OpenHoof agent without scraping a terminal.
...
In Zed, open the Agent panel and select “OpenHoof ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`OPENHOOF_GATEWAY_TOKEN`, `OPENHOOF_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`OPENHOOF_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `OPENHOOF_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `openhoof acp client` sets `OPENHOOF_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `openhoof`).
