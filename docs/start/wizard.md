...
CLI onboarding is the **recommended** way to set up OpenHoof on macOS,
...
`openhoof dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`openhoof configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.openhoof/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `openhoof doctor` first.
</Note>
...
Use `openhoof agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.openhoof/workspace-<agentId>`.
...
- CLI command reference: [`openhoof onboard`](/cli/onboard)
