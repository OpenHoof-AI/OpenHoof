...
summary: "Architecture of OpenHoof's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in OpenHoof
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how OpenHoof integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

OpenHoof uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, OpenHoof directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in OpenHoof's local TUI mode)                                             |
...
├── pi-tools.ts                    # createOpenHoofCodingTools()
...
├── openhoof-tools.ts              # OpenHoof-specific tools
...
Image injection is prompt-local: OpenHoof loads image refs from the current prompt and
...
2. **Custom Replacements**: OpenHoof replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **OpenHoof Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures OpenHoof's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, OpenHoof CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
OpenHoof wraps this with `guardSessionManager()` for tool result safety.
...
OpenHoof maintains an auth profile store with multiple API keys per provider:
...
OpenHoof loads custom pi extensions for specialized behavior:
...
OpenHoof also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | OpenHoof Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom OpenHoof tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.openhoof/agents/<agentId>/sessions/` (or `$OPENHOOF_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `OPENHOOF_LIVE_TEST=1`)
