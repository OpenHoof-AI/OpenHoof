...
This guide summarizes a sane workflow for working on the pi integration in OpenHoof.
...
```bash
OPENHOOF_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm openhoof agent --message "Hello" --thinking low`
...
State lives under the OpenHoof state directory. Default is `~/.openhoof`. If `OPENHOOF_STATE_DIR` is set, use that directory instead.

To reset everything:

- `openhoof.json` for config
