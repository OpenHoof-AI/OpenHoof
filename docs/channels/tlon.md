...
Tlon is a decentralized messenger built on Urbit. OpenHoof connects to your Urbit ship and can
...
```bash
openhoof plugins install @openhoof/tlon
```

Local checkout (when running from a git repo):

```bash
openhoof plugins install ./extensions/tlon
```
...
By default, OpenHoof blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `openhoof message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenHoof replies in-thread.
