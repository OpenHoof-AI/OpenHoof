---
title: IRC
description: Connect OpenHoof to IRC channels and direct messages.
summary: "IRC plugin setup, access controls, and troubleshooting"
read_when:
  - You want to connect OpenHoof to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

# IRC

Use IRC when you want OpenHoof in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.openhoof/openhoof.json`.
2. Set at least:
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, OpenHoof defaults to **mention-gating** in group contexts.
