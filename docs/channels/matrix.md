...
Matrix is an open, decentralized messaging protocol. OpenHoof connects as a Matrix **user**
...
```bash
openhoof plugins install @openhoof/matrix
```

Local checkout (when running from a git repo):

```bash
openhoof plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openhoof plugins install @openhoof/matrix`
   - From a local checkout: `openhoof plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenHoof calls the same
     login endpoint, stores the access token in `~/.openhoof/credentials/matrix/credentials.json`,
...
- On first connection, OpenHoof requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenHoof logs a warning.
...
`~/.openhoof/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openhoof pairing list matrix`
  - `openhoof pairing approve matrix <CODE>`
...
- On startup, OpenHoof resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
