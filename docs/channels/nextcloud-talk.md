...
```bash
openhoof plugins install @openhoof/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openhoof plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenHoof" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openhoof pairing list nextcloud-talk`
  - `openhoof pairing approve nextcloud-talk <CODE>`
