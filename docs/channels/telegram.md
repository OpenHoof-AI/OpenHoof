...
```
    ...
    Telegram does **not** use `openhoof channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `openhoof doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `openhoof doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `openhoof logs --follow`
...
- DM messages can carry `message_thread_id`; OpenHoof routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    OpenHoof can stream partial replies in real time:
    ...
    - DM: OpenHoof keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: OpenHoof keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), OpenHoof falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, OpenHoof skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, OpenHoof automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, OpenHoof retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - OpenHoof pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.openhoof/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, OpenHoof enqueues system events like:
...
```bash
openhoof message send --channel telegram --target 123456789 --message "hi"
openhoof message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `openhoof message poll` and support forum topics:

```bash
openhoof message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
openhoof message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, OpenHoof preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `openhoof channels status` warns when config expects unmentioned group messages.
    - `openhoof channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `openhoof logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, OpenHoof now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `OPENHOOF_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `OPENHOOF_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `OPENHOOF_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `openhoof doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `openhoof doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, OpenHoof falls back to the first normalized account ID and `openhoof doctor` warns.
