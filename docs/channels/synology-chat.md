---
summary: "Synology Chat webhook setup and OpenHoof config"
read_when:
  - Setting up Synology Chat with OpenHoof
...
```bash
openhoof plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `openhoof onboard` now shows Synology Chat in the same channel setup list as `openhoof channels add`.
   - Non-interactive setup: `openhoof channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your OpenHoof gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in OpenHoof.
   - Guided: `openhoof onboard`
   - Direct: `openhoof channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `openhoof pairing list synology-chat`
  - `openhoof pairing approve synology-chat <CODE>`
...
```bash
openhoof message send --channel synology-chat --target 123456 --text "Hello from OpenHoof"
openhoof message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
