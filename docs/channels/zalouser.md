...
read_when:
  - Setting up Zalo Personal for OpenHoof
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside OpenHoof.
...
- Install via CLI: `openhoof plugins install @openhoof/zalouser`
- Or from a source checkout: `openhoof plugins install ./extensions/zalouser`
...
- `openhoof pairing list zalouser`
- `openhoof pairing approve zalouser <code>`
...
- On startup, OpenHoof resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, OpenHoof stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in OpenHoof state. Example:
...
- OpenHoof sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, OpenHoof sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `openhoof channels logout --channel zalouser && openhoof channels login --channel zalouser`
...
- The channel now runs fully in OpenHoof without external CLI binaries.
