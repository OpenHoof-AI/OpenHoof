...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure OpenHoof HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `openhoof pairing approve slack <code>`.
...
- The interactive callback values are OpenHoof-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, OpenHoof falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while OpenHoof is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "OpenHoof",
    "description": "Slack connector for OpenHoof"
  },
  "features": {
    "bot_user": {
      "display_name": "OpenHoof",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/openhoof",
        "description": "Send a message to OpenHoof",
        "should_escape": false
      }
    ]
  },
...
OpenHoof supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, OpenHoof falls back to normal delivery for remaining payloads.
