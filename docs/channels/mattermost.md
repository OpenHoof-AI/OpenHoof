---
summary: "Mattermost bot setup and OpenHoof config"
read_when:
...
```bash
openhoof plugins install @openhoof/mattermost
```

Local checkout (when running from a git repo):

```bash
openhoof plugins install ./extensions/mattermost
```
...
4. Configure OpenHoof and start the gateway.
...
Native slash commands are opt-in. When enabled, OpenHoof registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, OpenHoof derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as OpenHoof.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to OpenHoof.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from OpenHoof, not `404`.
...
- Approve via:
  - `openhoof pairing list mattermost`
  - `openhoof pairing approve mattermost <CODE>`
...
Use these target formats with `openhoof message send` or cron/webhooks:
...
OpenHoof resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), OpenHoof sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When OpenHoof sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, OpenHoof derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and OpenHoof run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="openhoof-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"openhoof-mattermost-interactions",
...
`openhoof message send` and cron/webhook deliveries.
