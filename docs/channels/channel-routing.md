...
OpenHoof routes replies **back to the channel where a message came from**. The
...
OpenHoof infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, OpenHoof still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when OpenHoof would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.openhoof/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.openhoof`):

- `~/.openhoof/agents/<agentId>/sessions/sessions.json`
