...
read_when:
  - You want OpenHoof to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OpenHoof to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`openhoof onboard`) and `openhoof channels add` list optional channel plugins.
...
```bash
openhoof plugins install @openhoof/nostr
```

Use a local checkout (dev workflows):

```bash
openhoof plugins install --link <path-to-openhoof>/extensions/nostr
```
...
openhoof channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "openhoof",
        "displayName": "OpenHoof",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "openhoof@example.com",
        "lud16": "openhoof@example.com"
      }
    }
  }
}
