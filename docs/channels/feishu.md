...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenHoof to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current OpenHoof releases, so no separate plugin install
is required.
...
```bash
openhoof plugins install @openhoof/feishu
```
...
If you just installed OpenHoof, run onboarding:
...
2. Configuring app credentials in OpenHoof
...
1. You already ran `openhoof channels add` for Feishu
2. The gateway is running (`openhoof gateway status`)
...
```bash
openhoof pairing approve feishu <CODE>
```
...
2. Run `openhoof logs --follow` and look for `chat_id`
...
2. Run `openhoof logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `openhoof gateway status`  | Show gateway status           |
| `openhoof gateway install` | Install/start gateway service |
| `openhoof gateway stop`    | Stop gateway service          |
| `openhoof gateway restart` | Restart gateway service       |
| `openhoof logs --follow`   | Tail gateway logs             |
...
4. Check logs: `openhoof logs --follow`
...
5. Ensure the gateway is running: `openhoof gateway status`
6. Check logs: `openhoof logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "hoofd-fan",
        workspace: "/home/user/hoofd-fan",
        agentDir: "/home/user/.openhoof/agents/hoofd-fan/agent",
      },
      {
        id: "hoofd-xi",
        workspace: "/home/user/hoofd-xi",
        agentDir: "/home/user/.openhoof/agents/hoofd-xi/agent",
      },
    ],
  },
  bindings: [
