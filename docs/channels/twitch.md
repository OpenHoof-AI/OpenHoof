...
read_when:
  - Setting up Twitch chat integration for OpenHoof
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenHoof connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openhoof plugins install @openhoof/twitch
```

Local checkout (when running from a git repo):

```bash
openhoof plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENHOOF_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openhoof", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENHOOF_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENHOOF_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
