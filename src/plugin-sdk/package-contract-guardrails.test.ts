...
const PLUGIN_SDK_SUBPATH_PATTERN = /openhoof\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps repo openhoof/plugin-sdk/<name> references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references openhoof/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("does not leave referenced src/plugin-sdk source names stranded outside the public contract", () => {
    ...
    for (const sourceName of collectPluginSdkSourceNames()) {
      ...
      failures.push(
        `src/plugin-sdk/${sourceName}.ts is referenced as openhoof/plugin-sdk/${sourceName} in ${matchingRefs
