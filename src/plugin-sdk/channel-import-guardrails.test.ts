...
const SAME_CHANNEL_SDK_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']openhoof\/plugin-sdk\/discord["']/, /plugin-sdk-internal\/discord/],
  },
  {
    path: "extensions/slack/src/shared.ts",
    forbiddenPatterns: [/["']openhoof\/plugin-sdk\/slack["']/, /plugin-sdk-internal\/slack/],
  },
  {
    path: "extensions/telegram/src/shared.ts",
    forbiddenPatterns: [/["']openhoof\/plugin-sdk\/telegram["']/, /plugin-sdk-internal\/telegram/],
  },
  {
    path: "extensions/imessage/src/shared.ts",
    forbiddenPatterns: [/["']openhoof\/plugin-sdk\/imessage["']/, /plugin-sdk-internal\/imessage/],
  },
  {
    path: "extensions/whatsapp/src/shared.ts",
    forbiddenPatterns: [/["']openhoof\/plugin-sdk\/whatsapp["']/, /plugin-sdk-internal\/whatsapp/],
  },
  {
    path: "extensions/signal/src/shared.ts",
    forbiddenPatterns: [/["']openhoof\/plugin-sdk\/signal["']/, /plugin-sdk-internal\/signal/],
  },
];
...
const LOCAL_EXTENSION_API_BARREL_EXCEPTIONS = [
  ...
  // accounts.ts -> runtime-api.ts -> openhoof/plugin-sdk/matrix -> extensions/matrix/api.ts -> accounts.ts
...
describe("channel import guardrails", () => {
  ...
  it("keeps bundled extension source files off root and compat plugin-sdk imports", () => {
    for (const file of collectExtensionSourceFiles()) {
      ...
      expect(text, `${file} should not import openhoof/plugin-sdk root`).not.toMatch(
        /["']openhoof\/plugin-sdk["']/,
      );
      expect(text, `${file} should not import openhoof/plugin-sdk/compat`).not.toMatch(
        /["']openhoof\/plugin-sdk\/compat["']/,
      );
    }
  });
  ...
  it("keeps internalized extension helper surfaces behind local api barrels", () => {
    for (const extensionId of LOCAL_EXTENSION_API_BARREL_GUARDS) {
      for (const file of collectExtensionFiles(extensionId)) {
        ...
        ).not.toMatch(new RegExp(`["']openhoof/plugin-sdk/${extensionId}["']`, "u"));
      }
    }
  });
});
