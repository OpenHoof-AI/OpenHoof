...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: OpenHoofConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: OpenHoofConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/openhoof-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<OpenHoofConfig["bindings"]>[number];
