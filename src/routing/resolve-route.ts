...
function listAgents(cfg: OpenHoofConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: OpenHoofConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<OpenHoofConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: OpenHoofConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: OpenHoofConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OpenHoofConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<OpenHoofConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  OpenHoofConfig,
  {
    bindingsRef: OpenHoofConfig["bindings"];
    agentsRef: OpenHoofConfig["agents"];
    sessionRef: OpenHoofConfig["session"];
...
function resolveRouteCacheForConfig(cfg: OpenHoofConfig): Map<string, ResolvedAgentRoute> {
