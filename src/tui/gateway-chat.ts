...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set OPENHOOF_GATEWAY_TOKEN/OPENHOOF_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "openhoof-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.OPENHOOF_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.OPENHOOF_GATEWAY_PASSWORD);
