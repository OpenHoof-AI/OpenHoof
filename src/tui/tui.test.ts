...
describe("resolveInitialTuiAgentId", () => {
  const cfg: OpenHoofConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/openhoof" },
        { id: "ops", workspace: "/tmp/openhoof/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/openhoof/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run openhoof devices list");
    expect(state.pairingHint).toContain("openhoof devices list");
  });
