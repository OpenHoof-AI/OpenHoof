...
declare const __OPENHOOF_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "openhoof";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["OPENHOOF_VERSION"],
      runtimeVersion,
      env["OPENHOOF_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current OpenHoof version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __OPENHOOF_VERSION__ === "string" ? __OPENHOOF_VERSION__ : undefined,
  bundledVersion: process.env.OPENHOOF_BUNDLED_VERSION,
});
