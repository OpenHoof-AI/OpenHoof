...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "openhoof-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.OPENHOOF_BUNDLED_HOOKS_DIR;
    process.env.OPENHOOF_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.OPENHOOF_BUNDLED_HOOKS_DIR;
    } else {
      process.env.OPENHOOF_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"openhoof":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): OpenHoofConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("openhoof-plugin");
  ...
  it("does not treat Claude hooks.json bundles as OpenHoof hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".openhoof", "extensions", "claude-bundle");
