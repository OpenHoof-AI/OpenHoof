...
const fixtureRoot = path.join(os.tmpdir(), `openhoof-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@openhoof/test-hooks",
      version: "0.0.1",
      openhoof: { hooks: params.hooks },
...
describe("installHooksFromPath", () => {
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expectPathInstallFailureContains(result, "openhoof.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expectPathInstallFailureContains(
      result,
      "openhoof.hooks entry resolves outside package directory",
    );
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@openhoof/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@openhoof/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@openhoof/test-hooks@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@openhoof/test-hooks@0.0.2-beta.1",
      name: "@openhoof/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@openhoof/test-hooks@beta"');
    }
  });
});
