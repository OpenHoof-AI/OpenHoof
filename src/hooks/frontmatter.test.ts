...
`;
    ...
    expect(parsed.openhoof.emoji).toBe("💾");
    expect(parsed.openhoof.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openhoof.emoji).toBe("📝");
    expect(parsed.openhoof.events).toEqual(["command"]);
    expect(parsed.openhoof.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openhoof.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openhoof": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openhoof": {"events": ["test"]}}');
  });
...
describe("resolveOpenHoofMetadata", () => {
  it("extracts openhoof metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenHoofMetadata(frontmatter);
  ...
  it("returns undefined when openhoof key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openhoof: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenHoof" },
            { id: "npm", kind: "npm", package: "@openhoof/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openhoof/hook");
  });
...
`;
    ...
    const openhoof = resolveOpenHoofMetadata(frontmatter);
    expect(openhoof).toBeDefined();
    expect(openhoof?.emoji).toBe("💾");
    expect(openhoof?.events).toEqual(["command:new", "command:reset"]);
    expect(openhoof?.requires?.config).toEqual(["workspace.dir"]);
    expect(openhoof?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openhoof = resolveOpenHoofMetadata(frontmatter);
    expect(openhoof?.emoji).toBe("disk");
    expect(openhoof?.events).toEqual(["command:new"]);
  });
});
