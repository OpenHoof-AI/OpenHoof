...
homepage: https://docs.openhoof.ai/automation/hooks#command-logger
metadata:
  {
    "openhoof":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.openhoof/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.openhoof/logs/commands.log ~/.openhoof/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openhoof`:
   ```
   /home/username/.openhoof/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
