...
This directory contains hooks that ship with OpenHoof. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.openhoof/openhoof.json`:
...
2. Restart gateway: `pkill -9 -f 'openhoof.*gateway' && pnpm openhoof gateway`
3. Enable the hook: `openhoof hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openhoof.ai/automation/hooks
