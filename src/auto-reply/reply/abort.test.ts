...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop openhoof!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@openhoof_bot", { botUsername: "openhoof_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@openhoof_bot", { botUsername: "openhoof_bot" })).toBe(true);
