...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: OpenHoofConfig) => ({ mode: "final" })),
  };
});
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: OpenHoofConfig) => ttsMocks.resolveTtsConfig(cfg),
}));

const noAbortResult = { handled: false, aborted: false } as const;
const emptyConfig = {} as OpenHoofConfig;
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as OpenHoofConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "openhoof-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginId: "openhoof-codex-app-server",
        pluginRoot: "/Users/huntharo/github/openhoof-app-server",
      },
  ...
  it("falls back to OpenHoof once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(async () => ({ text: "openhoof fallback" }) satisfies ReplyPayload);
    ...
    const firstNotice = (firstDispatcher.sendToolResult as ReturnType<typeof vi.fn>).mock
      .calls[0]?.[0] as ReplyPayload | undefined;
    expect(firstNotice?.text).toContain("Routing this message to OpenHoof instead.");
  ...
  it("falls back to OpenHoof when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const notice = (dispatcher.sendToolResult as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as
      | ReplyPayload
      | undefined;
    expect(notice?.text).toContain("Routing this message to OpenHoof instead.");
    expect(replyResolver).toHaveBeenCalledTimes(1);
    expect(hookMocks.runner.runInboundClaim).not.toHaveBeenCalled();
  });
