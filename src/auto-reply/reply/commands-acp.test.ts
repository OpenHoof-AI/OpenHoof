...
function createDiscordParams(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

function createTelegramTopicParams(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "telegram",
    Surface: "telegram",
...
function createTelegramDmParams(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "telegram",
    Surface: "telegram",
    OriginatingChannel: "telegram",
...
async function runDiscordAcpCommand(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
  return handleAcpCommand(createTelegramTopicParams(commandBody, cfg), true);
}

async function runTelegramDmAcpCommand(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
  return handleAcpCommand(createTelegramDmParams(commandBody, cfg), true);
}

function createFeishuDmParams(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "feishu",
    Surface: "feishu",
...
async function runFeishuDmAcpCommand(commandBody: string, cfg: OpenHoofConfig = baseCfg) {
...
describe("/acp command", () => {
  ...
  it("spawns an ACP session and binds a Discord thread", async () => {
    ...
    const result = await runDiscordAcpCommand("/acp spawn codex --cwd /home/bob/hoofd");
    ...
    expect(hoisted.sessionBindingBindMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        metadata: expect.objectContaining({
          introText: expect.stringContaining("cwd: /home/bob/hoofd"),
        }),
      }),
    );
  ...
  it("accepts unicode dash option prefixes in /acp spawn args", async () => {
    const result = await runThreadAcpCommand(
      "/acp spawn codex \u2014mode oneshot \u2014thread here \u2014cwd /home/bob/hoofd \u2014label jeerreview",
    );
