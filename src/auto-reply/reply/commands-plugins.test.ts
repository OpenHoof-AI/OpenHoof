...
const workspaceHarness = createCommandWorkspaceHarness("openhoof-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".openhoof", "extensions", params.pluginId);
...
function buildCfg(): OpenHoofConfig {
  return {
    commands: {
      text: true,
      plugins: true,
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("openhoof-command-plugins-home-", async () => {
