...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-commands-"));
...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-commands-config-"));
  const configPath = path.join(dir, "openhoof.json");
  const previous = process.env.OPENHOOF_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENHOOF_CONFIG_PATH = previous;
    }
...
function buildParams(commandBody: string, cfg: OpenHoofConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/openhoof-session-store.json" },
    } as OpenHoofConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/openhoof-agent-compact";
      ...
      },
      {
        ...
        run: async () => {
          await withTempConfigPath({ messages: { ackReaction: ":)" } }, async (configPath) => {
            ...
            const written = await readJsonFile<OpenHoofConfig>(configPath);
