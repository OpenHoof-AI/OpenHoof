...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openhoofHome: process.env.OPENHOOF_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("OPENHOOF_HOME", snapshot.openhoofHome);
  restoreKey("OPENHOOF_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPENHOOF_STATE_DIR = join(home, ".openhoof");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "openhoof-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".openhoof", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): OpenHoofConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "openhoof"),
