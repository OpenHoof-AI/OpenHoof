...
describe("pairing setup code", () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    vi.stubEnv("OPENHOOF_GATEWAY_TOKEN", "");
    vi.stubEnv("HOOFBOT_GATEWAY_TOKEN", "");
    vi.stubEnv("OPENHOOF_GATEWAY_PASSWORD", "");
    vi.stubEnv("HOOFBOT_GATEWAY_PASSWORD", "");
    vi.stubEnv("OPENHOOF_GATEWAY_PORT", "");
    vi.stubEnv("HOOFBOT_GATEWAY_PORT", "");
  });
  ...
  it("honors env token override", async () => {
    const resolved = await resolvePairingSetupFromConfig(
      ...
      {
        env: {
          OPENHOOF_GATEWAY_TOKEN: "new-token",
        },
      },
    );
