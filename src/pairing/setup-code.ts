...
function resolveGatewayTokenFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return env.OPENHOOF_GATEWAY_TOKEN?.trim() || env.HOOFBOT_GATEWAY_TOKEN?.trim() || undefined;
}

function resolveGatewayPasswordFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    env.OPENHOOF_GATEWAY_PASSWORD?.trim() || env.HOOFBOT_GATEWAY_PASSWORD?.trim() || undefined
  );
}
...
async function resolveGatewayTokenSecretRef(
  ...
  if (mode !== "token") {
    const hasPasswordEnvCandidate = Boolean(
      env.OPENHOOF_GATEWAY_PASSWORD?.trim() || env.HOOFBOT_GATEWAY_PASSWORD?.trim(),
    );
