...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.openhoof.ai/gateway/health",
          "https://docs.openhoof.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.openhoof.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.openhoof/openhoof.json (gateway.auth.token) or OPENHOOF_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("openhoof config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("openhoof doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("openhoof dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control OpenHoof."
            : "Copy/paste this URL in a browser on this machine to control OpenHoof.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("openhoof dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.openhoof.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.openhoof.ai/security",
    "Security",
  );
    ...
    if (webSearchEnabled !== false && hasKey) {
      await prompter.note(
        [
          ...
          `Provider: ${label}`,
          ...(keySource ? [keySource] : []),
          "Docs: https://docs.openhoof.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
    } else if (!hasKey) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("openhoof configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.openhoof.ai/tools/web"}`,
          "Docs: https://docs.openhoof.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("openhoof configure --section web")}`,
          "",
          "Docs: https://docs.openhoof.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://openhoof.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control OpenHoof."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control OpenHoof.",
  );
