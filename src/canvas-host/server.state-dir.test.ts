...
describe("canvas host state dir defaults", () => {
  it("uses OPENHOOF_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("openhoof-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OpenHoof Canvas");
      } finally {
        await handler.close();
      }
    });
