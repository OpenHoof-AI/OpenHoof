...
import { listTelegramAccountIds } from "openhoof/plugin-sdk/telegram";
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set OPENHOOF_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.OPENHOOF_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
