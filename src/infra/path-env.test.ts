...
let ensureOpenHoofCliOnPath: typeof import("./path-env.js").ensureOpenHoofCliOnPath;

describe("ensureOpenHoofCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPENHOOF_PATH_BOOTSTRAPPED",
    "OPENHOOF_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOpenHoofCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/openhoof-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openhoof");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureOpenHoofCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling openhoof exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.OPENHOOF_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.OPENHOOF_PATH_BOOTSTRAPPED = "1";
    ensureOpenHoofCliOnPath({
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "openhoof");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.OPENHOOF_PATH_BOOTSTRAPPED;
      delete process.env.OPENHOOF_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.OPENHOOF_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/openhoof-path/case-linuxbrew");
