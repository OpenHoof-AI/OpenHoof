...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/openhoof-state", "devices")).toEqual({
      dir: path.join("/tmp/openhoof-state", "devices"),
      pendingPath: path.join("/tmp/openhoof-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/openhoof-state", "devices", "paired.json"),
    });
  });
