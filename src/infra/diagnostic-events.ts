...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __openhoofDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__openhoofDiagnosticEventsState) {
    globalStore.__openhoofDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__openhoofDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OpenHoofConfig): boolean {
