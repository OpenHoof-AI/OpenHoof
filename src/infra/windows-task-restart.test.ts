...
const resolvePreferredOpenHoofTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./tmp-openhoof-dir.js", () => ({
  resolvePreferredOpenHoofTmpDir: () => resolvePreferredOpenHoofTmpDirMock(),
}));
...
afterEach(() => {
  envSnapshot.restore();
  spawnMock.mockReset();
  resolvePreferredOpenHoofTmpDirMock.mockReset();
  resolvePreferredOpenHoofTmpDirMock.mockReturnValue(os.tmpdir());
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ OPENHOOF_PROFILE: "work" });

    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "OpenHoof Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "OpenHoof Gateway (work)" >nul 2>&1');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers OPENHOOF_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    relaunchGatewayScheduledTask({
      OPENHOOF_PROFILE: "work",
      OPENHOOF_WINDOWS_TASK_NAME: "OpenHoof Gateway (custom)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "OpenHoof Gateway (custom)" >nul 2>&1');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredOpenHoofTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ OPENHOOF_PROFILE: "work" });
