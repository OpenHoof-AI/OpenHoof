 ...
 * returning control to the caller (typically `triggerOpenHoofRestart` →
...
/**
 * Parse openhoof gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("openhoof")) {
 ...
 * Returns only PIDs that belong to openhoof gateway processes (not the current process).
 */
