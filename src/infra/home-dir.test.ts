...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers OPENHOOF_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for OPENHOOF_HOME", () => {
    const result = resolveRequiredHomeDir(
      { OPENHOOF_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when OPENHOOF_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ OPENHOOF_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { OPENHOOF_HOME: "/srv/openhoof-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/openhoof-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/openhoof-home " },
      expected: "/srv/openhoof-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/openhoof-home/docs"));
  });
