...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openhoof-state-dir-"));
  tempRoot = root;
  return root;
}
...
describe("legacy state dir auto-migration", () => {
  it("follows legacy symlink when it points at another legacy dir (hoofbot -> moltbot)", async () => {
    const root = await makeTempRoot();
    const legacySymlink = path.join(root, ".hoofbot");
    ...
    const targetMarker = path.join(root, ".openhoof", "marker.txt");
    expect(fs.readFileSync(targetMarker, "utf-8")).toBe("ok");
    expect(fs.readFileSync(path.join(root, ".moltbot", "marker.txt"), "utf-8")).toBe("ok");
    expect(fs.readFileSync(path.join(root, ".hoofbot", "marker.txt"), "utf-8")).toBe("ok");
  });

  it("skips state-dir migration when OPENHOOF_STATE_DIR is explicitly set", async () => {
    const root = await makeTempRoot();
    const legacyDir = path.join(root, ".hoofbot");
    fs.mkdirSync(legacyDir, { recursive: true });

    const result = await autoMigrateLegacyStateDir({
      env: { OPENHOOF_STATE_DIR: path.join(root, "custom-state") } as NodeJS.ProcessEnv,
      homedir: () => root,
    });
