...
vi.mock("./openhoof-root.js", () => ({
  resolveOpenHoofPackageRoot: vi.fn(async () => null),
  resolveOpenHoofPackageRootSync: vi.fn(() => null),
}));
...
let openhoofRoot: typeof import("./openhoof-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openhoofRoot = await import("./openhoof-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/openhoof");
    const wrapperArgv1 = abs("fixtures/bin/openhoof");
  ...
  it("uses resolveOpenHoofPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openhoof");
    (
      openhoofRoot.resolveOpenHoofPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/openhoof"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "openhoof" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "openhoof.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-openhoof");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/openhoof-bundle");
    (
      openhoofRoot.resolveOpenHoofPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/openhoof-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
