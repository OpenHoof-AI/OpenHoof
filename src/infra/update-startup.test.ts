...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OPENHOOF_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.OPENHOOF_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOpenHoofPackageRoot } = await import("./openhoof-root.js"));
    ...
    vi.mocked(resolveOpenHoofPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveOpenHoofPackageRoot).mockResolvedValue("/opt/openhoof");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/openhoof/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
