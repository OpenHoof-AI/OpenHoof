...
const DEFAULT_PACKAGE_NAME = "openhoof";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["openhoof", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "openhoof.mjs");
    ...
    const doctorStep = await runStep(
      step("openhoof doctor", doctorArgv, gitRoot, { OPENHOOF_UPDATE_IN_PROGRESS: "1" }),
    );
