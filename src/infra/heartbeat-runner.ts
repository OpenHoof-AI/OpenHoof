...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OpenHoofConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OpenHoofConfig) {
...
function resolveHeartbeatAgents(cfg: OpenHoofConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OpenHoofConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OpenHoofConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OpenHoofConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
