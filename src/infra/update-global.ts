...
const PRIMARY_PACKAGE_NAME = "openhoof";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const OPENHOOF_MAIN_PACKAGE_SPEC = "github:openhoof/openhoof#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "OpenHoof", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.OPENHOOF_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.OPENHOOF_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return OPENHOOF_MAIN_PACKAGE_SPEC;
  }
