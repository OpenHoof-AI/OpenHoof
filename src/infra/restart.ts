...
export function triggerOpenHoofRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.OPENHOOF_SYSTEMD_UNIT,
      process.env.OPENHOOF_PROFILE,
    );
  ...
  const label =
    process.env.OPENHOOF_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENHOOF_PROFILE);
