...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.OPENHOOF_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for OpenHoof. You can keep running the web gateway without it: \`${formatCliCommand("openhoof gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
