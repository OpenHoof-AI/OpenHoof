...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "openhoof.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (OpenHoof)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "openhoof.internal", expected: "openhoof.internal." },
    { value: "openhoof.internal.", expected: "openhoof.internal." },
    { value: "  openhoof.internal  ", expected: "openhoof.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { OPENHOOF_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { OPENHOOF_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("openhoof.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "openhoof.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      instanceLabel: "studio-london",
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/openhoof",
    });

    expect(txt).toContain(`$ORIGIN openhoof.internal.`);
    ...
    expect(txt).toContain(`_openhoof-gw._tcp IN PTR studio-london._openhoof-gw._tcp`);
    expect(txt).toContain(`studio-london._openhoof-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (OpenHoof)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/openhoof`);
  });
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN openhoof.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._openhoof-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (OpenHoof)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("openhoof.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("openhoof.internal."),
