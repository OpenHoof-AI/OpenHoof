...
const GATEWAY_DEVICE_ID_HEADER = "x-openhoof-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-openhoof-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-openhoof-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.OPENHOOF_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "OPENHOOF_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or OPENHOOF_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.OPENHOOF_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require OPENHOOF_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.OPENHOOF_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
