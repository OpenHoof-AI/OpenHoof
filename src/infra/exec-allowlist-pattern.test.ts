...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevOpenHoofHome = process.env.OPENHOOF_HOME;
    const prevHome = process.env.HOME;
    process.env.OPENHOOF_HOME = "/srv/openhoof-home";
    process.env.HOME = "/home/other";
    const openHoofHome = path.join(path.resolve("/srv/openhoof-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", openHoofHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevOpenHoofHome === undefined) {
        delete process.env.OPENHOOF_HOME;
      } else {
        process.env.OPENHOOF_HOME = prevOpenHoofHome;
      }
