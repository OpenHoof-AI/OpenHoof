...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.OPENHOOF_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.OPENHOOF_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.OPENHOOF_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OPENHOOF_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.OPENHOOF_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OPENHOOF_APNS_PRIVATE_KEY_P8 or OPENHOOF_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OPENHOOF_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
