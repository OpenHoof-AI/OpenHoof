...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENHOOF_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "OpenHoof";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(OpenHoof\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.OPENHOOF_MDNS_HOSTNAME?.trim() ||
    process.env.HOOFBOT_MDNS_HOSTNAME?.trim() ||
    "openhoof";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
