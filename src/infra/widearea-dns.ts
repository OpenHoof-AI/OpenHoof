...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENHOOF_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openhoof-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openhoof";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openhoof");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openhoof-gw");
  ...
  records.push(`_openhoof-gw._tcp IN PTR ${instanceLabel}._openhoof-gw._tcp`);
  records.push(`${instanceLabel}._openhoof-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openhoof-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openhoof-content-hash: ${contentHash}\n${contentBody}`;
}
