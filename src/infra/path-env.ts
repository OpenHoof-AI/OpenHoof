...
function candidateBinDirs(opts: EnsureOpenHoofPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `openhoof` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openhoof");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPENHOOF_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "openhoof"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openhoof` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenHoofCliOnPath(opts: EnsureOpenHoofPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENHOOF_PATH_BOOTSTRAPPED)) {
    return;
  }
