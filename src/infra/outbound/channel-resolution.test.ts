...
const loadOpenHoofPluginsMock = vi.hoisted(() => vi.fn());
...
vi.mock("../../plugins/loader.js", () => ({
  loadOpenHoofPlugins: (...args: unknown[]) => loadOpenHoofPluginsMock(...args),
}));
...
describe("outbound channel resolution", () => {
  beforeEach(() => {
    ...
    loadOpenHoofPluginsMock.mockReset();
  ...
  it("returns the already-registered plugin without bootstrapping", async () => {
    ...
    expect(loadOpenHoofPluginsMock).not.toHaveBeenCalled();
  });
  ...
  it("bootstraps plugins once per registry key and returns the newly loaded plugin", async () => {
    ...
    expect(loadOpenHoofPluginsMock).toHaveBeenCalledWith({
    ...
    expect(loadOpenHoofPluginsMock).toHaveBeenCalledTimes(1);
    expect(loadOpenHoofPluginsMock).toHaveBeenLastCalledWith({
      config: { autoEnabled: true },
      workspaceDir: "/tmp/workspace",
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
  ...
  it("retries bootstrap after a transient load failure", async () => {
    getChannelPluginMock.mockReturnValue(undefined);
    loadOpenHoofPluginsMock.mockImplementationOnce(() => {
    ...
    expect(loadOpenHoofPluginsMock).toHaveBeenCalledTimes(2);
  });
});
