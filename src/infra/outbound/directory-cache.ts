...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OpenHoofConfig | null = null;
  ...
  get(key: string, cfg: OpenHoofConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OpenHoofConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OpenHoofConfig): void {
