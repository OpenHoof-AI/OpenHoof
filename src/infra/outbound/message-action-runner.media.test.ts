...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred OpenHoof tmp root", async () => {
      const tmpRoot = resolvePreferredOpenHoofTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideOpenHoof = path.join(os.tmpdir(), "outside-openhoof", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
