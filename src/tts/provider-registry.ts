...
function resolveSpeechProviderPluginEntries(cfg?: OpenHoofConfig): SpeechProviderPlugin[] {
  const active = getActivePluginRegistry();
  const registry =
    (active?.speechProviders?.length ?? 0) > 0 || !cfg
      ? active
      : loadOpenHoofPlugins({ config: cfg });
  return registry?.speechProviders?.map((entry) => entry.provider) ?? [];
}

function buildProviderMaps(cfg?: OpenHoofConfig): {
...
export function listSpeechProviders(cfg?: OpenHoofConfig): SpeechProviderPlugin[] {
