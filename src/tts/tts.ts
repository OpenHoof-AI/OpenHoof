...
export function resolveTtsConfig(cfg: OpenHoofConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.OPENHOOF_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: OpenHoofConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: OpenHoofConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      const tempRoot = resolvePreferredOpenHoofTmpDir();
