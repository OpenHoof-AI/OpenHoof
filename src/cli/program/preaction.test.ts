...
beforeEach(() => {
  ...
  originalHideBanner = process.env.OPENHOOF_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.OPENHOOF_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.OPENHOOF_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("openhoof");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "openhoof", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("openhoof-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "openhoof", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "openhoof", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "openhoof", "channels", "add"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "openhoof", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.OPENHOOF_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "openhoof", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "openhoof", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "openhoof", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "openhoof", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "openhoof", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "openhoof", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "openhoof", "backup", "create", "--json"],
    });
