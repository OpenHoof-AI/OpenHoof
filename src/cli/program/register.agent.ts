...
${formatHelpExamples([
  ['openhoof agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openhoof agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openhoof agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openhoof agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openhoof agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openhoof agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openhoof.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openhoof.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openhoof agents set-identity --agent main --name "OpenHoof" --emoji "🐴"', "Set name + emoji."],
  ["openhoof agents set-identity --agent main --avatar avatars/openhoof.png", "Set avatar path."],
  [
    "openhoof agents set-identity --workspace ~/.openhoof/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openhoof agents set-identity --identity-file ~/.openhoof/workspace/IDENTITY.md --agent main",
