...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OpenHoof and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openhoof update", "Update a source checkout (git)"],
        ["openhoof update --channel beta", "Switch to beta channel (git + npm)"],
        ["openhoof update --channel dev", "Switch to dev channel (git + npm)"],
        ["openhoof update --tag beta", "One-off update to a dist-tag or version"],
        ["openhoof update --tag main", "One-off package install from GitHub main"],
        ["openhoof update --dry-run", "Preview actions without changing anything"],
        ["openhoof update --no-restart", "Update without restarting the service"],
        ["openhoof update --json", "Output result as JSON"],
        ["openhoof update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openhoof update wizard", "Interactive update wizard"],
        ["openhoof --update", "Shorthand for openhoof update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openhoof update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openhoof.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openhoof update status", "Show channel + version status."],
          ["openhoof update status --json", "JSON output."],
          ["openhoof update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openhoof.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
