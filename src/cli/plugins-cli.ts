...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "openhoof";
...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: OpenHoofConfig; warnings: string[] } {
...
async function runPluginInstallCommand(params: {
  ...
  if (fs.existsSync(resolved)) {
    if (opts.link) {
      ...
      let next: OpenHoofConfig = enablePluginInConfig(
        {
          ...cfg,
          plugins: {
            ...cfg.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage OpenHoof plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.openhoof.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "openhoof"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: OpenHoofConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.openhoof.ai/plugin");
      lines.push("");
