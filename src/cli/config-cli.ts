...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "openhoof config set secrets.providers.vault --provider-source file --provider-path /etc/openhoof/secrets.json --provider-mode json",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("openhoof doctor")}\` ${message}`;
}
...
function collectDryRunSchemaErrors(config: OpenHoofConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as OpenHoofConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "openhoof.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openhoof.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing openhoof.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
