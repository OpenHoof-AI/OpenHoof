...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-bundled-skills-test-"));
    process.env.OPENHOOF_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
