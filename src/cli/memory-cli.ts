    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openhoof memory status", "Show index and provider status."],
          ["openhoof memory status --deep", "Probe embedding provider readiness."],
          ["openhoof memory index --force", "Force a full reindex."],
          ['openhoof memory search "meeting notes"', "Quick search using positional query."],
          [
            'openhoof memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["openhoof memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.openhoof.ai/cli/memory")}\n`,
    );
