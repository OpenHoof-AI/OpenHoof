...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openhoof security audit", "Run a local security audit."],
          ["openhoof security audit --deep", "Include best-effort live Gateway probe checks."],
          ["openhoof security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "openhoof security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["openhoof security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["openhoof security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.openhoof.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("OpenHoof security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("openhoof security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("openhoof security audit --fix")}`));
        if (!fixResult) {
