...
import type { OpenHoofConfig, ConfigFileSnapshot } from "../config/types.openhoof.js";
...
vi.mock("../infra/openhoof-root.js", () => ({
  resolveOpenHoofPackageRoot: vi.fn(),
}));
...
const { resolveOpenHoofPackageRoot } = await import("../infra/openhoof-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/openhoof-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OpenHoofConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/openhoof-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOpenHoofPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("openhoof-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("openhoof-updated-root");
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveOpenHoofPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("openhoof");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/openhoof-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "openhoof-gateway" }],
      hints: [],
    });
    ...
    const cases = [
      {
        ...
        assert: () => {
          ...
          expect(logs.join("\n")).toContain("OpenHoof update status");
        },
      },
      {
        name: "json output",
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OpenHoofConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "openhoof@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("openhoof-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "openhoof@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:openhoof/openhoof#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("openhoof-update"));
          await updateCommand({ yes: true, tag: "github:openhoof/openhoof#main" });
        },
        expectedSpec: "github:openhoof/openhoof#main",
      },
      {
        name: "OPENHOOF_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("openhoof-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/openhoof-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("openhoof");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("openhoof-update");
    const localAppData = createCaseDir("openhoof-localappdata");
  ...
  it.each([
    ...
    },
    {
      ...
      invoke: async () => {
        await withEnvAsync(
          {
            OPENHOOF_STATE_DIR: "./state",
            OPENHOOF_CONFIG_PATH: "./config/openhoof.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            OPENHOOF_STATE_DIR: path.resolve("./state"),
            OPENHOOF_CONFIG_PATH: path.resolve("./config/openhoof.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            OPENHOOF_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OPENHOOF_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.OPENHOOF_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("openhoof-update-wizard");
    await withEnvAsync({ OPENHOOF_GIT_DIR: tempDir }, async () => {
      setTty(true);
