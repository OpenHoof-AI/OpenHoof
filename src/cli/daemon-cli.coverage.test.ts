...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      OPENHOOF_GATEWAY_PORT: String(params.port),
      ...(params.token ? { OPENHOOF_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENHOOF_STATE_DIR = "/tmp/openhoof-cli-state";
    process.env.OPENHOOF_CONFIG_PATH = "/tmp/openhoof-cli-state/openhoof.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        OPENHOOF_STATE_DIR: "/tmp/openhoof-daemon-state",
        OPENHOOF_CONFIG_PATH: "/tmp/openhoof-daemon-state/openhoof.json",
        OPENHOOF_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.openhoof.gateway.plist",
    });
