...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("openhoof gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.OPENHOOF_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.OPENHOOF_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("openhoof gateway install", env),
    startCommand: formatCliCommand("openhoof gateway", env),
