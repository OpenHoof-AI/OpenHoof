...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("openhoof-daemon-install-int-");
    configPath = path.join(tempHome, "openhoof.json");
    process.env.HOME = tempHome;
    process.env.OPENHOOF_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.OPENHOOF_GATEWAY_TOKEN).toBeUndefined();
  });
});
