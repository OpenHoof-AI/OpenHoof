...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPENHOOF_STATE_DIR: "/tmp/openhoof-daemon",
    OPENHOOF_CONFIG_PATH: "/tmp/openhoof-daemon/openhoof.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPENHOOF_STATE_DIR ?? "/tmp/openhoof-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPENHOOF_CONFIG_PATH ?? `${stateDir}/openhoof.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/openhoof-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENHOOF_STATE_DIR = "/tmp/openhoof-cli";
    process.env.OPENHOOF_CONFIG_PATH = "/tmp/openhoof-cli/openhoof.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.OPENHOOF_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.OPENHOOF_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.OPENHOOF_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "openhoof-gateway" }],
        hints: [],
      },
      healthy: false,
