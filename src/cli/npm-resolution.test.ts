...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@openhoof/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@openhoof/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@openhoof/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@openhoof/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @openhoof/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@openhoof/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@openhoof/plugin-alpha@1.2.3",
        installPath: "/tmp/openhoof/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@openhoof/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @openhoof/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@openhoof/plugin-alpha@latest",
      true,
      "/tmp/openhoof/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@openhoof/plugin-alpha@latest",
      installPath: "/tmp/openhoof/extensions/alpha",
      version: "1.2.3",
