...
const STEP_LABELS: Record<string, string> = {
  ...
  "openhoof doctor entry": "Checking doctor entrypoint",
  "openhoof doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g openhoof@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g openhoof@latest --omit=optional");
  }
