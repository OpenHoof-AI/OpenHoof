...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.openhoof.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OpenHoof hooks")
    ...
    .option("--hook-url <url>", "OpenHoof hook URL")
    .option("--hook-token <token>", "OpenHoof hook token")
