...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# OpenHoof Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(openhoof completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# OpenHoof Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(openhoof completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.openhoof.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $OPENHOOF_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "openhoof") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
