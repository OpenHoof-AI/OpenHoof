...
function appendHoofHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("openhoof skills list")}\` to see all skills.`
      : "No skills found.";
    return appendHoofHubHint(message, opts.json);
  }
  ...
  return appendHoofHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendHoofHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("openhoof skills list")}\` to see available skills.`,
      opts.json,
    );
  }
