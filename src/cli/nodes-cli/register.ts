    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openhoof nodes status", "List known nodes with live status."],
          ["openhoof nodes pairing pending", "Show pending node pairing requests."],
          ['openhoof nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["openhoof nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.openhoof.ai/cli/nodes")}\n`,
    );
