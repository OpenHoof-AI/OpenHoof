    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "openhoof node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["openhoof node status", "Check node host service status."],
          ["openhoof node install", "Install the node host service."],
          ["openhoof node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.openhoof.ai/cli/node")}\n`,
    );
