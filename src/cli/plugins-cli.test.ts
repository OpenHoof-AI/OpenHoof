...
const loadConfig = vi.fn<() => OpenHoofConfig>(() => ({}) as OpenHoofConfig);
const writeConfigFile = vi.fn<(config: OpenHoofConfig) => Promise<void>>(async () => undefined);
const resolveStateDir = vi.fn(() => "/tmp/openhoof-state");
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...
    writeConfigFile: (config: OpenHoofConfig) => writeConfigFile(config),
  };
});
...
describe("plugins cli", () => {
  ...
  beforeEach(() => {
    ...
    loadConfig.mockReturnValue({} as OpenHoofConfig);
    writeConfigFile.mockResolvedValue(undefined);
    resolveStateDir.mockReturnValue("/tmp/openhoof-state");
    resolveMarketplaceInstallShortcut.mockResolvedValue(null);
    installPluginFromMarketplace.mockResolvedValue({
      ok: false,
      error: "marketplace install failed",
    });
    enablePluginInConfig.mockImplementation((cfg: OpenHoofConfig) => ({ config: cfg }));
    recordPluginInstall.mockImplementation((cfg: OpenHoofConfig) => cfg);
    buildPluginStatusReport.mockReturnValue({
      plugins: [],
      diagnostics: [],
    });
    applyExclusiveSlotSelection.mockImplementation(({ config }: { config: OpenHoofConfig }) => ({
      config,
      warnings: [],
    }));
    uninstallPlugin.mockResolvedValue({
      ok: true,
      config: {} as OpenHoofConfig,
      warnings: [],
      actions: {
        entry: false,
        install: false,
  ...
  it("installs marketplace plugins and persists config", async () => {
    ...
    const installedCfg = {
      ...
      plugins: {
        ...
        installs: {
          alpha: {
            source: "marketplace",
            installPath: "/tmp/openhoof-state/extensions/alpha",
          },
        },
      },
    } as OpenHoofConfig;
    ...
    installPluginFromMarketplace.mockResolvedValue({
      ok: true,
      pluginId: "alpha",
      targetDir: "/tmp/openhoof-state/extensions/alpha",
      version: "1.2.3",
    ...
    loadConfig.mockReturnValue({
      plugins: {
        ...
        installs: {
          alpha: {
            source: "path",
            sourcePath: "/tmp/openhoof-state/extensions/alpha",
            installPath: "/tmp/openhoof-state/extensions/alpha",
          },
        },
      },
