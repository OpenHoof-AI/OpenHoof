...
const { makeStorePath } = createCronStoreHarness({ prefix: "openhoof-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPENHOOF_HOME for tilde expansion", () => {
    vi.stubEnv("OPENHOOF_HOME", "/srv/openhoof-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/openhoof-home", "cron", "jobs.json"));
  });
});
