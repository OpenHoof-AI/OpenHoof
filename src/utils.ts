...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".openhoof");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENHOOF_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENHOOF_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENHOOF_STATE_DIR.
