...
let originalOpenHoofLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.OPENHOOF_TEST_FILE_LOG;
  originalOpenHoofLogLevel = process.env.OPENHOOF_LOG_LEVEL;
  delete process.env.OPENHOOF_TEST_FILE_LOG;
  delete process.env.OPENHOOF_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.OPENHOOF_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalOpenHoofLogLevel === undefined) {
    delete process.env.OPENHOOF_LOG_LEVEL;
  } else {
    process.env.OPENHOOF_LOG_LEVEL = originalOpenHoofLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
