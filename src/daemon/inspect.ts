...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "openhoof" | "hoofbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["openhoof", "hoofbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["openhoof_service_marker"];
  const kindKeys = ["openhoof_service_kind"];
...
function isOpenHoofGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.openhoof.");
}

function isOpenHoofGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("openhoof-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isOpenHoofGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("openhoof gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("hoofbot") || lower.includes("moltbot");
}
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "hoofbot" : "moltbot",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "openhoof" && isOpenHoofGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "openhoof" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "openhoof" && isOpenHoofGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isOpenHoofGatewayTaskName(name)) {
        continue;
      }
