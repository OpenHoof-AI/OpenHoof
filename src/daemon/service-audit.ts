...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds OPENHOOF_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `openhoof gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENHOOF_GATEWAY_TOKEN does not match gateway.auth.token in openhoof.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.OPENHOOF_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.OPENHOOF_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `openhoof gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
