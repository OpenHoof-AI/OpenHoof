...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openhoof-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      OPENHOOF_LAUNCHD_LABEL: `ai.openhoof.launchd-int-${testId}`,
      OPENHOOF_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
