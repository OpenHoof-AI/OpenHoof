...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.OPENHOOF_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OPENHOOF_PROFILE: profile,
    OPENHOOF_GATEWAY_PORT: String(port),
    OPENHOOF_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OPENHOOF_SYSTEMD_UNIT: systemdUnit,
    OPENHOOF_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    OPENHOOF_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OPENHOOF_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform, extraPathDirs);
  const gatewayToken =
    env.OPENHOOF_GATEWAY_TOKEN?.trim() || env.HOOFBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OPENHOOF_GATEWAY_TOKEN: gatewayToken,
    OPENHOOF_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OPENHOOF_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OPENHOOF_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OPENHOOF_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OPENHOOF_LOG_PREFIX: "node",
    OPENHOOF_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OPENHOOF_SERVICE_KIND: NODE_SERVICE_KIND,
    OPENHOOF_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    OPENHOOF_STATE_DIR: sharedEnv.stateDir,
    OPENHOOF_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.OPENHOOF_STATE_DIR;
  const configPath = env.OPENHOOF_CONFIG_PATH;
