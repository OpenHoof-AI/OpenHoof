...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENHOOF_GATEWAY_PORT).toBe("18789");
    expect(env.OPENHOOF_GATEWAY_TOKEN).toBeUndefined();
    expect(env.OPENHOOF_SERVICE_MARKER).toBe("openhoof");
    expect(env.OPENHOOF_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENHOOF_SERVICE_VERSION).toBe("string");
    expect(env.OPENHOOF_SYSTEMD_UNIT).toBe("openhoof-gateway.service");
    expect(env.OPENHOOF_WINDOWS_TASK_NAME).toBe("OpenHoof Gateway");
    if (process.platform === "darwin") {
      expect(env.OPENHOOF_LAUNCHD_LABEL).toBe("ai.openhoof.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENHOOF_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENHOOF_SYSTEMD_UNIT).toBe("openhoof-gateway-work.service");
    expect(env.OPENHOOF_WINDOWS_TASK_NAME).toBe("OpenHoof Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.OPENHOOF_LAUNCHD_LABEL).toBe("ai.openhoof.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through OPENHOOF_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", OPENHOOF_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.OPENHOOF_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy HOOFBOT_GATEWAY_TOKEN to OPENHOOF_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", HOOFBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.OPENHOOF_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers OPENHOOF_GATEWAY_TOKEN over legacy HOOFBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPENHOOF_GATEWAY_TOKEN: "openhoof-token",
        HOOFBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.OPENHOOF_GATEWAY_TOKEN).toBe("openhoof-token");
  });

  it("omits OPENHOOF_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPENHOOF_GATEWAY_TOKEN: "   ",
        HOOFBOT_GATEWAY_TOKEN: " ",
      },
    });
    expect(env.OPENHOOF_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openhoof"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENHOOF_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openhoof-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OPENHOOF_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openhoof"));
  });

  it("uses OPENHOOF_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENHOOF_STATE_DIR: "/var/lib/openhoof" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openhoof"));
  });

  it("expands ~ in OPENHOOF_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENHOOF_STATE_DIR: "~/openhoof-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openhoof-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENHOOF_STATE_DIR: "C:\\State\\openhoof" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\openhoof");
  });
});
