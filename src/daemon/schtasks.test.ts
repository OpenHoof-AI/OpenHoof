...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when OPENHOOF_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".openhoof", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when OPENHOOF_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", OPENHOOF_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".openhoof-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers OPENHOOF_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        OPENHOOF_PROFILE: "rescue",
        OPENHOOF_STATE_DIR: "C:\\State\\openhoof",
      },
      expected: path.join("C:\\State\\openhoof", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", OPENHOOF_PROFILE: "default" },
      expected: path.join("/home/test", ".openhoof", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem OpenHoof Gateway",
          "cd /d C:\\Projects\\openhoof",
          "set NODE_ENV=production",
          "set OPENHOOF_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\openhoof",
          environment: {
            NODE_ENV: "production",
            OPENHOOF_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\openhoof\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\openhoof\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\OpenHoof Share\\node.exe" "\\\\fileserver\\OpenHoof Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\OpenHoof Share\\node.exe",
            "\\\\fileserver\\OpenHoof Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from OPENHOOF_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ OPENHOOF_STATE_DIR: path.join(tmpDir, "custom-state") }),
