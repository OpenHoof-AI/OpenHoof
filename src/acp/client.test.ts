...
const createTempDir = () => tempDirs.make("openhoof-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets OPENHOOF_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "openhoof",
    });

    expect(env.OPENHOOF_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("openhoof");
  });

  it("overrides pre-existing OPENHOOF_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      OPENHOOF_SHELL: "wrong",
    });
    expect(env.OPENHOOF_SHELL).toBe("acp-client");
  });
  ...
  it("preserves OPENHOOF_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        OPENHOOF_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["OPENHOOF_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default OpenHoof bridge", () => {
    ...
    expect(env.OPENHOOF_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.OPENHOOF_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "openhoof",
        serverArgs: ["acp"],
        defaultServerCommand: "openhoof",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("OPENHOOF_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "openhoof", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "openhoof", "dist", "entry.js");
    const shimPath = path.join(dir, "openhoof.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\openhoof\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/openhoof-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/openhoof-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/openhoof-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
