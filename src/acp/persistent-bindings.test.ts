...
type ConfiguredBinding = NonNullable<OpenHoofConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<OpenHoofConfig>,
): OpenHoofConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: OpenHoofConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/openhoof" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
  ...
  it("does not clear ACP metadata before reinitialize succeeds", async () => {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockReturnValue({
      acp: {
        ...
        mode: "persistent",
        backend: "acpx",
        runtimeOptions: { cwd: "/home/bob/hoofd" },
      },
    });
