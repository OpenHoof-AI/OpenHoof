...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: OpenHoofConfig) => Array<string | number> | undefined;
}): OpenHoofConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: OpenHoofConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: OpenHoofConfig) => Array<string | number> | undefined;
}): (cfg: OpenHoofConfig, dmPolicy: DmPolicy) => OpenHoofConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: OpenHoofConfig, allowFrom: string[]) => OpenHoofConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: OpenHoofConfig, groupPolicy: "open" | "allowlist" | "disabled") => OpenHoofConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: OpenHoofConfig) => OpenHoofConfig | Promise<OpenHoofConfig>;
  applySet?: (
    cfg: OpenHoofConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => OpenHoofConfig | Promise<OpenHoofConfig>;
}): Promise<{
  cfg: OpenHoofConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in OpenHoof config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "OPENHOOF_SECRET",
      envVarFormatError:
        'Use an env var name like "OPENHOOF_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends OpenHoofConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: OpenHoofConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: OpenHoofConfig) => Array<string | number>;
