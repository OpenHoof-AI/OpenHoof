...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@openhoof/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-catalog-"));
  ...
  it("preserves plugin ids when they differ from channel ids", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-channel-catalog-state-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-catalog-home-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@openhoof/env-demo-channel",
            openhoof: {
              channel: {
                id: "env-demo-channel",
              ...
              install: {
                npmSpec: "@openhoof/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        OPENHOOF_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-catalog-state-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@openhoof/default-env-demo",
            openhoof: {
              channel: {
                id: "default-env-demo",
              ...
              install: {
                npmSpec: "@openhoof/default-env-demo",
              },
            },
          },
        ],
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: OpenHoofConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
