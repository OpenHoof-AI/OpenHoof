...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OpenHoofConfig, policy: ChannelAccessPolicy) => OpenHoofConfig;
  resolveAllowlist: (params: { cfg: OpenHoofConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OpenHoofConfig; resolved: TResolved }) => OpenHoofConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OpenHoofConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OpenHoofConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OpenHoofConfig, policy: ChannelAccessPolicy): OpenHoofConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OpenHoofConfig, policy: ChannelAccessPolicy): OpenHoofConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OpenHoofConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OpenHoofConfig; resolved: string[] }) => {
      calls.push("apply");
