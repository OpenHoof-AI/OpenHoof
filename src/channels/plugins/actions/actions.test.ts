...
function telegramCfg(): OpenHoofConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OpenHoofConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OpenHoofConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OpenHoofConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OpenHoofConfig;
...
function createSignalAccountOverrideCfg(): OpenHoofConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("derives discord action listings from channel and moderation gates", () => {
    const cases = [
      {
        name: "defaults",
        cfg: { channels: { discord: { token: "d0" } } } as OpenHoofConfig,
        expectUploads: true,
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OpenHoofConfig,
        accountId: undefined,
        params: {
