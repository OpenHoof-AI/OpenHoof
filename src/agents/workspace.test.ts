...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENHOOF_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENHOOF_HOME: "/srv/openhoof-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/openhoof-home"), ".openhoof", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".openhoof", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("openhoof-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("openhoof-workspace-");
    await fs.mkdir(path.join(tempDir, ".openhoof"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-workspace-hardlink-"));
    try {
