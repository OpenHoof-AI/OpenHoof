...
test("exec sets OPENHOOF_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-openhoof-shell", {
    command: "node -e \"process.stdout.write(process.env.OPENHOOF_SHELL || '')\"",
    pty: true,
  });
