...
const defaultTools = createOpenHoofCodingTools({ senderIsOwner: true });
...
describe("createOpenHoofCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: OpenHoofConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createOpenHoofCodingTools({
    ...
    const anthropicTools = createOpenHoofCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: OpenHoofConfig = {
      tools: {
        exec: {
    ...
    const allowed = createOpenHoofCodingTools({
    ...
    const denied = createOpenHoofCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createOpenHoofCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
