...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: OpenHoofConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/openhoof" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: OpenHoofConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: OpenHoofConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: OpenHoofConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: OpenHoofConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: OpenHoofConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: OpenHoofConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: OpenHoofConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/openhoof");
  });

  it("uses OPENHOOF_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "openhoof-home");
    vi.stubEnv("OPENHOOF_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OpenHoofConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".openhoof", "workspace"));
  });

  it("uses OPENHOOF_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "openhoof-home");
    vi.stubEnv("OPENHOOF_HOME", home);
    // Clear state dir so it falls back to OPENHOOF_HOME
    vi.stubEnv("OPENHOOF_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as OpenHoofConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".openhoof", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/openhoof-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/openhoof-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-agent-scope-"));
