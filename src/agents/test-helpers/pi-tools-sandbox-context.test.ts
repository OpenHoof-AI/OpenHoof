...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("openhoof-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("openhoof-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("openhoof-sbx-");
