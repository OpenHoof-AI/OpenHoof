...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPENHOOF_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".openhoof", `workspace-${profile}`);
  }
  return path.join(home, ".openhoof", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".openhoof";
