    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "openhoof.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "openhoof.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "openhoof.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("openhoof-");
  const acpxRoot = await tempDirs.make("openhoof-acpx-plugin-");
  const helperRoot = await tempDirs.make("openhoof-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("openhoof-");
  const pluginRoot = await tempDirs.make("openhoof-plugin-");
  const outsideDir = await tempDirs.make("openhoof-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("openhoof-");
    const pluginRoot = await tempDirs.make("openhoof-claude-bundle-");
