...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseOpenHoofManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyOpenHoofManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveOpenHoofMetadata(
  frontmatter: ParsedSkillFrontmatter,
): OpenHoofSkillMetadata | undefined {
  const metadataObj = resolveOpenHoofManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveOpenHoofManifestRequires(metadataObj);
  const install = resolveOpenHoofManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveOpenHoofManifestOs(metadataObj);
  return {
