...
import { resolvePreferredOpenHoofTmpDir } from "../infra/tmp-openhoof-dir.js";
...
async function withOutsideHardlinkInOpenHoofTmp<T>(
  params: {
    openHoofTmpDir: string;
  ...
  const hardlinkPath = path.join(params.openHoofTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.openHoofTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.openHoofTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.openHoofTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const openHoofTmpDir = resolvePreferredOpenHoofTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred OpenHoof tmp root",
      media: path.join(openHoofTmpDir, "image.png"),
      expected: path.join(openHoofTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred OpenHoof tmp root",
      media: pathToFileURL(path.join(openHoofTmpDir, "photo.png")).href,
      expected: path.join(openHoofTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred OpenHoof tmp root",
      media: path.join(openHoofTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(openHoofTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(openHoofTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside openhoof tmp root",
      media: path.join(os.tmpdir(), "outside-openhoof", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked OpenHoof tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(openHoofTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(openHoofTmpDir, { recursive: true });
      const symlinkPath = path.join(openHoofTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked OpenHoof tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenHoofTmp(
      {
        openHoofTmpDir,
  ...
  it("rejects symlinked OpenHoof tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenHoofTmp(
