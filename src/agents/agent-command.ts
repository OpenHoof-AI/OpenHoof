...
function prependInternalEventContext(
  ...
  if (body.includes("OpenHoof runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("openhoof agents list")}" to see configured agents.`,
      );
    }
  }
