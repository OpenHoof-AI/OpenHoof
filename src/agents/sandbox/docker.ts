...
} from "openhoof/plugin-sdk/windows-spawn";
...
import { markOpenHoofExecEnv } from "../../infra/openhoof-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openhoof.sandbox=1");
  args.push("--label", `openhoof.sessionKey=${params.scopeKey}`);
  args.push("--label", `openhoof.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openhoof.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markOpenHoofExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "openhoof.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openhoof sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openhoof sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openhoof sandbox recreate --all");
}
