...
import { planOpenHoofModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: OpenHoofConfig): {
  config: OpenHoofConfig;
  sourceConfigForSecrets: OpenHoofConfig;
} {
...
export async function ensureOpenHoofModelsJson(
  config?: OpenHoofConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveOpenHoofAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planOpenHoofModelsJson({
      cfg,
