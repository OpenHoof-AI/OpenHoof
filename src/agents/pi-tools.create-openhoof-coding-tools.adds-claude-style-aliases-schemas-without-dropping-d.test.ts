...
describe("createOpenHoofCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-read-"));
    try {
      ...
      const contents = "Hello from openhoof read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createOpenHoofCodingTools({ sandbox });
