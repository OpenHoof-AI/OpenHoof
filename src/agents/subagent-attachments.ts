...
function resolveAttachmentLimits(config: OpenHoofConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".openhoof", "attachments");
  const relDir = path.posix.join(".openhoof", "attachments", attachmentId);
