...
type CoreToolDefinition = {
  ...
  includeInOpenHoofGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const openhoofTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOpenHoofGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:openhoof": openhoofTools,
