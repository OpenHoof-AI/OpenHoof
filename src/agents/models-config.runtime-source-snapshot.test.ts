...
function createOpenAiApiKeySourceConfig(): OpenHoofConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): OpenHoofConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): OpenHoofConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): OpenHoofConfig {
  return {
    models: {
      providers: {
        openai: {
...
function withGatewayTokenMode(config: OpenHoofConfig): OpenHoofConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function withGeneratedModelsFromRuntimeSource(
  params: {
    ...
    runtimeConfig: OpenHoofConfig;
    candidateConfig?: OpenHoofConfig;
  },
  ...
  await withTempHome(async () => {
    try {
      ...
      await ensureOpenHoofModelsJson(params.candidateConfig ?? loadConfig());
      await runAssertions();
    } finally {
      clearRuntimeConfigSnapshot();
      clearConfigCache();
...
describe("models-config runtime source snapshot", () => {
  ...
  it("uses non-env marker from runtime source snapshot for file refs", async () => {
    await withTempHome(async () => {
      ...
      const runtimeConfig: OpenHoofConfig = {
        models: {
          providers: {
            moonshot: {
              baseUrl: "https://api.moonshot.ai/v1",
      ...
      try {
        ...
        await ensureOpenHoofModelsJson(loadConfig());
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    await withTempHome(async () => {
      ...
      const clonedRuntimeConfig: OpenHoofConfig = {
        ...runtimeConfig,
        agents: {
          defaults: {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureOpenHoofModelsJson(clonedRuntimeConfig);
  ...
  it("keeps source markers when runtime projection is skipped for incompatible top-level shape", async () => {
    await withTempHome(async () => {
      ...
      const incompatibleCandidate: OpenHoofConfig = {
        ...createOpenAiApiKeyRuntimeConfig(),
      };

      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureOpenHoofModelsJson(incompatibleCandidate);
        await expectGeneratedProviderApiKey("openai", "OPENAI_API_KEY"); // pragma: allowlist secret
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
