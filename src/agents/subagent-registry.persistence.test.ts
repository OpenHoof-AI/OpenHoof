...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["OPENHOOF_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-subagent-"));
  ...
  it("uses isolated temp state when OPENHOOF_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "openhoof-test-state"));
  });
});
