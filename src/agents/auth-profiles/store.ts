 ...
 * A common mistake — caused by the similarity with the `openhoof.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (openhoof.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.OPENHOOF_AUTH_STORE_READONLY === "1";
