...
const GOOGLE_LIVE_SINGLE_KEY = "OPENHOOF_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPENHOOF_LIVE_ANTHROPIC_KEY",
    listVar: "OPENHOOF_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPENHOOF_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPENHOOF_LIVE_${base}_KEY`;
