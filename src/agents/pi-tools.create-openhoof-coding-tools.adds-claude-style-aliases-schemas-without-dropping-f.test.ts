...
describe("createOpenHoofCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-alias-"));
    try {
      const tools = createOpenHoofCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-structured-write-"));
    try {
      const tools = createOpenHoofCodingTools({ workspaceDir: tmpDir });
      ...
      await writeTool?.execute("tool-structured-write", {
        ...
        content: [
          ...
          { type: "input_text", text: "const root = path.join(process.env.HOME, 'hoofd');\n" },
        ],
      });

      const written = await fs.readFile(path.join(tmpDir, "structured-write.js"), "utf8");
      expect(written).toBe(
        "const path = require('path');\nconst root = path.join(process.env.HOME, 'hoofd');\n",
      );
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-structured-edit-"));
    try {
