...
function findTool(name: string, config: OpenHoofConfig) {
  const allTools = createOpenHoofTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: OpenHoofConfig; env?: NodeJS.ProcessEnv }) {
...
describe("openhoof tools runtime web metadata wiring", () => {
