...
function applyModelProviderToolPolicy(
  ...
  // Models with a native web_search tool cannot receive OpenHoof's
...
function resolveExecConfig(params: { cfg?: OpenHoofConfig; agentId?: string }) {
...
export function createOpenHoofCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOpenHoofReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
