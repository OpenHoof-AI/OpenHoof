...
const originalBundledDir = process.env.OPENHOOF_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("openhoof-bundled-");
  const workspaceDir = await createTempDir("openhoof-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: OpenHoofConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.OPENHOOF_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
...
afterEach(async () => {
  process.env.OPENHOOF_BUNDLED_PLUGINS_DIR = originalBundledDir;
  clearPluginManifestRegistryCache();
  await Promise.all(
