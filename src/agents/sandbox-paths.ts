...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const openHoofTmpDir = path.resolve(resolvePreferredOpenHoofTmpDir());
  if (!isPathInside(openHoofTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: openHoofTmpDir });
  return resolved;
}
