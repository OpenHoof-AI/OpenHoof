...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openhoof: process.env.OPENHOOF_GATEWAY_TOKEN,
    hoofbot: process.env.HOOFBOT_GATEWAY_TOKEN,
  };
  ...
  beforeEach(() => {
    ...
    delete process.env.HOOFBOT_GATEWAY_TOKEN;
  });

  afterAll(() => {
    if (envSnapshot.openhoof === undefined) {
      delete process.env.OPENHOOF_GATEWAY_TOKEN;
    } else {
      process.env.OPENHOOF_GATEWAY_TOKEN = envSnapshot.openhoof;
    }
    if (envSnapshot.hoofbot === undefined) {
      delete process.env.HOOFBOT_GATEWAY_TOKEN;
    } else {
      process.env.HOOFBOT_GATEWAY_TOKEN = envSnapshot.hoofbot;
    }
  });
  ...
  it("uses OPENHOOF_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OPENHOOF_GATEWAY_TOKEN = "local-env-token";
    process.env.HOOFBOT_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
