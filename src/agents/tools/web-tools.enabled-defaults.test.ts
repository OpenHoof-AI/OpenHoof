...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "openhoof" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "OpenHoof", url: "https://openhoof.ai/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest openhoof release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://openhoof.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://openhoof.ai/docs"]);
