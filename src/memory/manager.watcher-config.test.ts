...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): OpenHoofConfig {
    const defaults: NonNullable<NonNullable<OpenHoofConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: OpenHoofConfig) {
