...
const TEMP_MEDIA_PREFIX = "openhoof-media-";
...
function createGroqAudioConfig(): OpenHoofConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OpenHoofConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OpenHoofConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: OpenHoofConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredOpenHoofTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        OPENHOOF_AGENT_DIR: isolatedAgentDir,
