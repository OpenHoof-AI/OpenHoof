...
import type { ConfigFileSnapshot, OpenHoofConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: OpenHoofConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your OpenHoof state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your OpenHoof state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.OPENHOOF_GATEWAY_TOKEN) || hasNonEmptyString(env.HOOFBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.OPENHOOF_GATEWAY_PASSWORD) ||
    hasNonEmptyString(env.HOOFBOT_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("openhoof security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.OPENHOOF_GATEWAY_TOKEN) ||
    hasNonEmptyString(env.HOOFBOT_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: OpenHoofConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: OpenHoofConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: OpenHoofConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("openhoof status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("openhoof security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set OPENHOOF_GATEWAY_TOKEN/OPENHOOF_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("openhoof security audit --deep")}".`,
    });
  }
