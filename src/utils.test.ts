...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("openhoof-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("openhoof-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("openhoof-lid-a-", async (first) => {
      await withTempDir("openhoof-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openhoof when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openhoof-config-dir-"));
    try {
  ...
  it("expands OPENHOOF_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/openhoof-home",
      OPENHOOF_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/openhoof-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers OPENHOOF_HOME over HOME", () => {
    vi.stubEnv("OPENHOOF_HOME", "/srv/openhoof-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/openhoof-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENHOOF_HOME prefix when OPENHOOF_HOME is set", () => {
    vi.stubEnv("OPENHOOF_HOME", "/srv/openhoof-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/openhoof-home")}/.openhoof/openhoof.json`)).toBe(
      "$OPENHOOF_HOME/.openhoof/openhoof.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENHOOF_HOME replacement when OPENHOOF_HOME is set", () => {
    vi.stubEnv("OPENHOOF_HOME", "/srv/openhoof-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/openhoof-home")}/.openhoof/openhoof.json`),
    ).toBe("config: $OPENHOOF_HOME/.openhoof/openhoof.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openhoof", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "openhoof"),
    );
  });
  ...
  it("prefers OPENHOOF_HOME for tilde expansion", () => {
    vi.stubEnv("OPENHOOF_HOME", "/srv/openhoof-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/openhoof")).toBe(path.resolve("/srv/openhoof-home", "openhoof"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/openhoof", env)).toBe(path.resolve("/srv/openhoof-home", "openhoof"));
  });
