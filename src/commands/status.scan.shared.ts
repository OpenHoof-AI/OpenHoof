...
export function hasExplicitMemorySearchConfig(cfg: OpenHoofConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: OpenHoofConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: OpenHoofConfig, agentId: string) => { store: { path: string } } | null;
