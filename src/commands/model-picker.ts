...
type PromptDefaultModelResult = { model?: string; config?: OpenHoofConfig };
...
function resolveConfiguredModelRaw(cfg: OpenHoofConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OpenHoofConfig): string[] {
...
export function applyModelAllowlist(cfg: OpenHoofConfig, models: string[]): OpenHoofConfig {
