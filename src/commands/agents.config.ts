...
function resolveAgentName(cfg: OpenHoofConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenHoofConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenHoofConfig): AgentSummary[] {
