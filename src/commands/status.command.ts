...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByOpenHoof ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByOpenHoof ? "installed · " : "";
  ...
  runtime.log(theme.heading("OpenHoof status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`openhoof devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("openhoof devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("openhoof devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openhoof security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openhoof security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openhoof.ai/faq");
  runtime.log("Troubleshooting: https://docs.openhoof.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openhoof status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openhoof logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openhoof status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openhoof gateway probe")}`);
  }
}
