...
import { resolveOpenHoofPackageRoot } from "../infra/openhoof-root.js";
...
function resolveMode(cfg: OpenHoofConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenHoof doctor");

  const root = await resolveOpenHoofPackageRoot({
  ...
  let cfg: OpenHoofConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("openhoof configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openhoof config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openhoof setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("openhoof config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("openhoof config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openhoof doctor --fix")}" to apply changes.`);
  }
