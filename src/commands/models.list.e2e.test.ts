...
const ensureOpenHoofModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveOpenHoofAgentDir = vi.fn().mockReturnValue("/tmp/openhoof-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureOpenHoofModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveOpenHoofAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureOpenHoofModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureOpenHoofModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureOpenHoofModelsJson).toHaveBeenCalled();
    expect(ensureOpenHoofModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
