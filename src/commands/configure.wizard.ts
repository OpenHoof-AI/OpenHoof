...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.OPENHOOF_GATEWAY_TOKEN ?? process.env.HOOFBOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
    process.env.OPENHOOF_GATEWAY_PASSWORD ??
    process.env.HOOFBOT_GATEWAY_PASSWORD ??
    configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from openhoof.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "OpenHoof update wizard" : "OpenHoof configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: OpenHoofConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("openhoof doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      ...
      token:
        ...
        process.env.HOOFBOT_GATEWAY_TOKEN ??
        baseLocalProbeToken,
      password:
        process.env.OPENHOOF_GATEWAY_PASSWORD ??
        process.env.HOOFBOT_GATEWAY_PASSWORD ??
