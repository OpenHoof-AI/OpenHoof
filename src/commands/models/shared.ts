...
export async function loadValidConfigOrThrow(): Promise<OpenHoofConfig> {
...
export async function updateConfig(
  mutator: (cfg: OpenHoofConfig) => OpenHoofConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenHoofConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenHoofConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openhoof agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
