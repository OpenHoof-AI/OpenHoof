...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenHoofAgentDir: vi.fn().mockReturnValue("/tmp/openhoof-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openhoof-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openhoof-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/openhoof-dev/openhoof.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenHoofAgentDir: mocks.resolveOpenHoofAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/openhoof-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenHoofAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/openhoof-dev/openhoof.json");
    expect(payload.auth.storePath).toBe("/tmp/openhoof-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/openhoof-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/openhoof-agent-custom");
