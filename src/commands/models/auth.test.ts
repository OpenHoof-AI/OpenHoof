...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: OpenHoofConfig;
  let lastUpdatedConfig: OpenHoofConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/openhoof/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/openhoof/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/openhoof/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: OpenHoofConfig) => OpenHoofConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/openhoof/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/openhoof/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `openhoof plugins list --json`.',
    );
  });
