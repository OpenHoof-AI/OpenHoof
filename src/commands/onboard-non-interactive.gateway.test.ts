...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("openhoof-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "openhoof");
  ...
  it("uses OPENHOOF_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      const envToken = "tok_env_fallback_123";
      const workspace = path.join(stateDir, "openhoof");
      const prevToken = process.env.OPENHOOF_GATEWAY_TOKEN;
      process.env.OPENHOOF_GATEWAY_TOKEN = envToken;
      ...
      try {
        ...
        if (prevToken === undefined) {
          ...
          process.env.OPENHOOF_GATEWAY_TOKEN = prevToken;
        }
      }
    });
  }, 60_000);
        ...
        await runNonInteractiveSetup(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "OPENHOOF_GATEWAY_TOKEN",
          },
          runtime,
        );
        ...
        runNonInteractiveSetup(
          {
            nonInteractive: true,
            mode: "local",
            workspace: path.join(stateDir, "openhoof"),
            authChoice: "skip",
            skipSkills: true,
            skipHealth: false,
            installDaemon: false,
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `openhoof gateway status --deep` for more detail.");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.OPENHOOF_STATE_DIR = stateDir;
      process.env.OPENHOOF_CONFIG_PATH = path.join(stateDir, "openhoof.json");
