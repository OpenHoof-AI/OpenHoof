...
import type { OpenHoofConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: OpenHoofConfig): string[] {
...
export async function noteSecurityWarnings(cfg: OpenHoofConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openhoof security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.openhoof/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("openhoof approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.openhoof.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openhoof configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openhoof config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openhoof doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openhoof config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('openhoof config set session.dmScope "per-channel-peer"') +
