...
import { readConfigFileSnapshot, writeConfigFile, type OpenHoofConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("openhoof config set gateway.auth.mode token")} or ${formatCliCommand("openhoof config set gateway.auth.mode password")}.`,
  ].join(" ");
}
