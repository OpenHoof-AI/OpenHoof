...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPENHOOF_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openhoof-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-oauth-sync-"));
    ...
    process.env.OPENHOOF_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-oauth-nosync-"));
    process.env.OPENHOOF_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPENHOOF_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-oauth-external-"));
    process.env.OPENHOOF_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPENHOOF_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENHOOF_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openhoof-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
