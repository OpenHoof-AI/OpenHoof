...
import { OpenHoofSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: OpenHoofConfig): {
  config: OpenHoofConfig;
  removed: string[];
} {
  const parsed = OpenHoofSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: OpenHoofConfig): void {
