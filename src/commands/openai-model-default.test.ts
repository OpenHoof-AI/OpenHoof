...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OpenHoofConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OpenHoofConfig) => { changed: boolean; next: OpenHoofConfig };
  defaultModel: string;
  overrideConfig: OpenHoofConfig;
  alreadyDefaultConfig: OpenHoofConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OpenHoofConfig) => config,
      applyDefaultConfig: (config: OpenHoofConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: OpenHoofConfig = { agents: { defaults: {} } };
