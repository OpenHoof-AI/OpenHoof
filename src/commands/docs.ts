...
const SEARCH_TOOL = "https://docs.openhoof.ai/mcp.SearchOpenHoof";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openhoof.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openhoof docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openhoof.ai/");
      runtime.log(`Search: ${formatCliCommand('openhoof docs "your query"')}`);
    }
    return;
  }
