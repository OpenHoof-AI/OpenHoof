...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "OPENHOOF_GATEWAY_TOKEN",
          placeholder: "OPENHOOF_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like OPENHOOF_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. OpenHoof will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: OpenHoof trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.openhoof.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
