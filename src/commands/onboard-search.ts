...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<OpenHoofConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<OpenHoofConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: OpenHoofConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: OpenHoofConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: OpenHoofConfig, provider: SearchProvider): OpenHoofConfig {
  const providerEntry = resolvePluginWebSearchProviders({
    config,
    bundledAllowlistCompat: true,
  }).find((candidate) => candidate.id === provider);
...
function preserveDisabledState(original: OpenHoofConfig, result: OpenHoofConfig): OpenHoofConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with openhoof configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — OpenHoof will store a reference instead of the API key.",
