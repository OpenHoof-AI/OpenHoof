...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "OpenHoof", ".openhoof");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/openhoof-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".openhoof");
  ...
  it("anchors cloud detection to OS homedir when OPENHOOF_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".openhoof");
    const originalOpenHoofHome = process.env.OPENHOOF_HOME;
    process.env.OPENHOOF_HOME = "/tmp/openhoof-home-override";
    ...
    try {
      ...
      if (originalOpenHoofHome === undefined) {
        delete process.env.OPENHOOF_HOME;
      } else {
        process.env.OPENHOOF_HOME = originalOpenHoofHome;
      }
    }
  });
