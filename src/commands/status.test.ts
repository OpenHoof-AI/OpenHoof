...
beforeAll(() => {
  envSnapshot = captureEnv(["OPENHOOF_PROFILE"]);
  process.env.OPENHOOF_PROFILE = "isolated";
});
...
vi.mock("../infra/openhoof-root.js", () => ({
  resolveOpenHoofPackageRoot: vi.fn().mockResolvedValue("/tmp/openhoof"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openhoof/pnpm-lock.yaml",
      markerPath: "/tmp/openhoof/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.openhoof.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.openhoof.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("openhoof status --all") ||
          line.includes("openhoof --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("OPENHOOF_GATEWAY_TOKEN", "abcd1234", async () => {
