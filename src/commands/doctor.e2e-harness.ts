...
export const resolveOpenHoofPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenHoofPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveOpenHoofPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENHOOF_STATE_DIR;
  originalUpdateInProgress = process.env.OPENHOOF_UPDATE_IN_PROGRESS;
  process.env.OPENHOOF_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENHOOF_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENHOOF_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENHOOF_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
