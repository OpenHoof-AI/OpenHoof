...
type EnvSnapshot = {
  HOME?: string;
  OPENHOOF_HOME?: string;
  OPENHOOF_STATE_DIR?: string;
  OPENHOOF_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    OPENHOOF_HOME: process.env.OPENHOOF_HOME,
    OPENHOOF_STATE_DIR: process.env.OPENHOOF_STATE_DIR,
    OPENHOOF_OAUTH_DIR: process.env.OPENHOOF_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OpenHoofConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OpenHoofConfig) {
...
async function runStateIntegrityText(cfg: OpenHoofConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.OPENHOOF_HOME = tempHome;
    process.env.OPENHOOF_STATE_DIR = path.join(tempHome, ".openhoof");
    delete process.env.OPENHOOF_OAUTH_DIR;
    fs.mkdirSync(process.env.OPENHOOF_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OPENHOOF_OAUTH_DIR is explicitly configured", async () => {
    process.env.OPENHOOF_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenHoofConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints openhoof-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: OpenHoofConfig = {};
    ...
    expect(text).toMatch(/openhoof sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/openhoof sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /openhoof sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
