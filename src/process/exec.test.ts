...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        OPENHOOF_BASE_ENV: "base",
        OPENHOOF_TO_REMOVE: undefined,
      },
      env: {
        OPENHOOF_TEST_ENV: "ok",
      },
    });

    expect(resolved.OPENHOOF_BASE_ENV).toBe("base");
    expect(resolved.OPENHOOF_TEST_ENV).toBe("ok");
    expect(resolved.OPENHOOF_TO_REMOVE).toBeUndefined();
    expect(resolved.OPENHOOF_CLI).toBe(OPENHOOF_CLI_ENV_VALUE);
  });
