...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing openhoof.extensions; update the plugin package to include openhoof.extensions (for example ["./dist/index.js"]). See https://docs.openhoof.ai/help/troubleshooting#plugin-install-fails-with-missing-openhoof-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_OPENHOOF_EXTENSIONS: "missing_openhoof_extensions",
  EMPTY_OPENHOOF_EXTENSIONS: "empty_openhoof_extensions",
...
function ensureOpenHoofExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_OPENHOOF_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json openhoof.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_OPENHOOF_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Bundle "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "openhoof security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "openhoof security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureOpenHoofExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureOpenHoofExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from openhoof.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-openhoof"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "openhoof security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "openhoof security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "openhoof-npm-pack-",
    spec,
    timeoutMs,
