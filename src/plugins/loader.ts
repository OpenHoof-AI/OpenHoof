...
import { resolveOpenHoofPackageRootSync } from "../infra/openhoof-root.js";
...
import { discoverOpenHoofPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  ...
  const packageRoot = resolveOpenHoofPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`openhoof/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpenHoofPluginDefinition;
  register?: OpenHoofPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpenHoofPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpenHoofPluginDefinition;
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverOpenHoofPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverOpenHoofPlugins>["candidates"][number];
  right: ReturnType<typeof discoverOpenHoofPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadOpenHoofPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadOpenHoofPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  const getJiti = (modulePath: string) => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "openhoof/plugin-sdk": pluginSdkAlias } : {}),
      ...resolvePluginSdkScopedAliasMap(),
    };
  ...
  const discovery = discoverOpenHoofPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into OpenHoof yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: OpenHoofPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as OpenHoofPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
