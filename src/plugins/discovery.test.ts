...
import { clearPluginDiscoveryCache, discoverOpenHoofPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("openhoof-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverOpenHoofPlugins>[0],
) {
  return discoverOpenHoofPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      openhoof: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverOpenHoofPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".openhoof", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".openhoof", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverOpenHoofPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@openhoof/voice-call",
  ...
  it("normalizes bundled provider package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@openhoof/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@openhoof/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@openhoof/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@openhoof/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("openhoof");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@openhoof/escape-pack",
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverOpenHoofPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPENHOOF_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverOpenHoofPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPENHOOF_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverOpenHoofPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPENHOOF_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
