...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      openhoof: { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "openhoof.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.openhoof/extensions and preserves scoped package ids", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@openhoof/voice-call" });
  });
  ...
  it("installs from a zip archive", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@openhoof/zipper" });
  });
  ...
  it("rejects packages without openhoof.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@openhoof/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("openhoof.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_OPENHOOF_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when openhoof.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@openhoof/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing openhoof.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_OPENHOOF_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without openhoof.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.openhoof).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses openhoof.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@openhoof/cognee-openhoof"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it("preserves scoped package names when no plugin manifest id is present", async () => {
    ...
    expectInstalledWithPluginId(res, extensionsDir, "@openhoof/test-plugin");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@openhoof/voice-call@0.0.1",
              name: "@openhoof/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@openhoof/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@openhoof/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@openhoof/voice-call@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@openhoof/voice-call@0.0.2-beta.1",
      name: "@openhoof/voice-call",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@openhoof/voice-call@beta"');
    }
  });
  ...
  it("allows explicit prerelease npm tags", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@openhoof/voice-call@0.0.2-beta.1");
    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@openhoof/voice-call@beta",
    });
    expect(packTmpDir).not.toBe("");
  });
});
