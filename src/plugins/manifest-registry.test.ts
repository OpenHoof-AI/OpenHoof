...
function makeTempDir() {
  return makeTrackedTempDir("openhoof-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "openhoof.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "openhoof.plugin.json");
  const linkedManifest = path.join(rootDir, "openhoof.plugin.json");
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: {
        ...
        OPENHOOF_STATE_DIR: path.join(homeA, ".state"),
      },
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: {
        ...
        HOME: homeB,
        OPENHOOF_HOME: undefined,
        OPENHOOF_STATE_DIR: path.join(homeB, ".state"),
      },
    });
