...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-openhoof",
      targetDir: "/tmp/opik-openhoof",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-openhoof": {
              source: "npm",
              spec: "@opik/opik-openhoof",
              integrity: "sha512-old",
              installPath: "/tmp/opik-openhoof",
            },
          },
        },
      },
      pluginIds: ["opik-openhoof"],
      dryRun: true,
    });
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @openhoof/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @openhoof/missing.",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@openhoof/voice-call",
      targetDir: "/tmp/openhoof-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@openhoof/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@openhoof/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@openhoof/voice-call");
    expect(result.config.plugins?.entries?.["@openhoof/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@openhoof/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@openhoof/voice-call",
      installPath: "/tmp/openhoof-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { OPENHOOF_HOME: "/srv/openhoof-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/openhoof-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
