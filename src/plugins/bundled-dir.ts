...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.OPENHOOF_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
  ...
  try {
    const packageRoots = [
      resolveOpenHoofPackageRootSync({ cwd: process.cwd() }),
      resolveOpenHoofPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
