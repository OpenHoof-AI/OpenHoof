...
export type OpenHoofPluginConfigSchema = {
...
export type OpenHoofPluginToolContext = {
...
export type OpenHoofPluginToolFactory = (
  ctx: OpenHoofPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type OpenHoofPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type OpenHoofPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<OpenHoofConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). OpenHoof uses this to expose
 ...
 *
 * Runs after OpenHoof resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common OpenHoof auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and OpenHoof wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before OpenHoof builds its generic
...
/**
 * Provider-owned stream wrapper hook after OpenHoof applies its generic
 ...
 *
 * Return `true` or `false` to override OpenHoof's built-in provider cache TTL
 ...
 *
 * Runs only after OpenHoof exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after OpenHoof loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * OpenHoof calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic OpenHoof wrappers.
   *
   ...
   *
   * Called after OpenHoof resolves the raw configured credential but before the
   ...
   * hint than OpenHoof's generic auth-store guidance.
   */
   ...
   * `errorMessage` when OpenHoof should surface a provider-specific hint for
   ...
   * merging. OpenHoof deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * OpenHoof uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `openhoof doctor`.
   *
   ...
   *
   * OpenHoof calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep OpenHoof's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: OpenHoofConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: OpenHoofConfig, value: unknown) => void;
  applySelectionConfig?: (config: OpenHoofConfig) => OpenHoofConfig;
...
export type OpenHoofPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current OpenHoof configuration */
  config: OpenHoofConfig;
 ...
 */
export type OpenHoofPluginCommandDefinition = {
...
export type OpenHoofPluginHttpRouteAuth = "gateway" | "plugin";
export type OpenHoofPluginHttpRouteMatch = "exact" | "prefix";

export type OpenHoofPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type OpenHoofPluginHttpRouteParams = {
  path: string;
  handler: OpenHoofPluginHttpRouteHandler;
  auth: OpenHoofPluginHttpRouteAuth;
  match?: OpenHoofPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type OpenHoofPluginCliContext = {
  program: Command;
  config: OpenHoofConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type OpenHoofPluginCliRegistrar = (ctx: OpenHoofPluginCliContext) => void | Promise<void>;

export type OpenHoofPluginServiceContext = {
  config: OpenHoofConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type OpenHoofPluginService = {
  id: string;
  start: (ctx: OpenHoofPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: OpenHoofPluginServiceContext) => void | Promise<void>;
};

export type OpenHoofPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

export type OpenHoofPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  kind?: PluginKind;
  configSchema?: OpenHoofPluginConfigSchema;
  register?: (api: OpenHoofPluginApi) => void | Promise<void>;
  activate?: (api: OpenHoofPluginApi) => void | Promise<void>;
};

export type OpenHoofPluginModule =
  | OpenHoofPluginDefinition
  | ((api: OpenHoofPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

export type OpenHoofPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | OpenHoofPluginToolFactory,
    opts?: OpenHoofPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: OpenHoofPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: OpenHoofPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: OpenHoofPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: OpenHoofPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: OpenHoofPluginService) => void;
   ...
   */
  registerCommand: (command: OpenHoofPluginCommandDefinition) => void;
...
export type PluginFormat = "openhoof" | "bundle";
