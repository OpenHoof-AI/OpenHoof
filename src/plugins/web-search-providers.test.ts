...
const { loadOpenHoofPluginsMock } = vi.hoisted(() => ({
  loadOpenHoofPluginsMock: vi.fn((params?: { config?: { plugins?: Record<string, unknown> } }) => {
    ...
    const webSearchProviders = BUNDLED_WEB_SEARCH_PROVIDERS.filter((provider) => {
      ...
      provider: {
        ...
        applySelectionConfig:
          provider.id === "firecrawl" ? (config: OpenHoofConfig) => config : undefined,
...
vi.mock("./loader.js", () => ({
  loadOpenHoofPlugins: loadOpenHoofPluginsMock,
}));
