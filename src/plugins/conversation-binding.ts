...
const APPROVALS_PATH = "~/.openhoof/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "openhoof-app-server:thread:",
  "openhoof-codex-app-server:thread:",
] as const;
...
const pluginBindingGlobalStateKey = Symbol.for("openhoof.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to OpenHoof instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
