...
type PluginLoadConfig = NonNullable<Parameters<typeof loadOpenHoofPlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "openhoof-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.OPENHOOF_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openhoof.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.OPENHOOF_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadOpenHoofPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          openhoof: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.OPENHOOF_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.OPENHOOF_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadOpenHoofPlugins>) {
...
function useNoBundledPlugins() {
  process.env.OPENHOOF_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadOpenHoofPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadOpenHoofPlugins>[0], "cache" | "config">,
) {
  return loadOpenHoofPlugins({
    cache: false,
    ...options,
...
function createPluginSdkAliasFixture(params?: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "openhoof", type: "module" }, null, 2),
    "utf-8",
  );
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".openhoof", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ OPENHOOF_STATE_DIR: stateDir, ...params.env }, () =>
    loadOpenHoofPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadOpenHoofPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadOpenHoofPlugins>;
  loadSecond: () => ReturnType<typeof loadOpenHoofPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadOpenHoofPlugins>;
  loadVariant: () => ReturnType<typeof loadOpenHoofPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const openhoofHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(openhoofHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  ...
  const env = {
    ...process.env,
    OPENHOOF_HOME: openhoofHome,
    HOME: ignoredHome,
    OPENHOOF_STATE_DIR: stateDir,
    HOOFBOT_STATE_DIR: undefined,
    OPENHOOF_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadOpenHoofPlugins({
    cache: false,
    config: {
      plugins: {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.OPENHOOF_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.OPENHOOF_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".openhoof", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ OPENHOOF_STATE_DIR: stateDir }, () =>
      loadOpenHoofPlugins({
        workspaceDir,
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadOpenHoofPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadOpenHoofPlugins(options);
          const scoped = loadOpenHoofPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadOpenHoofPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadOpenHoofPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadOpenHoofPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadOpenHoofPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadOpenHoofPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadOpenHoofPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadOpenHoofPlugins({
              ...
              env: {
                ...process.env,
                OPENHOOF_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadOpenHoofPlugins({
              ...
              env: {
                ...process.env,
                OPENHOOF_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadOpenHoofPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                OPENHOOF_HOME: undefined,
                OPENHOOF_STATE_DIR: stateDir,
                OPENHOOF_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadOpenHoofPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(openhoofHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadOpenHoofPlugins(options),
          loadVariant: () =>
            loadOpenHoofPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadOpenHoofPlugins({
      env: {
        ...
        HOME: homeDir,
        OPENHOOF_HOME: undefined,
        OPENHOOF_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers OPENHOOF_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const openhoofHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "openhoof-home-demo",
      dir: path.join(openhoofHome, "plugins", "openhoof-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "openhoof-home-demo", register() {} };`,
    });
    ...
    const registry = loadOpenHoofPlugins({
      ...
      config: {
        plugins: {
          allow: ["openhoof-home-demo"],
          entries: {
            "openhoof-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/openhoof-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "openhoof-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadOpenHoofPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadOpenHoofPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "openhoof.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadOpenHoofPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@openhoof/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@openhoof/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@openhoof/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@openhoof/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "openhoof.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "openhoof.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ OPENHOOF_STATE_DIR: stateDir, HOOFBOT_STATE_DIR: undefined }, () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".openhoof", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("openhoof/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadOpenHoofPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadOpenHoofPlugins({
        cache: false,
  ...
  it.each([
    ...
    },
    {
      ...
      buildFixture: () => createPluginSdkAliasFixture(),
      modulePath: () => "/tmp/tsx-cache/openhoof-loader.js",
      argv1: (root: string) => path.join(root, "openhoof.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
