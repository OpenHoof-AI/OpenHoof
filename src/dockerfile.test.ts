...
describe("Dockerfile", () => {
  it("uses shared multi-arch base image refs for all root Node stages", async () => {
    ...
    expect(dockerfile).toContain("FROM ${OPENHOOF_NODE_BOOKWORM_IMAGE} AS ext-deps");
    expect(dockerfile).toContain("FROM ${OPENHOOF_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${OPENHOOF_NODE_BOOKWORM_IMAGE} AS base-default");
    expect(dockerfile).toContain("FROM ${OPENHOOF_NODE_BOOKWORM_SLIM_IMAGE} AS base-slim");
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG OPENHOOF_INSTALL_BROWSER");
  ...
  it("Docker GPG fingerprint awk uses correct quoting for OPENHOOF_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
