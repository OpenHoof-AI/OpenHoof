...
const OPENHOOF_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of OPENHOOF_COMPILED_SECRET_TARGETS) {
...
const OPENHOOF_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: OPENHOOF_COMPILED_SECRET_TARGETS,
    entriesById: OPENHOOF_TARGETS_BY_ID,
  });
