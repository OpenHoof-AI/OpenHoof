...
function localOpenHoofProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "openhoof",
...
function createLocalOpenHoofResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localOpenHoofProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveOpenHoofUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-reset-"));
    const profileDir = path.join(tempRoot, "openhoof");
    ...
    const ops = createLocalOpenHoofResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveOpenHoofUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-reset-no-own-"));
    const profileDir = path.join(tempRoot, "openhoof");
    fs.mkdirSync(profileDir, { recursive: true });
