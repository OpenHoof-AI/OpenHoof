...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openhoof-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + mustang-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateOpenHoofProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENHOOF_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENHOOF_BROWSER_COLOR).toUpperCase();
