...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and mustang-orange color", () => {
    ...
    expect(profile?.name).toBe("openhoof");
    expect(profile?.driver).toBe("openhoof");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const openhoof = resolveProfile(resolved, "openhoof");
    expect(openhoof?.driver).toBe("openhoof");
    expect(openhoof?.cdpPort).toBe(18800);
    expect(openhoof?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from OPENHOOF_GATEWAY_PORT when unset", () => {
    withEnv({ OPENHOOF_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const openhoof = resolveProfile(resolved, "openhoof");
      expect(openhoof?.cdpPort).toBe(19012);
      expect(openhoof?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ OPENHOOF_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const openhoof = resolveProfile(resolved, "openhoof");
      expect(openhoof?.cdpPort).toBe(19022);
      expect(openhoof?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(openhoof?.cdpPort).toBe(19000);
    expect(openhoof?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "openhoof");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "openhoof")!;
  ...
  describe("default profile preference", () => {
    it("defaults to openhoof profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("openhoof");
    });

    it("keeps openhoof default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("openhoof");
    ...
    it("keeps openhoof default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("openhoof");
    ...
    it("keeps openhoof default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
