  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          openhoof: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OPENHOOF_GATEWAY_PORT;
    process.env.OPENHOOF_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OPENHOOF_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OPENHOOF_GATEWAY_PASSWORD;
