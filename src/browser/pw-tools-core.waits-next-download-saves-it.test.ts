...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOpenHoofTmpDir: vi.fn(() => "/tmp/openhoof"),
}));
vi.mock("../infra/tmp-openhoof-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOpenHoofTmpDir.mockReturnValue("/tmp/openhoof");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".openhoof-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOpenHoofTmpDir.mockReturnValue("/tmp/openhoof-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "openhoof-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "openhoof-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOpenHoofTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "openhoof-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "openhoof-preferred", "downloads")}${path.sep}`),
    );
  });
