...
const { stopOpenHoofChromeMock } = vi.hoisted(() => ({
  stopOpenHoofChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopOpenHoofChrome: stopOpenHoofChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopOpenHoofChromeMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["openhoof", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      openhoof: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.openhoof).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopOpenHoofChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("openhoof browser stop failed: Error: oops");
  });
});
