...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "openhoof" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.OPENHOOF_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "openhoof",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/openhoof" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    openhoof: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchOpenHoofChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "openhoof";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.OPENHOOF_GATEWAY_PORT;
  process.env.OPENHOOF_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.OPENHOOF_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.OPENHOOF_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.OPENHOOF_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.OPENHOOF_GATEWAY_PASSWORD;
  } else {
    process.env.OPENHOOF_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
