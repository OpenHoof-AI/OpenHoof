...
vi.mock("./chrome.js", () => ({
  resolveOpenHoofUserDataDir: vi.fn(() => "/tmp/openhoof-test/openhoof/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveOpenHoofUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "openhoof-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveOpenHoofUserDataDir).mockReturnValue(userDataDir);
