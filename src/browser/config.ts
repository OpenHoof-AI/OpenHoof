import type { BrowserConfig, BrowserProfileConfig, OpenHoofConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENHOOF_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPENHOOF_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openhoof" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENHOOF_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENHOOF_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenHoofConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENHOOF_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_OPENHOOF_BROWSER_PROFILE_NAME]
        ? DEFAULT_OPENHOOF_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "openhoof";
