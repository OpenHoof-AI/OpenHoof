...
export function resolveOpenHoofUserDataDir(profileName = DEFAULT_OPENHOOF_BROWSER_PROFILE_NAME) {
...
export async function launchOpenHoofChrome(
  ...
  const userDataDir = resolveOpenHoofUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_OPENHOOF_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateOpenHoofProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🐴 openhoof browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`openhoof browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`openhoof browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🐴 openhoof browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopOpenHoofChrome(
