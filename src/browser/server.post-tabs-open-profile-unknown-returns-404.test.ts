...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "openhoof" }),
    });
    ...
    const createHoofd = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "legacyhoofd", driver: "hoofd" }),
    });
    expect(createHoofd.status).toBe(200);
    const createHoofdBody = (await createHoofd.json()) as {
      profile?: string;
      transport?: string;
      cdpPort?: number | null;
      userDataDir?: string | null;
    };
    expect(createHoofdBody.profile).toBe("legacyhoofd");
    expect(createHoofdBody.transport).toBe("cdp");
    expect(createHoofdBody.cdpPort).toBeTypeOf("number");
    expect(createHoofdBody.userDataDir).toBeNull();

    const explicitUserDataDir = "/tmp/openhoof-brave-profile";
    ...
    const deleteDefault = await realFetch(`${base}/profiles/openhoof`, {
      method: "DELETE",
    });
