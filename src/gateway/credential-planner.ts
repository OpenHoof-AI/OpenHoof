...
type GatewaySecretDefaults = NonNullable<OpenHoofConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${OPENHOOF_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(
    env,
    ["OPENHOOF_GATEWAY_TOKEN", "HOOFBOT_GATEWAY_TOKEN"],
    includeLegacyEnv,
  );
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["OPENHOOF_GATEWAY_PASSWORD", "HOOFBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}
