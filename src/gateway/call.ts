...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.OPENHOOF_GATEWAY_URL) ??
      trimToUndefined(process.env.HOOFBOT_GATEWAY_URL));
  ...
  const allowPrivateWs = process.env.OPENHOOF_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set OPENHOOF_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: openhoof doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__OPENHOOF_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
