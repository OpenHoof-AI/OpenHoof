...
const { createOpenHoofTools } = await import("../agents/openhoof-tools.js");
...
type SessionSendTool = ReturnType<typeof createOpenHoofTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOpenHoofTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPENHOOF_GATEWAY_PORT", "OPENHOOF_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPENHOOF_GATEWAY_PORT = String(gatewayPort);
  process.env.OPENHOOF_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("OPENHOOF_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createOpenHoofTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
