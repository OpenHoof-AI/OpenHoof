...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("openhoof doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with OPENHOOF_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with OPENHOOF_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.OPENHOOF_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://openhoof-gateway.ai:18789",
      onConnectError,
    });
