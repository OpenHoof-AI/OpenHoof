...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `openhoof-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OPENHOOF_CONFIG_PATH = path.join(root, "openhoof.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "openhoof.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "openhoof-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: OpenHoofConfig) =>
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadOpenHoofPlugins: () => pluginRegistryState.registry,
  };
});
