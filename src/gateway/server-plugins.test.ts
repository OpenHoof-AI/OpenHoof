...
const loadOpenHoofPlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadOpenHoofPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadOpenHoofPlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadOpenHoofPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.openhoof.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
