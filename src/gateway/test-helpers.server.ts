...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "OPENHOOF_SKIP_BROWSER_CONTROL_SERVER",
  "OPENHOOF_SKIP_GMAIL_WATCHER",
  "OPENHOOF_SKIP_CANVAS_HOST",
  "OPENHOOF_BUNDLED_PLUGINS_DIR",
  "OPENHOOF_SKIP_CHANNELS",
  "OPENHOOF_SKIP_PROVIDERS",
  "OPENHOOF_SKIP_CRON",
  "OPENHOOF_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENHOOF_STATE_DIR = path.join(tempHome, ".openhoof");
  delete process.env.OPENHOOF_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPENHOOF_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENHOOF_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENHOOF_SKIP_CANVAS_HOST = "1";
  process.env.OPENHOOF_SKIP_CHANNELS = "1";
  process.env.OPENHOOF_SKIP_PROVIDERS = "1";
  process.env.OPENHOOF_SKIP_CRON = "1";
  process.env.OPENHOOF_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPENHOOF_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openhoof-test-no-bundled-extensions")
    : "openhoof-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".openhoof-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".openhoof-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__openhoofTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__openhoofTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPENHOOF_GATEWAY_TOKEN"]);
  const prev = process.env.OPENHOOF_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENHOOF_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.OPENHOOF_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
