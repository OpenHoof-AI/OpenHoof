...
import { createOpenHoofTools } from "../agents/openhoof-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-openhoof-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-openhoof-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-openhoof-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-openhoof-thread-id")?.trim() || undefined;
  ...
  const allTools = createOpenHoofTools({
