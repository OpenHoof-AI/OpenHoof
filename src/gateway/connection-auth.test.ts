...
function cfg(input: Partial<OpenHoofConfig>): OpenHoofConfig {
  return input as OpenHoofConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: OpenHoofConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  OPENHOOF_GATEWAY_TOKEN: "env-token",
  OPENHOOF_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  const cases: ConnectionAuthCase[] = [
    ...
    {
      name: "includeLegacyEnv controls HOOFBOT fallback",
      cfg: cfg({
        gateway: {
          mode: "local",
          auth: {},
      ...
      env: {
        ...
        HOOFBOT_GATEWAY_PASSWORD: "legacy-password", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      options: {
  ...
  it("resolves config-first token SecretRef even when OPENHOOF env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when OPENHOOF env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
