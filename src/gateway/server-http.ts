...
import { handleSlackHttpRequest } from "openhoof/plugin-sdk/slack";
...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-OpenHoof-Token header (query parameters are not allowed).",
      );
      return true;
    }
