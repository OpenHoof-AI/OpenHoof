...
export function resolveHooksConfig(cfg: OpenHoofConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OpenHoofConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openhoof-token"] === "string"
      ? req.headers["x-openhoof-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-openhoof-idempotency-key"]) ||
