...
const gatewayEnv = {
  OPENHOOF_GATEWAY_TOKEN: "env-token", // pragma: allowlist secret
  OPENHOOF_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
} as NodeJS.ProcessEnv;

function makeRemoteGatewayConfig(remote: { token?: string; password?: string }): OpenHoofConfig {
  return {
    gateway: {
      mode: "remote",
      remote,
...
function withGatewayAuthEnv<T>(env: NodeJS.ProcessEnv, fn: () => T): T {
  const keys = [
    ...
    "OPENHOOF_SERVICE_KIND",
    "HOOFBOT_GATEWAY_TOKEN",
    "HOOFBOT_GATEWAY_PASSWORD",
  ] as const;
...
describe("gateway credential precedence parity", () => {
  const cases: TestCase[] = [
    ...
    },
    {
      ...
      env: {
        HOOFBOT_GATEWAY_TOKEN: "legacy-token", // pragma: allowlist secret
