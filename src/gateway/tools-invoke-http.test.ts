...
let lastCreateOpenHoofToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/openhoof-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateOpenHoofToolsContext?.agentTo,
          agentThreadId: lastCreateOpenHoofToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createOpenHoofTools: (ctx: Record<string, unknown>) => {
      lastCreateOpenHoofToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateOpenHoofToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateOpenHoofToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateOpenHoofToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-openhoof-message-to": "channel:24514",
        "x-openhoof-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
