...
import type { ConfigValidationIssue, OpenHoofConfig } from "../../config/types.openhoof.js";
import {
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: OpenHoofConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadOpenHoofPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    runtimeOptions: {
  ...
  // our cache. However, both loadConfig() and loadOpenHoofPlugins() already cache
