...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: OpenHoofConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: OpenHoofConfig) {
  ...
  async function expectNoTokenGeneration(cfg: OpenHoofConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): OpenHoofConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
    ...
    await expectResolvedToken({
      cfg: {
        gateway: {
          auth: {
            mode: "token",
            token: "${OPENHOOF_GATEWAY_TOKEN}",
          },
        },
      },
      env: {
        OPENHOOF_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${OPENHOOF_GATEWAY_TOKEN}",
    });
  });

  it("uses OPENHOOF_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        OPENHOOF_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
  ...
  it("uses OPENHOOF_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await ensureGatewayStartupAuth({
      cfg: {
        gateway: {
          auth: {
      ...
      env: {
        OPENHOOF_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
