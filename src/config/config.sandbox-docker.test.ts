...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("openhoof-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "openhoof-sandbox-browser-global" },
      agentBrowser: { network: "openhoof-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("openhoof-sandbox-browser-agent");
  });
