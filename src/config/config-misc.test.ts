...
import { buildWebSearchProviderConfig, withTempHome, writeOpenHoofConfig } from "./test-helpers.js";
import { OpenHoofSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = OpenHoofSchema.safeParse({
      $schema: "https://openhoof.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://openhoof.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = OpenHoofSchema.safeParse({ $schema: 123 });
...
describe("ui.seamColor", () => {
  ...
  it("rejects non-hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "mustang" } });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = OpenHoofSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeOpenHoofConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeOpenHoofConfig(home, {
        gateway: { bind: "${OPENHOOF_BIND}" },
      });

      const prev = process.env.OPENHOOF_BIND;
      process.env.OPENHOOF_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.OPENHOOF_BIND;
        } else {
          process.env.OPENHOOF_BIND = prev;
        }
      }
    });
  });
