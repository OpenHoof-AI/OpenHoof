...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OPENHOOF_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-openhoof",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-openhoof#general");
  });
  ...
  it("derives session transcripts dir from OPENHOOF_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENHOOF_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when OPENHOOF_STATE_DIR differs from stored paths", () => {
