...
import type { ConfigFileSnapshot } from "./types.openhoof.js";
import { OpenHoofSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(OpenHoofSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.openhoof/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/openhoof/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/openhoof/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/openhoof/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/openhoof/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OpenHoofSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OpenHoofConfig";
    const hints = mainSchemaHints;
