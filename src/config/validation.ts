...
import type { OpenHoofConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenHoofConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: OpenHoofConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OpenHoofConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenHoofSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenHoofConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenHoofConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as OpenHoofConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as OpenHoofConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; OpenHoof mapped it automatically for compatibility.",
  }));
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native OpenHoof config schema.
