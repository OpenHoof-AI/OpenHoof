...
import type { OpenHoofConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOpenHoofVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  openhoof config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  openhoof config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: OpenHoofConfig,
  pathSegments: string[],
): { changed: boolean; next: OpenHoofConfig } {
...
function coerceConfig(value: unknown): OpenHoofConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenHoofConfig;
}
...
function stampConfigVersion(cfg: OpenHoofConfig): OpenHoofConfig {
...
function warnIfConfigFromFuture(cfg: OpenHoofConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenHoofVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenHoof (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenHoofConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenHoofConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenHoofConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenHoofConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpenHoofConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENHOOF_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OPENHOOF_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPENHOOF_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPENHOOF_WATCH_SESSION === "string" &&
        deps.env.OPENHOOF_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENHOOF_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENHOOF_WATCH_COMMAND === "string" &&
        deps.env.OPENHOOF_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENHOOF_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPENHOOF_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: OpenHoofConfig | null = null;
let runtimeConfigSourceSnapshot: OpenHoofConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENHOOF_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENHOOF_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): OpenHoofConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): OpenHoofConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: OpenHoofConfig;
  candidate: OpenHoofConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: OpenHoofConfig): OpenHoofConfig {
...
export function loadConfig(): OpenHoofConfig {
...
export async function readBestEffortConfig(): Promise<OpenHoofConfig> {
...
export async function writeConfigFile(
  ...
  // edits to openhoof.json remain visible (no stale snapshot).
}
