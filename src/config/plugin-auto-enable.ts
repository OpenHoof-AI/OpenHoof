import { hasAnyWhatsAppAuth } from "openhoof/plugin-sdk/whatsapp";
...
function isWhatsAppConfigured(cfg: OpenHoofConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpenHoofConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return hasMeaningfulChannelConfig(entry);
}
...
function collectModelRefs(cfg: OpenHoofConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpenHoofConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: OpenHoofConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: OpenHoofConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OpenHoofConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: OpenHoofConfig, pluginId: string): OpenHoofConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
