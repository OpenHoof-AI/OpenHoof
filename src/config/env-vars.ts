...
function collectConfigEnvVarsByTarget(cfg?: OpenHoofConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: OpenHoofConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: OpenHoofConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: OpenHoofConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.OPENHOOF_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
