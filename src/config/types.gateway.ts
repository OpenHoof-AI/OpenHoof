...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "openhoof.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.openhoof/workspace/canvas). */
  root?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/openhoof"). */
  basePath?: string;
 ...
 * Used when Hoofbot runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
...
export type GatewayReloadConfig = {
   ...
   * @see https://github.com/openhoof/openhoof/issues/47711
   */
