...
function resolveRepoRoot(): string {
  const fromPackage = resolveOpenHoofPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    OPENHOOF_STATE_DIR: path.join(os.tmpdir(), "openhoof-config-doc-baseline-state"),
    OPENHOOF_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
