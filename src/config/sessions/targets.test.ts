...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): OpenHoofConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: OpenHoofConfig, env: NodeJS.ProcessEnv) =>
    ...
    expect(targets).toEqual([
      {
        ...
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".openhoof/agents/main/sessions/sessions.json"),
        ),
      },
      {
        agentId: "work",
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".openhoof/agents/work/sessions/sessions.json"),
        ),
      },
    ]);
  });
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        OPENHOOF_STATE_DIR: envStateDir,
      };
      const cfg: OpenHoofConfig = {};
