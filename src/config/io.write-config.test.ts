...
describe("config io write", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhoof-config-io-"));
  });
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".openhoof", "openhoof.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".openhoof", "logs", "config-audit.jsonl");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withSuiteHome(async (home) => {
        const stateDir = path.join(home, ".openhoof");
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      const invalidConfig: OpenHoofConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "openhoof config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'openhoof config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withSuiteHome(async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          OPENHOOF_WATCH_MODE: "1",
          OPENHOOF_WATCH_SESSION: "watch-session-1",
          OPENHOOF_WATCH_COMMAND: "gateway --force",
