...
import { ensureOpenHoofExecMarkerOnProcess } from "./infra/openhoof-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "openhoof.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "openhoof.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.OPENHOOF_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.OPENHOOF_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.OPENHOOF_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.OPENHOOF_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[openhoof] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `OpenHoof ${VERSION} (${commit})` : `OpenHoof ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[openhoof] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[openhoof] ${parsed.error}`);
      process.exit(2);
    }
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[openhoof] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
