...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-install-sh-"));
  const cliPath = path.join(root, "openhoof");
...
OPENHOOF_BIN="$FAKE_OPENHOOF_BIN"
resolve_openhoof_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_OPENHOOF_BIN: cliPath,
        OPENHOOF_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("OpenHoof 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("OpenHoof dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("OpenHoof dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "openhoof-install-stdin-"));
